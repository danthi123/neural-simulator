...
| **Grounded communication learning** | One six-seed experiment joins a learned food percept, hunger, request, consequence, and satiety. A second learns a two-intent by two-referent vocal convention from listener consequences, including combinations withheld from training. An isolated neural selector passes its four-seed physiology gate without host channel choice. Three local reward-credit circuits failed the yoked-reward control. A fourth host-timed trace was retired before formal testing. A fifth circuit now produces an audited, action-selective neural trace under one fixed epoch on CPU and GPU, with symmetric outcome modulation and zero changed weights. Its live preregistration separates that trace from learned reward expectation and requires contingent/yoked, omission, permutation, lesion, and plasticity-isolation controls. No learning or formal seed has run. Injected exploration, fixed channels, and a host listener/readout remain. These are preverbal causal loops, not natural language. See the [learned-convention finding](../research/findings/2026-08-03-developmental-vocal-convention-6seed-GO.md), [selector finding](../research/findings/2026-08-03-neural-vocal-selector-gateA-v2-4seed-GO.md), [v5 smoke](../research/findings/2026-08-03-neural-vocal-credit-gateB-v5-cross-backend-smoke-QUALIFIED.md), and [learning preregistration](../research/findings/2026-08-03-neural-vocal-credit-gateB-v5-learning-PREREGISTRATION.md). |
...
timing and backend defects. Its corrected v5 replacement now has an audited
CPU/GPU action trace and a live learning preregistration. The next step is a
collision-checked seed manifest followed by a reserved-seed implementation
smoke that keeps trace and learned expectation separate. Formal seeds remain
sealed until that smoke passes.
