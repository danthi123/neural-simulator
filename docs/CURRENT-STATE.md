...
| **Grounded communication learning** | One six-seed experiment joins a learned food percept, hunger, request, consequence, and satiety. A second six-seed experiment learns an external two-intent by two-referent raw vocal convention from contingent listener responses and correctly composes both combinations withheld from training. Fresh brains also learn a swapped convention; no-consequence, unrelated-reward, dopamine-lesion, context-lesion, and perception-lesion controls fail as predicted. The system still uses injected motor babbling, fixed channels, and a host listener/readout. These are preverbal causal loops, not natural language. See the [single-request finding](../research/findings/2026-08-03-grounded-speech-action-loop-6seed-GO.md) and [learned-convention finding](../research/findings/2026-08-03-developmental-vocal-convention-6seed-GO.md). |
...
The immediate priority is to replace injected vocal exploration with intrinsic
brain-generated exploration and add error-driven unlearning so one brain can
adapt when a listener changes convention. The same loop should then expand to
