...
localized the next step: sparse trace cells remain unable to make the striatal
expectation cells fire before reward even after the complete four-point weight
ladder. The current bounded test increases converging neural action-context
afferents instead, following a prior project diagnostic of the same cell type.
Only after expectation engages will the existing slow GABA-B/GIRK output be
tested in a graded regime so prediction overlaps the outcome. Formal seeds
remain sealed until that reserved-seed dynamics smoke passes.
