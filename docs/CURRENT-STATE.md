...
backend diagnosis is complete for the locked matched-state path: initialization
is byte-identical across all 15 checked arrays, and the corrected 1,200-step
NumPy/RTX replay is byte-exact for voltage, recovery state, and spikes. Both
corrections are opt-in so existing experiments keep their prior behavior. The
next action is to derive replacement V13 calibration and replication seeds
through the fail-closed controller; held-out and learning seeds remain sealed.
