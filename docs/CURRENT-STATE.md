...
measurement on a runner/controller authority mismatch; no seed ran. V4 now
binds those authorities and preregisters fresh mechanically derived calibration
seed `384414` and replication seed `568500`. The next action is to freeze and
commit its complete source, then freeze the controller configuration. Held-out
and Stage-1 seeds remain sealed.
