...
retired. The command emitters and tests are corrected. V3 is preregistered with
fresh mechanically derived calibration seed `577995` and replication seed
`578403`. The next action is to freeze and commit its complete source, then
freeze the controller configuration. Held-out and Stage-1 seeds remain sealed.
