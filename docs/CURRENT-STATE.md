...
| Self-monitoring and honesty | Confidence, attention, authorship, and source-monitoring pieces exist. The production conversation hook is default-off and moat-safe. An exact metadata floor caught current high-confidence wrong recalls, and a newer independent RF source-memory echo caught the same six-seed stressed failures without reading the exact source fact. The final learned biological source monitor is still open. |
...
- host-computed novelty, appraisal, confidence, or learning-progress scalars in
  some experiments;
- exact source metadata and engineered source-memory echoes for current honesty
  safety;
...
4. **Self-based honesty.** Abstention and hedging are partly wired into the
   self-model now. The source-memory echo is safer than exact metadata, but the
   final confidence/source signal must be learned from brain activity.
...
The next major build should be a minimal grounded conversation loop:
