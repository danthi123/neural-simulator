...
localized the next step: neither a complete sparse-route weight ladder nor a
`24/64/128/200` single-route convergence ladder made the striatal expectation
cells fire before reward. The prior MSN firing-and-learning diagnostic used two inputs,
a fixed convergent up-state afferent and a separate plastic context afferent.
The next step is a new preregistered, bounded engagement smoke of that distinct
dual-afferent mechanism, with the fixed arm required to remain subthreshold on
its own. Only after learned expectation engages will the existing slow GABA-B/
GIRK output be tested. Formal seeds remain sealed.
