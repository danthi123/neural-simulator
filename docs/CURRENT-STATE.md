...
Replacement calibration and replication seeds are now mechanically fixed, and
the complete candidate source and controller inputs are frozen. A dry readiness
check passed without creating a command or running a simulation. The next
action is replacement calibration on NumPy. CuPy calibration remains blocked
until the CPU artifact is validated and sealed; held-out and Stage-1 seeds
remain sealed.
