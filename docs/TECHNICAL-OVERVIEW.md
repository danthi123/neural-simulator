...
| Grounded communication | A six-seed experiment learned one visual food association and joined hunger, request-or-silence activity, food delivery, and satiety. A six-seed successor learned an externally defined two-intent by two-referent raw vocal convention through consequence-gated local plasticity, including withheld combinations and a swapped convention. Lesions and withheld-consequence controls broke learning or action as predicted. An isolated neural selector passes 4/4 development seeds, and later experiments established pieces of action-local expectation and eligibility, but no construction has yet joined these into clean completed actions with delayed credit. V13's deterministic compatibility passed; its calibration and dependent replication are procedurally undefined because a locked CPU-before-GPU order was violated. The completed paired-state diagnostic shows that the large response split follows backend-native population initialization, while a smaller neuron-update arithmetic divergence remains under identical state and is not fixed by deterministic sparse mode. Corrected calibration stays sealed pending both fixes. Injected motor babbling, fixed channels, and host listener/readout remain; this is preverbal communication learning, not natural language. |
