...
| CCT | GATE 2 shells out to `tools/claim_check.py` on staged findings without ever running its own `--selftest`, so the registry's "refuse a gate whose selftest cannot fail" rule (class 3) never covered it — a broken claim_check (round 5's `errors="replace"` regression, issue 1 below) could ship silently as far as THIS registry is concerned | 1 (round 5's own SOUND-WITH-ISSUES review, 2026-09-25, issue 3) | `gates/claim_check_selftest` — a thin wrapper whose `check()` returns `tools.claim_check.selftest()`, so the registry's `selftest_first` refusal and `run_all` BLOCKING now cover claim_check's own instrument health, not just its per-doc scan | registry, pre-commit | ✅ BLOCKS |
