...
| SR | an `ssh`/`rsync -e ssh` call (or a call to a script known to ssh, e.g. `pool_sync.sh`) sits inside a `while ... read ...; do ... done` loop body — or a function called from one — without `-n` or a stdin redirect, so it silently drains the loop's own stdin and truncates the scan (a partial drain can also dispatch a line FRAGMENT as a job) | 3 (2026-09-25: `pool_queue.sh` queued 1 of N job-file lines twice in one night, fixed `e76106fd8`; `pool_autodispatch.sh`'s `revision_available()` starved pool1+pool2 07:35-09:59 EDT with 74 runnable lines queued, fixed `096dfdae0`; `aws_idle_stop.sh`'s instance loop only ever checks the FIRST instance, unfixed here — see below) | `gates/ssh_stdin_in_read_loop` — a lexical scan (quotes/comments/heredocs masked first, so a REMOTE command string that itself contains the literal text "while/read/do/done" cannot open a spurious local frame) of staged `*.sh` files: every `ssh`/`rsync -e ssh`/known-ssh-script call lexically inside a `while`/`for`/`until`…`read`…`do…done` body, or inside a function called (directly or transitively) from one, must carry `-n`, a stdin redirect, or an upstream pipe. `tools/aws_idle_stop.sh:100/104/116` is the one known live instance this gate finds and does NOT fix (left for a separate lane) | registry, pre-commit | ✅ BLOCKS |
