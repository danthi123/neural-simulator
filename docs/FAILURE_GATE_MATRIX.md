...
| PRA | an AMENDMENT appended to an EXISTING prereg was never checked for run-order at all — `gates/prereg_before_run` (CLASS PR) only sees files git reports ADDED, and its own docstring names an amendment-without-a-log as a declared blind spot | 2 (2026-09-25: the opus review of branch research/slotbinder-gate-latency found the gap; the opus review of this gate's first version found it UNSOUND — it read the default index instead of the one `commit -a` / `commit -- <paths>` commits, returned early when nothing was ADDED, saw only `#` headings (missing 835fc252e, the motivating slot-binder commit), blocked on the auto-appended `_provenance/runs.jsonl` and on record subsections, and its provenance half had an inverted premise: 0 true / 3 false blocks over 400 commits) | `gates/prereg_amendment_order` — a commit that MODIFIES a `*prereg*.md` with AMENDMENT ACTIVITY (a new amendment/log entry in any corpus form: `#` heading, `**AMENDMENT N`/`**Amendment N`/`**ADDENDUM`, `**A<n>, <date>` or a dated bold bullet inside an amendment log; or an edit to a committed amendment's body; record subsections such as `### AMENDMENT 6, smoke record` exempt) while also writing a `research/findings/raw/**` data artifact (not `_provenance/`, `.prov.json`, `.progress_*`) BLOCKS; escape `amendment-same-commit: <reason>` among the commit's own added lines. Reads the staged index itself (honours `GIT_INDEX_FILE`), judges merges against every parent, fails CLOSED when git cannot be read. Replay over 2500 main commits: 19 blocks, TP 19 / FP 0 / FN 0 | registry, pre-commit (reads `git diff --cached` itself; the registry's `--diff-filter=A` list is not used) | ✅ BLOCKS |
