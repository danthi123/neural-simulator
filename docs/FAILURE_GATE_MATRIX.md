...
| DMS | a `<!--derived-->` marker exempts more than the numbers it derives: a standalone marker ran to the next `## ` heading, so one marker hid whole sections, and three line-scanner patch rounds each opened a new scope hole (`###` headings, no blank line after a table, a close marker sharing a line, a `# derived` comment in a fence, lists, fence desync) | 1 incident + 8 holes in 3 patch rounds (2026-09-25: "0 checked against 336 artifact values" while 0.1525/0.140 contradicted the cited 0.17/0.1625) | `tools/claim_check.py` scoping rebuilt on a CommonMark parser (markdown-it-py + tables) + `gates/claim_check_scope` (selftest = `SELFTEST_CASES`, one case per hole, both directions, every commit; check = LOW COVERAGE, >= 40 claims and < 5% checked) + `tests/test_claim_check_scope.py` (each case re-run against main and rounds 1-3) | pre-commit G2 + registry (`--diff-filter=A` scope) | ✅ BLOCKS |
...
re-derived the same way: `grep '^| ' docs/FAILURE_GATE_MATRIX.md | awk -F'|' '{print $(NF-1)}'` tabulated to
43 BLOCKS-family + 1 STRUCTURAL + 9 reporting-family = 53.
The 2026-09-25 **DMS** row (BLOCKS, the claim_check derived-marker scope rebuilt on a parser) makes it
44 · 1 · 9 — 54, re-derived with the same tabulation: 44 BLOCKS-family + 1 STRUCTURAL + 9 reporting-family = 54.)
