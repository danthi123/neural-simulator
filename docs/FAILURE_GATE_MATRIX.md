...
| PRA | an AMENDMENT appended to an EXISTING prereg was never checked for run-order at all — `gates/prereg_before_run` (CLASS PR) only sees files git reports ADDED, and its own docstring names an amendment-without-a-log as a declared blind spot | 1 (2026-09-25, opus review of branch research/slotbinder-gate-latency: PR's `--diff-filter=A` scope makes an amendment to an EXISTING `*PREREG*.md` invisible to the same run-order rule a brand-new prereg gets) | `gates/prereg_amendment_order` — (1) mechanical: a commit that MODIFIES a `*prereg*.md` file, adding a new amendment heading (`^#{1,6}\s*amendment\b`, covers every real heading style in the corpus), while ALSO staging any `research/findings/raw/**` artifact, BLOCKS (escape: `amendment-same-commit: <reason>` among the commit's own added lines; skipped during a merge). (2) provenance, best-effort: a newly-added raw artifact whose `.prov.json` `git_sha` is cited by exactly one amendment section elsewhere in the corpus, but is NOT an ancestor (`git merge-base --is-ancestor`) of that amendment's introducing commit (`git log -S"<heading>"`, only trusted at exactly one hit), BLOCKS — any ambiguity in either resolution is a SKIP, never a guessed block | registry, pre-commit (`--diff-filter=A` scope for part 2; part 1 reads the full staged status itself) | ✅ BLOCKS |
