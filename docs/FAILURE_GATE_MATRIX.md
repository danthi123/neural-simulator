...
| CCT | GATE 2 shells out to `tools/claim_check.py` on staged findings without ever running its own `--selftest`, so the registry's "refuse a gate whose selftest cannot fail" rule (class 3) never covered it — a broken claim_check (round 5's `errors="replace"` regression, issue 1 below) could ship silently as far as THIS registry is concerned | 2 (round 5's own SOUND-WITH-ISSUES review, 2026-09-25, issue 3; round 6's review, issue 11: a real regression surfaced as this WRAPPER failing its selftest -- labelled a 'false positive', check() skipped) | `gates/claim_check_selftest` — a thin wrapper whose `check()` returns `tools.claim_check.selftest()` problems VERBATIM, each labelled BROKEN INSTRUMENT, so `run_all` BLOCKS with the instrument's own words; its `selftest()` proves only the wrapper's mechanics with injected fake instruments (broken / healthy / crashing). Pinned from the other side by claim_check's `cct_gate_reports_broken_instrument_verbatim` case, re-derived against round 6's copy of this file | registry, pre-commit | ✅ BLOCKS |
...
**Score: 43 BLOCKING · 1 structural · 9 reporting · 0 ungated — 53 rows.** (The previous line read
