...
| DR | deep research (local record + EXTERNAL literature) NOT done when hammering a wall — the "≥2 levers ⇒ research" line was PRINTED by `before_you_build`, never enforced; CC (>1h runs only) and RM (refuted-register only) both miss an UN-researched lane | 6 (2026-08-09 forgetting arc: 5 mechanism levers + a 6th queued before any deep research, all re-deriving the existing CLS / Phase-1.4 solution) | `gates/deep_research_at_wall` (a mechanism-finding on a lane already carrying ≥ threshold recent findings must carry a `.external_searches.jsonl` entry with a NON-EMPTY external source) | registry | ✅ BLOCKS |
| CT | a committed reusable workflow script fans out `agent()` calls that declare no `model`, so every stage inherits OPUS by default — the exact cost leak (a 6-agent Opus doc workflow burned ~500k tokens; ~50% of the weekly limit in 1.5 days) | 1 (2026-08-19, owner-flagged: "enforce these changes, not just rely on memory") | `gates/workflow_cost_tiering` (a `.claude/workflows/*.js` `agent()` must name its tier; same detector runs in `tools/cost_audit.py` in the heartbeat over ad-hoc scripts) | registry | ✅ BLOCKS |

**Score: 28 BLOCKING · 1 structural · 7 reporting · 0 ungated — 36 rows.** (The previous line read
