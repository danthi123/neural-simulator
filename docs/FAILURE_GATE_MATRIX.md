...
| DMS | a standalone `<!--derived-->` marker opened BLOCK scope until the next `## ` heading, so ONE marker suppressed every number in a whole section (or several); a run reported "0 checked against 336 artifact values" and still printed "every measurement traces to a cited artifact", while two of its numbers (0.1525, 0.140) did not match the artifact it cited (0.17, 0.1625) | 1 (2026-09-25, independent verifier; hand-fixed on main same day — `research/findings/2026-09-24-gap4-transport-ceiling-bound-census-clamp-load-bearing-fullsize-UNDEFINED.md`, 43 numbers now checked) | `tools/claim_check.py` itself (a standalone marker now scopes to its own paragraph/table — closes at the next blank line AFTER content starts, the next heading, or an explicit `<!--/derived-->` — instead of the whole rest of the section; `selftest()` runs on EVERY commit via pre-commit GATE 2, not just when the file changes) + `gates/claim_check_coverage` (second, independent layer: a SUBSTANTIAL doc, >= `LOW_COVERAGE_MIN_TOTAL` claims, that checked none/almost-none of them is flagged corpus-wide regardless of which mechanism produced the suppression; floor calibrated against a 2026-09-25 retro-scan of the 353 findings added since 2026-09-01 — 71 legitimately-all-derived short docs, up to 39 claims, must keep passing) | pre-commit G2 (selftest, every commit) + registry (coverage, staged + full-corpus audit) | ✅ BLOCKS |

**Score: 43 BLOCKING · 1 structural · 9 reporting · 0 ungated — 53 rows.** (Re-derived after this session's +1
row, **DMS** (BLOCKS): `grep '^| ' docs/FAILURE_GATE_MATRIX.md | awk -F'|' '{print $(NF-1)}'` tabulates to 43
BLOCKS-family (41 plain `✅ BLOCKS` + 2 parenthetical variants) + 1 STRUCTURAL + 9 reporting-family (6 `🟡
REPORTS` + 1 `🟡 REPORTS (40 live hits)` + 1 `🟡 PARTIAL` + the anomalous `✅ REPORTS` row **Y**) = 53, matching
`grep -c '^| '` minus the header row. The previous line read
