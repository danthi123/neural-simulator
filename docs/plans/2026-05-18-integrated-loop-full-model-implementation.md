...
- **Working-memory query readout (`wm`).** Present a queried role (including a novel composed role-filler combination that was not drilled, to require genuine relational generalization, not memorization). Population-vote the concept pools for the bound filler. Emit the answer ONLY if the trustworthy grounded-output gate passes; otherwise abstain (a wrong emission and an abstention on a groundable query both score 0; a correct gated emission scores 1). Accuracy over the N queries = `wm`.
