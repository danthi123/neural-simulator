...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5's frozen-dynamics commit/arousal tag plus symmetric outcome circuit passes reserved smoke on NumPy and CuPy with one configuration and no weight changes; an independent audit is clear and the learning preregistration is live. Formal phases remain sealed. | Commit the collision-checked seed manifest, then build and audit the reserved-seed learning smoke before opening any formal partition. Require actual local learning, yoked neutrality, contingent acquisition, omission, permutation, and all lesions. | S1-S2 |
...
- **GPU, primary:** Gate B v5's host-winner-free commit/arousal mechanism passes frozen-dynamics reserved NumPy/CuPy smoke, its independent audit is clear, and the learning preregistration is live. Commit a collision-checked seed manifest, then implement and audit reserved seed zero before opening any formal phase.
