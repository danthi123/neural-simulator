...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5 learned a separate action-local expectation without plastic leakage on NumPy and CuPy, but dopamine suppression was only `5.56%/8.86%` and omission stayed silent. V6's preregistered four-point weight ladder produced no pre-outcome expectation spikes and is retired. V7 is preregistered from the prior dense-convergent MSN finding. Formal phases remain sealed. | Run v7's bounded `24/64/128/200` trace-size ladder on reserved seed zero, then require learned pre-outcome expectation, graded GABA-B suppression, causal LHb/RMTg omission, permutation, lesions, cross-backend agreement, and an independent audit before assigning formal seeds. | S1-S2 |
...
- **GPU, primary:** Gate B v5 learned action-local expectation but failed its prediction-error and omission outputs. V6 proved that increasing sparse trace-route weight does not create pre-outcome expectation. Implement and audit preregistered v7's dense-convergence ladder on reserved seed zero; formal seeds remain sealed.
