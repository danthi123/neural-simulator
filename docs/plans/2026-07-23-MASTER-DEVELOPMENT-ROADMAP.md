...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1 and v2 are NO-GO: the v2 action-value critic preserved contingent learning but yoked reward saturated toward opposite arbitrary actions across its two clean seeds. Gate B v3 is preregistered and unrun. | Test a spiking LHb-RMTg expected-omission path, reward veto, and local critic normalization while retaining contingent acquisition, yoked neutrality, and all causal lesions. | S1-S2 |
...
- **GPU, primary:** Gate B v2 is a clean two-seed NO-GO because a local spiking action-value critic did not prevent arbitrary yoked learning. Gate B v3 is preregistered around a neural expected-omission error and critic normalization; development and held-out seeds remain locked.
