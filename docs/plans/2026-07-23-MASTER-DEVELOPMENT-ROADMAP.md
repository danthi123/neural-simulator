...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO. V4's action-tagged dendritic expectation passes seed-zero NumPy smoke but violates its firing bound on the exact CuPy run (`45.833 > 20 Hz/cell`); formal phases are sealed. | Localize the cross-backend physiology difference on smoke without backend-specific tuning, then retain normalization, yoked neutrality, contingent acquisition, and all lesions on fresh seeds. | S1-S2 |
...
- **GPU, primary:** Gate B v4's dendritic expectation mechanism passes seed-zero NumPy smoke but the exact CuPy run exceeds its bounded value-firing criterion. Diagnose that backend difference on smoke; all scientific seeds remain sealed.
