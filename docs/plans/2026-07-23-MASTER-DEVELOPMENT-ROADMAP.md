...
| **compositional consolidation** (A1) [mechanism: systems-consolidation] | replay does not yet produce reliable hippocampus-independent cortical memory | V1 shared-bridge transfer is NO-GO. V2 local opponent inhibition remained seed-fragile. V3's learned index relay was UNDEFINED. V4's target plateau suppressed intact target firing. V5 (learned CA1→cortex reinstatement) made consolidation causal + hippocampus-independent on both seeds but was NO-GO on seed-413 false recall + order control. **V5+SFA (2026-08-06) CLOSED the interference wall: intrinsic spike-frequency-adaptation one-of-N eviction on the target attractor drives retest false recall under 0.15 on BOTH seeds (load-bearing vs its lesion); the sole residual is the replay-ORDER control** (`2026-08-06-replay-cortical-consolidation-v5-sfa-eviction-closes-interference-wall-order-control-is-next.md`). **2026-08-25: the order-SENSITIVE mechanism this row's residual named is now GO 6/6 on a separate toy consolidation network (board #130)** — a balanced directed-sweep replay + isolated order-STDP reliably potentiates the directional cue→target trace over shuffled (finding `2026-08-25-order-consolidation-recalib-balanced-directed-sweep-replay-6seed-GO`). **2026-08-25 (v8): PORTED onto a real-scale CA1 substrate (n_ca3=1800, ~production scale) — GO 6/6, unchanged margins, all anti-cheats intact** (commits `97358dac8`/`34c117dfb`, finding `2026-08-25-order-consolidation-v8-real-scale-ca1-port-6seed-GO.md`); v8's own residual: assembly:region ratio (~33-67%) still duplicates the toy's, not the biological ~1-5% sparse code. **2026-09-08 (v9): the further-sparsity rung is CHARACTERIZED NO-GO 0/6** (region-scale-up 33%→17%, finding `2026-09-08-order-consolidation-v9-true-sparsity-scaleup-NO-GO-readout-threshold-not-mechanism.md`) — the causal order-STDP mechanism stays intact (attribution 1.0, physical trace stronger 6/6) but a fixed-threshold behavioral readout doesn't transfer to the sparser region; traced to one full-region wiring pathway's fixed in-degree diluting signal as its candidate pool grows. | Next: (1) a region-growth-compensating in-degree law for the one full-region `cortical_association` pathway; (2) re-derive the fixed correct-rate thresholds as a function of the substrate's own operating point rather than an absolute constant; (3) separately, still open: wire v8's real-scale mechanism into the production V5+SFA CA1→cortex system (not yet production-integrated). | S3 |
...
- **SCALING GO/NO-GO = do NOT buy hardware yet** (finding `2026-09-18-scaling-go-no-go-raw-scale-falsified-lever-is-data-do-not-buy-hardware-yet.md`; 10-agent adversarially-verified workflow). RAW param/GPU scale is FALSIFIED as the fluency lever on the broad domain (token-supply capacity-saturated; the realistic large-token analog REGRESSED); the lever is DATA (token-throughput + curation atop a modest capacity floor), owner's skepticism vindicated. The mouth (neural-render) IS the keystone (~half the blocked rows) but the lever there is data, not a bigger GPU.
  **NEXT, in order, all zero-purchase:** (1) FREE analytical gate — DONE, INCONCLUSIVE: the additive-law fit on the existing grid straddles the fluency band (free-fit E~3.85 falsifies, Chinchilla-prior E~3.64 doesn't) because the grid is under-crossed (d384 only run at ~2-4 tok/param); it also SHOWED the joint d384×20-tok/param cell would NOT resolve E (both fits predict ~the same loss there). So (2) the DECISIVE zero-purchase test is heavier: extend d384 to a tok/param range comparable to d96/d192 (~370M tokens) on the EXISTING 3090 (post-game) to pin E; then (3) confirm the winning point survives on the BIOLOGICAL local-rule readout at deployable vocab within the 3090 envelope. Only then is a compute buy justified. Honest fork: else keep Qwen as the scaffold.
  A strategic PLAN doc is to be written on this verdict.
- **One-brain 11-organ default-on flip: real cause found + fixed** (last week's per_neuron_ou_seed cause was DISPROVEN by cheap-confirm; real cause = heterogeneity seed-trap + homeostasis config split, fixed additively on branch research/onebrain-flip-curiosity-fix). NEXT: re-run the 38-faculty regression battery (post-game; GPU/RAM paused for the owner's game) → on all_pass LAND the flip.
- **Landed this cycle:** Rank3 neural-parser reuse for value_choice/gnw extractors (default-off, 22/22 parity); Rank1 semantic-recall prodscale de-risk runner (GPU 6-seed queued); vision S2-template-learning ARC CLOSED (all 3 classes NO-GO, residual is DATA); verb→object NO-GO; weekly report published.

**2026-09-16 (4 WIRING FLIPS LANDED default-ON; host-removal in verify; vision lever; owner asked for a Vikunja freshness pass):**
- **4 spiking wire-ins now default-ON** (owner's #1 metric — FINISH over de-risk). The integrated `/api/brain-chat` no-regression battery (AWS r7i, 128GB numpy) returned all_pass (0/38 regress) for all four: `BRAIN_SPIKING_NOVELTY`/`_ANAPHOR`/`_QROUTE` (commit 1f444173) + **B-curiosity `_XEDGE_CD6` NOW LANDED** (commit 46e83fca, delivers board #197) — the read-isolation NO-GO 3/6 is genuinely CLOSED by the `train_drive_scale=1.5` retune (re-verified 6/6 GO through the production wrapper's own self-test) and the flip is answer-preserving; this resolves the "NEXT rung is its no-regression check" item from the 2026-09-09 entry below. Ledger `onebrain-xedge-curiosity-d6` on_by_default PARTIAL→YES.
- **HOST-REMOVAL RETIRED — scaffold_retired 1→4 (owner's #1 metric moved).** The novelty/anaphor/qroute host fallback paths (the `seen` set / anaphor word-list / qroute if-elif cascade + their `spiking_*_enabled()` gates + `BRAIN_SPIKING_*` flags) are DELETED (merge 15ec47eb → main); spiking is the SOLE path for all three (LESION controls kept, qroute regex extractors kept). Integrated differential verify GO (AWS r7i): branch-default vs main-default /api/brain-chat answers BYTE-IDENTICAL across all 26 probes (md5 eba9e3b5, 0 differ — the qroute tie→GENERIC ripple is unreachable by any real probe) + branch battery all_pass 0/38. Ledger 3 rows scaffold_retired:YES/RETIRED; headline 1→4. Board #216 closed; AWS terminated + SG deleted (no leak).
  Finding `2026-09-16-host-removal-novelty-anaphor-qroute-RETIRED-byte-identical-differential-GO.md`. Follow-on: `multi_turn_agent_v2.py` has a SEPARATE un-retired host-anaphor scaffold.
- **Vision configural-binding: n_glimpses (temporal evidence integration) lifts capability_go 2/6→4/6** (peaks at G=6, plateaus by 8; still partial, not the 5/6 bar) — finding `2026-09-16-vision-nglimpses-temporal-evidence-integration-lifts-capability-2of6-to-4of6.md`. Next lever is the rate-ceiling (a richer S2/C2 code), not more glimpses. 0-token pool.
- **Token `_n2M` bend-test sweep PAUSED at 9/18** (owner gaming; local GPU idle). Resume `game.sh off`; harvest at 18/18 — does the deep-context NLL curve BEND at ~74M tokens? (owner #1 mouth fork; supersedes the AWS-grid NEXT below — fineweb is local now).

**2026-09-09 (overnight; MANY landings):**
- **MOUTH token-scaling DIRECTION test = GO** (finding `2026-09-09-mouth-token-scaling-fineweb-edu-...-DEEP-MARGIN-LIFT-direction-GO`): same-domain FineWeb-Edu training (linattn d192) lifts the wt103 held-out deep-context margin_vs_trigram **+0.204 at depth 10-99** (uniform +0.17–0.22 across deep buckets) vs the -0.286 baseline — the token-supply lever the 2026-09-01 GO predicted, confirmed on the real same-domain scale test; STEP-1's -0.312 NO-GO was a domain-MIX artifact. Still sub-trigram at deep context (-0.082) but climbing decisively.
  **NEXT: the AWS capacity×token-supply grid (spec §5b) — Stage A (d96/d192 × {0.4B,2B}); 2B/10B cells need a ≥70GB-RAM instance (run on AWS, at owner-delegated discretion); d96×0.4B fits locally.** Qwen stays the declared open-prose scaffold until the mouth crosses the trigram at deep context on broad domain.
- **SCAFFOLD-RETIREMENT FLIPS LANDED** (`eca75a3f1`): host rf composer → spiking onebrain composer (developed-brain default) + numpy-KB LTM → substrate-store (memory-in-weights), both default-on, no-regression GO (flip A recall 20/20 answer-identical, flip B 40/40 via query_patient). The owner's 2026-09-04 #1 priority.
- **MEMORY #130 sparsity scale-up WALL FELL** (`12e1ac34`): v10 region-growth in-degree law (the v9 NO-GO's own named fix) = 6/6 GO, anti-cheats intact — v9's NO-GO was a diluted-readout artifact of a fixed constant at a grown scale.
- **B-curiosity → WM cross-edge 6/6 production GO** (`df56bb8d`, lesion-attributable through the live D6 hold-query): the `_XEDGE_CD6` default-on flip is data-justified — NEXT rung is its no-regression check (Claude's-call-with-rigor per the delegation).
- **C-self GNW congruence-spiking default-ON** (`2ed015d0c`); **D-perception** configural-binding under strict anti-cheats = PARTIAL (learning load-bearing 4/6, floor not cleared — next-mechanism named); **gate-hardening** (`8335a6af0`): parallel_audit under-count fix + blocking compute-idle gate + GAME_MODE-awareness.
