...
| **Deductive inference** | HAVE (emergent, spiking); **multi-hop chain NOW WIRED live in production conversation (2026-08-25)** | inheritance/taxonomy/cancellation (EMERGE-26/27); **transitive over EMERGENT codes 6-seed** (`2026-07-08-emerge28-...GO.md`); multi-hop `query_chain` moat/hop; **compositional chain route** (`research/runners/compositional_chain_route.py`, default-ON) dispatches a possessive-clause question ("what does the wolf's prey eat?") to 2 genuinely-spiking `query_patient` hops and DERIVES the answer, framed as the brain's own inference; moat-hardened (untaught hop / ambiguous hop both abstain) and lesion-proven (`BRAIN_CHAIN_ROUTE=0`) live on `/api/brain-chat` (`2026-08-25-integrated-conversational-state-diagnostic-2.md`) | the route fires on ONE hard-coded sentence template (a host regex), not the logical structure of the question — a logically-identical chain phrased as an explicit conjunction gets zero benefit (confirmed live, same finding) → **fix the comprehension front end's fixed-2-content-token arity** (`ChatBrain._neural_question_parse`), then **replace the host regex/role-noun-hint detector with a learned relation-extraction circuit** (named in the route's own docstring) → eventually **workspace-routed re-entrant chaining** (P1.2, GNW global broadcast) |
...
- **NEW (a separate parallel lane, reasoning-frontier + DA-axis, landed the same day, not previously in this
  log): conversational multi-hop reasoning is now WIRED + confirmed LIVE.** The compositional chain route
  (`472ad5dcd`/`c4db3b0c0`/`3bb9bfdf7`) DERIVES an answer over 2 taught facts on the real `/api/brain-chat`
  endpoint ("what does the wolf's prey eat?" -> "grass", framed as its own inference); the DA-axis cupy-interop
  fix (`1c46e00d3`) ended a silent per-turn error that had zeroed 3 default-ON faculties, and curiosity — dormant
  in the 2026-08-25 integrated diagnostic — now fires live. Both confirmed working TOGETHER, plus a live
  lesion proof and the route's own moat hardening, in `2026-08-25-integrated-conversational-state-diagnostic-2.md`
  (task `#141`, board `docs/.vikunja_sync`). **Next action:** the route only matches ONE hard-coded question
  template — fix `ChatBrain._neural_question_parse`'s fixed-2-content-token arity so reasoning generalizes
  across phrasing, not just the one shape that got a bespoke regex (see the finding's section 6 + §2.6's
  updated Deductive-inference row above).
