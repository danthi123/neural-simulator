...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5 learned a separate action-local expectation without plastic leakage on NumPy and CuPy, but dopamine suppression was only `5.56%/8.86%` and omission stayed silent. V6 and v7 could not make expectation fire before reward. V8's dual-afferent ladder had no learned operating point. V9's dendritic route engaged causally, but its output was UNDEFINED because rewarded action `0` was never expressed. V10 put eligibility on the canonical policy routes and proved local coactivity, but both actions crossed in every fixed action window. V11's symmetric corollary-discharge boundary was retired at construction because it activated without motor spikes on NumPy and CuPy. Formal seed `1` and all learning phases remain sealed. | Research how a corollary population remains quiescent until motor-copy input, then recruits temporary symmetric inhibition and autonomously recovers. A successor needs a new gate and topology, not recurrence tuning, a longer warmup, Python stop-on-winner, or host reset. Only after an action-contingent boundary passes may policy reward learning reopen. | S1-S2 |
...
- **GPU, primary:** V10 showed that policy-local eligibility is causal but could not produce one clean completed action. V11's preregistered symmetric corollary-discharge boundary then failed construction on NumPy and CuPy: it fired in the no-action catch with zero motor spikes. Research and preregister a new action-contingent quiescence/release mechanism before another topology; do not tune or rerun V11, consume formal seed `1`, or open policy learning.
