...
**⭐⭐ 2026-08-01 UPDATE — the must-solve core is unlocked; the frontier has moved to INTEGRATION + parallel faculty arcs.**
- **gap#4 on-bridge SURPASSED** (supersedes item 1's "shrunk-task in progress"): transport-free **e-prop + population coding** trains deep credit on the production bridge — √K curve to **K=16 0.926 > LIF ceiling**, decorrelation refuted, clean provenance (`deep-credit-on-spikes` registry). The substrate now LEARNS via a biological local rule. 6-seed strengthening in flight (K=8 seed-variable). **The learned-instructive-signal (the "true crux", §2.8) is now the live gap#4 frontier = arc B.**
...
2. **gap#5 replay follow-ons — replay GO (d6e140bf) + learned band GO (a051d84d) + merge mechanism GO (50255443), finish to closure** **[3090]** *(IN PROGRESS)* — (a) **learned band** (emergence) **DONE 6-seed GO**: STDP + directed traversal grows the forward-asymmetric band (Mehta); learned band replays in the trained direction; (b) **merge — CORE MECHANISM DE-RISKED 6-seed GO**: replay is AdEx-specific (Izh spreads), so the faithful merge is a **wake/sleep phase-switch** (Izh/dt1.0 conversation → AdEx/dt0.1 SWR/sleep replay), memory preserved byte-identical + replay==native; **remaining = wire into the real merged conversational agent + a sleep trigger**; (c) **neural reader** (Bayesian decode is a measurement instrument) *(next)*.
3. **A1 compositional consolidation → lasting world-model** **[3090]** *(being taken; **RE-ROUTED 2026-07-29**)* — `ca1→concept/composer` pathway + NMDA-supported concept assemblies (reuse the P0.3-validated slow-NMDA attractor) + concept-selective replay.
   **⚠️ SLOT-ALLOCATION IS RETIRED AS A METHOD (not as a capability).** Weight-history metaplasticity allocates at N=3-5 (6/6 vs control 0-1/6) and ceilings by N=8-12 across three formulations at 6 seeds, with the code-separation confound controlled; hard-suppression and discrete-count are both WORSE, and subtractive normalisation (`btsp_mean_subtract`) lifts N=12 to only 3/6.
   **▶ THE RE-ROUTE:** the ceiling is an artifact of the **LOCALIST one-fact-per-slot design**, not a missing rule — a **sparse distributed** store over ONE shared population recalls **200 facts at 0.915 with NO allocator**, permuted control at chance at every scale (toy). Next: take it to the substrate via the existing G.20 ensemble machinery, gated on distinct recall at N=3/12/50/200 under permuted + scramble controls.
