...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5's frozen-dynamics commit/arousal tag plus symmetric outcome circuit passes reserved smoke on NumPy and CuPy with one configuration and no weight changes; formal phases remain sealed. | Re-audit corrected v5, then preregister fresh formal partitions requiring actual local learning, yoked neutrality, contingent acquisition, omission, permutation, and all lesions. | S1-S2 |
...
- **GPU, primary:** Gate B v5's host-winner-free commit/arousal mechanism passes frozen-dynamics reserved NumPy/CuPy smoke. Re-audit the corrected controls and preregister actual learning/yoked/omission gates before assigning any formal seed.
