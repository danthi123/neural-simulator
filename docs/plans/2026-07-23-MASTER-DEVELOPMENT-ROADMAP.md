...
**Status:** LIVING master plan. Created 2026-07-23; **last synced 2026-08-04** (current action-credit, source-monitor, replay-consolidation, and visual-identity boundaries are reflected in §§7-8). Update it as results/walls land.
...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v10 established pieces of local expectation, policy eligibility, and selection but did not earn usable credit. V11/V12 action boundaries failed construction. V13 now targets the missing tonic GPi/SNr-like output substrate. Deterministic compatibility earned GO across six seeds and both backends, but seed-`1013` calibration violated its locked CPU-before-GPU order. Calibration and the dependent seed-`1019` replication are procedurally undefined; both seeds are consumed. Seed `1021`, Stage-1 seed `1031`, capability seed `2`, and learning remain sealed. The completed eight-cell transplant diagnostic shows that the large suppression split follows backend-native initialization; a smaller neuron-update arithmetic divergence remains under identical state, and deterministic sparse mode does not fix it. | Freeze and validate backend-neutral initialization plus standardized neuron-update arithmetic on fresh diagnostic-only seeds. Only then run mechanically derived replacement calibration and replication seeds through the fail-closed controller. | S1-S2 |
| **source monitoring** [mechanism: source-monitor-coresidency] | stable learned source attribution on the shared bridge | V1 calibration passed but development repeated on only 2/3 seeds. V2 local FS competition cleared absolute margins but violated no-harm. V3 threshold homeostasis failed both fresh seeds. V4 formal `601/607` are UNDEFINED due to an evaluator signature bug; intact and learning-lesion behavior was also identical. | Preregister a different local mechanism on fresh seeds only after smoke proves a nonzero rival burden that the mechanism causally reduces. | S2-S3 |
| **compositional consolidation** (A1) [mechanism: systems-consolidation] | replay does not yet produce reliable hippocampus-independent cortical memory | V1 shared-bridge transfer is NO-GO. V2 local opponent inhibition remained seed-fragile. V3's learned index relay was UNDEFINED because its required sleep relay and inhibitory loops never activated. V4's target plateau suppressed intact target firing and was retired at smoke. | Replace the failed target-plateau candidate with a different mechanism under a new preregistration and fresh seeds; retain replay-order/content controls and low false recall. | S3 |
...
- **CPU/GPU, primary:** V13 deterministic compatibility earned GO across six seeds and both backends. The subsequent seed-`1013` calibration is procedurally undefined because CPU and GPU started concurrently despite the locked CPU-first order; dependent seed `1019` is diagnostic only, and both seeds are consumed. Held-out seed `1021` and Stage-1 seed `1031` remain sealed.
  The completed eight-cell state-transplant matrix at diagnostic seed `7606856` confirms that backend-native initialization causes the large inhibitory-response split. It also localizes a smaller execution difference to neuron-state arithmetic: `u` and then `v` diverge while conductances remain exact, and deterministic sparse mode does not help. The exact next work is to freeze and validate backend-neutral initialization and standardized neuron arithmetic on fresh diagnostic-only seeds, followed by mechanically derived replacement calibration and replication seeds. RAG retrieval, evidence manifests, and the fail-closed stage controller support research and process integrity; they do not count as brain capabilities.
...
1. **Correct V13 initialization and arithmetic, then close executed-action credit** **[CPU + 3090]** — the eight-cell seed-`7606856` diagnostic is complete: the large suppression split follows backend-native initialization, while identical state still develops a smaller neuron-update divergence that deterministic sparse mode does not remove. Freeze and validate backend-neutral initialization plus standardized neuron arithmetic on fresh diagnostic-only seeds; then rerun calibration and replication with fresh derived seeds under the controller. Only after Stage 0 earns GO return to contingent learning, yoked neutrality, and causal lesion collapse.
