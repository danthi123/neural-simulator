...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v12 established useful pieces but did not earn usable credit. V14 now has source-bound candidate compilation, independent verification, real low/high transport, and authenticated intact/Nap/Cav2.2/SK/HCN readiness arms with raw-trace scoring. Both endpoint five-arm runs returned `UNAVAILABLE` with no scientific verdict because production protocols are incomplete. Held-out and learning seeds remain sealed. | Implement the source-reported 101-spike event-count protocol with bounded no-spike timeout. Keep AHP, HCN current-step, and 12-cell SK cohort gates unavailable until fully specified; only then run scientific causal scoring and return to contingent learning controls. | S1-S2 |
...
- **Stage B experiment engine:** source-bound packets, independent verification,
  low/high real transport, four intrinsic-current lesion arms, digest-bound
  raw-trace scoring, and five-arm receipts are implemented. Both endpoint
  readiness runs completed with null scientific verdicts. The primary methods
  specify rate/CV analysis over 101 spontaneous action potentials, so the next
  runner is event-count-driven with a bounded no-spike timeout; medium-AHP,
  HCN input-resistance, and SK cohort protocols remain fail-closed.
