...
**Status:** LIVING master plan. Created 2026-07-23. Update it as results/walls land.
...
| **Access / global broadcast (GNW)** | HAVE (4 rungs) | Dehaene ignition/broadcast | `_gnw_rung1..4_*` (`2026-07-07-GNW-rung{1,2,3,4}-*GO.md`): ignition, single-content access, report==reasoning, workspace-speaks-answer | Rung-2 winner phase-erratic; no explicit persistent workspace region | see §2.6 |
...
| **SWR replay + temporal context** (the live open wall) | HAVE (drive) / FRONTIER (readout) | `run_swr_replay_phase`, `run_concept_replay_phase`, RANK-1 spontaneous reactivation GO, RANK-2 forward-chain | **recall READOUT blocked** (`2026-07-18-gap5-SWR-replay-readout-BLOCKED-schaffer-conductance-cap-precisely-localized.md`): (a) Schaffer STP fixed; (b) **specificity** — fixed-random dense Schaffer → **learn CA3→CA1 during encoding** (Schaffer LTP, not fixed projection); (c) **order** — intrinsic-fatigue/E→E-STD NEGATIVE → **theta-gamma phase-precession** (research gate 2026-07-23); reverse replay → symmetric CA3 + reward-gated |
...
| **Core affect + standing affect-STATE region** | MISSING → BUILDABLE-NOW **(THE keystone)** | all ingredients spiking (SNc-δ valence, DA-salience arousal, AgRP/POMC interoception) | no persistent integrated state → **`affect` region = slow-NMDA valence×arousal attractor** (Russell circumplex) driven by δ + salience + drive + mood-modulator; holds across a turn, reads onto cognition |
...
| **Epistemic emotions + curiosity** | MISSING → BUILDABLE-NOW **(the reframe)** | all raw signals (familiarity novelty, RPE/surprise, A→W, learning-progress template `_homeostatic_drive_rl_cheap_first_probe.py`) | refuses instead of craves → **invert the moat's ACTION**: DRIVE=gap·learnable(x)→ASK pool via filled `from_novelty`; **REWARD = learning-progress `g_before−g_after`** (Oudeyer/Schmidhuber — the noisy-TV/confab cure); information is DA reward (Bromberg-Martin-Hikosaka); curiosity up-gates plasticity (Gruber-Ranganath) |
...
| Faculty | Tag | HAVE (cite) | WALL → SURPASS |
|---|---|---|---|
| **GNW ignition/broadcast/access** | HAVE | `_gnw_rung1..4` (ignition bimodal, single-content, report==reason, workspace-speaks) | limit-cycle degeneracy → **heterogeneity+noise async attractor**; no eviction → **salience-weighted mutual inhibition** (Rung-2b); no explicit persistent workspace region → **consolidate rungs into one region (P1.2 keystone)** |
...
| **Self-schema (attention/agency, AST)** | BUILDABLE-NOW (DR-3) | inputs exist (GNW occupancy, familiarity scalar, authorship tag); persistence (lineage + lived-fact store) | must be an axis ⟂ content → **train on workspace-occupancy meta-variable** (Graziano/Wilterson AST; DR-3) |
...
| **Belief attribution / false belief** | FRONTIER (flagship social build) | — (the deferred wall in common-ground) | divergent belief store → **agent-keyed fact-store slice (D3 register keyed by agent) with witnessing-gated writes** (transmission-gate push/pop = witnessing); precedent: ToMnet, Bayesian inverse planning, spiking-SNN ToM (Sci Reports 2024) |
...
| **Curiosity / intrinsic motivation** | BUILDABLE-NOW (genuinely unbuilt) + honesty guard | all raw signals; `from_novelty` empty stub (`neuromodulators.py:732`); learning-progress template | noisy-TV/confab trap → **reward LEARNING PROGRESS (g_before−g_after), not raw novelty**; band-pass by learnable(x); ask-only-on-NOVEL (moat by construction) |
...
|---|---|---|---|---|
| **gap#4** | deep multi-layer directed credit for accuracy | rule beats reservoir 6-seed; on-bridge op-point + learned-signal open | (a) op-point: population-coded credit + η-matched + bistable apical hold; (b) **learned self-predicting microcircuit** (Sacramento Eq.9, apical silent when correct — fixes accuracy + moat); (c) **learned feedback** (PAL/KP where FA degrades); Nature-2026 confirms the vectorized apical signal; teacher-bridged interim | S3 (bridged) → S5 (internalized) |
| **gap#5** | SWR-replay recall READOUT | CA3 completion CLOSED; readout blocked (Schaffer specificity + order) | **learn CA3→CA1 Schaffer during encoding** (LTP, not fixed projection) for specificity; **theta-gamma phase-precession** for order (research gate 2026-07-23); reverse replay via symmetric CA3 + reward-gate | S3–S4 |
| **compositional consolidation** | compositional engram stranded in hippocampus | missing pathway root-caused | **add `ca1→composer/concept` pathway + NMDA-supported target + concept-selective replay** — the highest-value memory build (makes the world-model *lasting*) | S3 |
...
| **learned binder over structured codes (gap#2 depth)** | FHRR exact-inverse algebra is a host idealization; multi-attr bundling not learnable-from-scratch on point neurons | single-attr on-spikes GO | **dendritic multiplicative binding** (two-compartment: apical=role, basal=filler, Mikulasch-Priesemann analog pre-spike) + local PPMI normalization for generalization | S3 (frontier) |
| **flat-distinct vs generalizing codes** | two code tracks disjoint | — | **unify perception codes with generalizing PPMI/stream codes under the learned binder** (cross-modal Hebbian convergence, already GO for vision→concept) | S3 |
...
| **rich object recognition / IT invariance-at-scale** | vision-only, no clutter/occlusion | pooler codon works on well-posed sets | **natural-image-patch training of V1→V2→IT with Földiák trace-rule + sparse coding** (on-bridge STDP feature-learning at scale) | S2–S3 |
...
| **no molecular fixation tier** | single-timescale weights | audit item #12 | **two-timescale per-synapse weight (tag + PRP) = synaptic tagging & capture → behavioral tagging** | S2+ |
...
**Already in flight (per `GAP_CLOSURE_MISSION.md` / recent commits):**
...
2. **P0.3 affect-state region** (`_affect_state_region_derisk.py`) — **[CPU]**, the emotion keystone everything downstream reads from. Preceded by DR-2 tag if not already done.
3. **DR-3 self-schema** (`_self_schema_region_derisk.py`) — **[CPU]**, the self-model keystone + the reusable meta-schema class (also unlocks M2 + W3).
4. **P1.2 GNW workspace region + deliberation loop** — **[CPU→3090]**, the architectural integrator keystone; F2-deliberation, F4-self-model, and the "genuine reasoning" claim all route through it.
5. **W3 false-belief register** (`_false_belief_register_derisk.py`) — **[CPU]**, the flagship social-cognition build (reuses D3 register + transmission gate); the single biggest missing conversational faculty.

**Queue to a free GPU lane so idle compute is never wasted:** P3.2 gap#4 seeded deep-credit re-run + `_gap4_PAL_learned_feedback_derisk.py` **[3090/AWS]**; P3.3 causal forward-model probe **[3090]** (the build that unlocks causal + counterfactual + complete deliberation); P2.1 teacher-develop-loop **[3090]**.
