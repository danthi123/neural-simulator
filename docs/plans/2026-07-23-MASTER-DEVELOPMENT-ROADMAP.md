...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5-v9 established local action traces and expectation under narrow controls but did not reach evaluable policy learning. V10 proved eligibility on canonical policy routes, but both actions crossed. V11's recurrent boundary self-activated without motor output. V12's feed-forward disinhibitory boundary passed four causal inhibition audits and recovered autonomously, but activated during warmup, missed the locked guard-suppression floor, and admitted both motor channels on CuPy. Capability seed `2` and all learning remain sealed. | Research a genuinely different mechanism for quiet initialization and stronger motor-contingent release. Retain the successful causal-inhibition and autonomous-recovery controls. Do not tune/rerun V12, erase startup, weaken the criterion, or open policy learning before construction passes on CPU and GPU. | S1-S2 |
...
- **GPU, primary:** V12's preregistered feed-forward disinhibitory boundary passed its four matched inhibitory-path audits and recovered autonomously, but failed construction on NumPy and CuPy. It activated the boundary and both motors during warmup, guard suppression missed the fixed `50%` requirement in every action window, and CuPy admitted both motor channels. Research a genuinely different initialization-control and motor-contingent-release mechanism before another topology; do not tune or rerun V12, consume capability seed `2`, or open policy learning.
