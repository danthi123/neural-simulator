...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5 learned a separate action-local expectation without plastic leakage on NumPy and CuPy, but dopamine suppression was only `5.56%/8.86%` and omission stayed silent. V6 and v7 could not make expectation fire before reward. V8's dual-afferent ladder had no learned operating point. V9's route-level graded dendrite passes reserved engagement at locked center `2`: late expectation is `167/48` by channel, zero without learning, and `1` under the dendritic-route lesion. Its corrected output run passed every protocol check but was UNDEFINED because every block selected `[1, null, 1, null]` and never expressed rewarded action `0`. Formal phases remain sealed. | Research and preregister a successor that restores post-training expression of the learned action through the neural policy without host forcing. Only after that gate passes may a fresh protocol test causal GABA-B reward suppression and omission. Do not repeat or extend v9, reopen its center ladder, tune its output, run NumPy agreement, or assign formal seeds. | S1-S2 |
...
- **GPU, primary:** Gate B v5 learned action-local expectation but failed its prediction-error and omission outputs. V6 and v7 show that neither sparse-route weight nor single-route population size creates pre-outcome expectation. V8's fixed-plus-plastic dual-afferent ladder had no valid operating point. V9 route-level graded dendritic integration passes reserved engagement at locked center `2` with causal learning and plateau lesions, but its corrected output run is UNDEFINED because the neural policy never re-expressed rewarded action `0` in any probe block. Build a research gate for post-training action-policy expression before another output candidate; do not repeat or extend v9 or open formal seeds.
