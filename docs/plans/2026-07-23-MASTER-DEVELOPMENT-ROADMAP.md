...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5 learned a separate action-local expectation without plastic leakage on NumPy and CuPy, but dopamine suppression was only `5.56%/8.86%` and omission stayed silent. V6 and v7 could not make expectation fire before reward. V8's dual-afferent ladder was silent at its only subthreshold point; every stronger fixed input predicted without learning and failed the learning-lesion control. The v9 evidence gate selected the existing graded dendritic plateau as a distinct postsynaptic integration mechanism; formal phases remain sealed. | Build and run the preregistered reserved-seed v9 smoke at fixed centers `16/8/4/2`. Route only the plastic trace-to-expectation synapses through the plateau and require both learning and dendritic-route lesions to remove at least 80% of intact pre-outcome expectation. Do not interpolate, reopen v6-v8, or test GABA-B/omission before engagement passes. | S1-S2 |
