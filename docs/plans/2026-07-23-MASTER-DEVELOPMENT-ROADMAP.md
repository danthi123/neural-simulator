...
2. **gap#5 replay follow-ons — DIRECTIONAL replay GO (d6e140bf) + learned band GO (a051d84d), now finish to closure** **[3090]** *(IN PROGRESS)* — (a) **learned band** (emergence) **DONE 6-seed GO**: STDP + directed traversal grows the forward-asymmetric band (Mehta), and the learned band replays in the trained direction (FWD→forward, REV→reverse); (b) **merge** onto the one-brain co-resident slice *(next)*; (c) **neural reader** (the Bayesian decode is a measurement instrument) *(next)*. The Ecker model-build + the learned band are DONE.
