...
**Status:** LIVING master plan. Created 2026-07-23; **last synced 2026-08-02** (Phase-0 keystones DR-1/DR-3/P0.3/P1.2/W3 all landed → §2 tags GO/QUALIFIED-GO; gap#4 on-bridge launch-bound wall + gap#5 encode-WIN/replay-boundary → §7; §8 re-pointed to the real frontier). Update it as results/walls land.
...
This CONVERGES with the "lower-CV read" surpass above and sharpens it: the leading cause is **credit-factor VARIANCE** (the Izhikevich reset makes each per-example credit signal too noisy to accumulate alignment even though its mean/σ′ is healthy) → the named surpass is a **dendritic apical compartment with plateau-timescale averaging** of the credit factor (the standing dendritic-cortex priority, now implicated by measurement). Cheap decisive next test: measure credit-factor variance LIF-vs-Izh + eligibility-trace-smooth the on-bridge credit and check whether it restores convergence.
