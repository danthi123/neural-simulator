...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO. V3 preserved perfect contingent learning, but yoked reward also reached 100% and intact critic normalization left its expected-omission pathway silent on both clean GPU seeds. | Give bounded learned expectation a persistent effect at outcome time while retaining normalization, yoked neutrality, contingent acquisition, and all causal lesions on fresh seeds. | S1-S2 |
| **source monitoring** [mechanism: source-monitor-coresidency] | stable learned source attribution on the shared bridge | V1 calibration passed but development repeated on only 2/3 seeds. V2 local FS competition cleared absolute margins but violated no-harm. V3 threshold homeostasis failed both fresh seeds. V4 local inhibitory STDP passes real-circuit CPU/GPU smoke and final audit; formal `601/607` remain untouched. | Run exact V4 calibration from one immutable revision with state-matched lesions, adaptive-inhibition controls, and explicit preservation budget. | S2-S3 |
...
| **visual identity learning** [mechanism: visual-identity-spike-latency] | invariant identity still depends on weak representations after host top-k removal | Spike-latency selection removed host top-k. A temporal-binding successor failed both fresh seeds. A hierarchical V2-part to trace-bound-IT successor passes smoke without host first-K; identity-pure tracks and fixed V2 inhibitory afferent current remain disclosed scaffolds. Formal `503/509` are untouched pending final audit. | If audit clears, run exact calibration with learning-off, pixel, RF, temporal, trace, and V1/V2/IT all-fired controls. | S2-S3 |
...
- **GPU, primary:** Gate B v3 is a clean two-seed NO-GO: contingent learning reached 100%, but yoked reward also reached 100% and intact critic normalization left the expected-omission pathway silent. Design a fresh bounded persistence mechanism on smoke seeds; development and held-out seeds remain locked.
...
- **CPU pool, self-monitoring:** source v4 now recruits every source-to-FS pool and changes all six inhibitory routes on reserved smoke seed `600` on CPU and GPU after the original aggregate smoke was rejected. Final audit cleared; run exact formal `601/607` from one immutable archive.
- **CPU/GPU, perception:** after temporal binding failed, a hierarchical V2-part to trace-bound-IT successor now passes smoke without host first-K selection. Keep its disclosed track-boundary and inhibitory-current scaffolds visible while final audit decides whether formal `503/509` may open.
