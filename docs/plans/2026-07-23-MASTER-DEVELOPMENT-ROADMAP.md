...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. V14 compiles and verifies source-bound candidates, runs authenticated intact/Nap/Cav2.2/SK/HCN arms, writes compact authenticated traces, and strictly aggregates the five resolved subgates. An exact 512-point, 24-dimensional Sobol manifest and batched GPU engineering runner are implemented with per-candidate authority policies; NumPy/CPU remains authoritative. The low endpoint was silent and `UNAVAILABLE`; the high endpoint failed Nap-silence and Cav2.2-CV signs. Held-out and learning seeds remain sealed. | Execute the filed screen, confirm viable candidates on authoritative CPU runs, and integrate campaign dispatch and feedback. Keep AHP, HCN current-step, 12-cell SK cohort, and Nap voltage gates unavailable until fully specified; only then proceed to complete causal scoring and contingent learning controls. | S1-S2 |
...
- **Stage B experiment engine:** source-bound packets, independent verification,
  ...
  all 2,560 candidate-arm traces. Strict GPU triage produced 409 failures, 101
  inconclusive candidates, and 2 engineering passes (Sobol 284 and 404).
  The low endpoint was silent and `UNAVAILABLE`; the high endpoint failed
  ...
  search iteration remain to be integrated. Medium-AHP, HCN
  input-resistance, SK cohort, and Nap voltage protocols remain fail-closed.
