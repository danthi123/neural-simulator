...
| **learned binder over structured codes (gap#2 depth)** [mechanism: coincidence-binding] | FHRR exact-inverse algebra is a host idealization; multi-attr bundling not learnable-from-scratch on point neurons | single-attr on-spikes GO | **SlotBinderComposer** (competitive WTA slots + on-bridge Hebbian write + pointer indirection; 6-seed GO at SMALL scale). ⚠️ 2026-09-04c: the 404-fact scale de-risk came back **NO-GO** on the DENSE all-to-all slot→filler wiring (968M synapses / ~463GB by the consumer-hardware gate, does NOT fit the 3090; incumbent FHRR `RFPhasorComposer` measured 334MB/0.9s/correct at the same scale, ~1000× lighter) — banks the dense-wiring METHOD, not the capability. Next rung = **L2 sparsify the slot→filler pathway** (~40× fewer synapses) then re-de-risk; FHRR stays the efficient+correct incumbent meanwhile (the dendritic-multiplicative-binding lever is still ruled out — worst generalizer). | S3 (frontier) |
...
**2026-09-04b (NEWEST — full-force session, usage reset; MANY landings):** FLIP LANDED (`4ea2ff74` — linattn is the DEFAULT spiking own-voice mouth). SCAFFOLD-RETIREMENTS landed: recall-gate→real-LTM (`bbff5076`, recall 0/4→4/4 grounded — the real one-brain bottleneck cleared); affect-coupling→NEURAL (`87631edf`, a neuromodulator gain on the spiking read, PARTIAL: mood-lexicon SOURCE still host); gpu-queue dispatcher tracking-loss FIXED + deployed (`44f3ef85`). VSA composer→learned ROADMAP landed (`78cdaedf`) — ⚠️ but the SlotBinder wholesale path came back NO-GO at live scale (`1a48c1008`: dense all-to-all wiring 968M synapses / ~463GB, does NOT fit the 3090; FHRR is ~1000× lighter + correct).
CORRECTED path (2026-09-04e): L2-SPARSIFY the slot→filler pathway (fanout=32 = 6/6-perfect on composition; VRAM-fit check pending), NOT the dense wholesale swap; FHRR stays the efficient incumbent until L2 clears.
IN FLIGHT: num/den fully-spiking re-do; one-brain Stage-1 (retire the open-ended Qwen fallback); SlotBinder-at-404-fact-scale de-risk (the gate on wiring SlotBinder default); + the prioritized fluency-scale GPU sweep. NEXT: harvest those → wire SlotBinder if it fits the 3090 → one-brain surface-Qwen retirement → continuous-learning (still deferred).
