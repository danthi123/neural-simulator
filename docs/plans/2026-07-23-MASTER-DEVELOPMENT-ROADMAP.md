...
Corrected next mechanism: a **LEARNED feedback (Kolen-Pollack / weight-mirror rotates B→W, sidestepping the failing W→B rotation)** re-tested at this operating point, and/or a two-compartment dendritic credit with a different FA fixed-point; an interventional settle-steps (temporal-averaging) sweep is confirming averaging does not help.
