...
| **open-ended non-fact generation (R4)** | production can't exceed bounded corpus-attested inventory (~4 orders too small) | biggest categorical gap. **⭐ 2026-09-01 REFRAME — GO-TOKEN-LEVER 6/6 (`2026-09-01-generative-cortex-token-supply-lever-broad-domain-plateau-is-starvation-not-capacity-wall.md`): the broad-domain fluency plateau is TOKEN-STARVATION at a CAPACITY-MATCHED model, NOT a capacity/architecture wall.** A single-variable token-supply sweep on the WKV cortex at a fixed SMALL capacity (d96/V2000) lowers broad-domain deep-NLL monotonically (mean −0.646 nats, 6/6), still descending at 4.5 tok/param (2.7× past the record's ~1.7 d512 operating point), with the margin over a fair trigram GROWING with tokens (OPPOSITE of the d512 "flat-while-trigram-improves" result). ⇒ "~4 orders of PARAMS" over-states the wall at a matched operating point; the binding constraint is MATCHED-QUALITY TOKEN SUPPLY. Honest residual: top NLL 3.93 still ~0.24 nats above a ppl20-40 fluency band and tok/param only 4.5 vs Chinchilla ~20. | **scale the biologizable WKV cortex on MATCHED-QUALITY TOKEN SUPPLY = distillation-as-DATA (phi/TinyStories recipe — train ON teacher-generated broad-topic simple-style text; NOT a train-time soft-target loss, banked NEGATIVE 2026-05-16) + the capacity×tokens×vocab surface + gap#4 deep credit + developmental domain growth; also scale the spiking HTM Temporal-Memory generator** (`fused_htm_permanence_update`); retire the RA-21M transformer | S5 |
