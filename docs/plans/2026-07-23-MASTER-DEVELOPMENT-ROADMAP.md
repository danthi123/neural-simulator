...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. V14 now compiles and verifies source-bound candidates and runs authenticated intact/Nap/Cav2.2/SK/HCN production arms. The low endpoint was silent and `UNAVAILABLE`; the high endpoint failed Nap-silence and Cav2.2-CV signs. Held-out and learning seeds remain sealed. | Batch authenticated candidate screening under the resolved causal subgates and compact timeout traces. Keep AHP, HCN current-step, 12-cell SK cohort, and Nap voltage gates unavailable until fully specified; only then proceed to complete causal scoring and contingent learning controls. | S1-S2 |
...
- **Stage B experiment engine:** source-bound packets, independent verification,
  four intrinsic-current lesion arms, authority-pinned 101-spike or timeout
  production traces, digest-bound scoring, and five-arm receipts are
  implemented. The low endpoint was silent and `UNAVAILABLE`; the high
  endpoint failed Nap-silence and Cav2.2-CV signs. Next, batch candidate
  screening and compact timeout traces; medium-AHP, HCN input-resistance, SK
  cohort, and Nap voltage protocols remain fail-closed.
