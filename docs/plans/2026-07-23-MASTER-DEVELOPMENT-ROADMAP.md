...
IN FLIGHT: num/den fully-spiking re-do; one-brain Stage-1 (retire the open-ended Qwen fallback); + the prioritized fluency-scale GPU sweep. UPDATE 2026-09-04d: the SlotBinder-at-404-fact-scale gate is RESOLVED — L1 dense NO-GO then L2 sparsify GO (`research/findings/2026-09-04-slotbinder-L2-sparse-fanout-derisk-GO-fits-3090-and-composes.md`, fanout=32 fits the 3090 + 6-seed composition GO). NEXT: L3 wire `composer_kind="slotbinder"` as the production default → one-brain surface-Qwen retirement → continuous-learning (still deferred).
