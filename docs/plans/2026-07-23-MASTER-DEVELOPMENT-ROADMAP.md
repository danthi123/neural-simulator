...
2. **gap#5 replay follow-ons — DIRECTIONAL replay is GO (d6e140bf), now finish to closure** **[3090]** *(IN PROGRESS)* — (a) **learned band** (emergence): grow the asymmetric near-diagonal band from a directed-traversal stream via STDP (Mehta asymmetric place-field expansion) instead of hand-wiring it — the emergence-bar version; (b) **merge** onto the one-brain co-resident slice; (c) **neural reader** (the Bayesian decode is a measurement instrument). The Ecker model-build itself is DONE.
