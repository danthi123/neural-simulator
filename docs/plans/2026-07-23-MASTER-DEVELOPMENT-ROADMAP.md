...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. V14 compiles and verifies source-bound candidates, runs authenticated intact/Nap/Cav2.2/SK/HCN arms, writes compact authenticated traces, and strictly aggregates the five resolved subgates. The fresh V3 successor screen covered Sobol indices 512-1023 across 512 candidates and completed all 2,560 GPU arm traces. Strict triage returned 421 engineering failures, 91 engineering-inconclusive candidates, and 0 engineering passes. A preregistered 36-trace diagnostic then found non-recovering firing despite restored simplified fast-Na availability and incompatible calcium-SK regimes, retiring the current single-compartment packet as a structural engineering NO-GO. | Build and validate a source-bound fast-Na/Kv3, soma/proximal-dendrite, local-Cav2.2-SK successor before another parameter search. Keep candidates 284 and 404 closed, the 12-cell SK cohort unavailable, and held-out/learning seeds sealed; then integrate campaign dispatch and feedback. | S1-S2 |
...
- **Stage B experiment engine:** source-bound packets, independent verification,
  ...
  search iteration. Candidates 284 and 404 remain closed, and the heterogeneous
  12-cell SK cohort remains fail-closed.
