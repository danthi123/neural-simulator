...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v12 established useful pieces but did not earn usable credit. V13 adds tonic GPi/SNr-like output, but calibration seed `1013` violated locked CPU-before-GPU order; it and dependent replication `1019` are consumed and undefined. Backend-neutral initialization is exact at step zero. Replay v1 localized a step-1,114 subnormal mismatch; after removing GPU flush-to-zero, replay v2 kept voltage, recovery, and spikes byte-exact for all 1,200 rows on NumPy/RTX. Held-out and learning seeds remain sealed. | Derive replacement calibration and replication seeds through the fail-closed controller. Only after Stage 0 earns GO return to contingent learning, yoked neutrality, and causal lesion collapse. | S1-S2 |
