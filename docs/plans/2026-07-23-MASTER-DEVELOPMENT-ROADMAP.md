...
**Status:** LIVING master plan. Created 2026-07-23; **last synced 2026-07-24** (Phase-0 keystones DR-1/DR-3/P0.3/P1.2/W3 all landed → §2 tags GO/QUALIFIED-GO; gap#4 on-bridge launch-bound wall + gap#5 encode-WIN/replay-boundary → §7; §8 re-pointed to the real frontier). Update it as results/walls land.
...
| **rich object recognition / IT invariance-at-scale** (A4b) | vision-only, no clutter/occlusion; V2/IT possibly inert | pooler codon works on well-posed sets; deferral-audit **validate-or-retire V2/IT** queued | **DiCarlo position-invariance test + Földiák trace-rule + competitive pooling**; else retire STDP V2/IT and standardize on the validated V1→pooler codon; scale via **natural-image-patch training of V1→V2→IT** (on-bridge STDP at scale) | S2–S3 |
...
- **gap#5 SPLIT:** sharp-band **ENCODE-WIN** 6/6 no-dendrites (fe12ce2c, resolves §5 fragility); **REPLAY-BOUNDARY** — needs the exact **Ecker-2022 CA3 model-build** (5cf4a205 supersedes the old Schaffer/phase-precession surpass).

**The real frontier now (the next to queue — the Phase-0 set is banked, so these are it):**
1. **gap#4 on-bridge surpass — shrunk-task** **[3090]** *(IN PROGRESS)* — fit a trainable-scale spiking self-predicting net inside the launch-bound compute budget; then answer learned-vs-fixed feedback on spikes. Also fix the **±5 BDSP weight-clamp** (`fused_bdsp_update` clips at lr=0, 6a9a44c3).
2. **gap#5 Ecker-2022 CA3 model-build** **[3090/CPU]** — banded recurrent matrix + E/I-transient SWR state + adaptation regime that permits the moving-bump ordered replay.
3. **A1 compositional consolidation → lasting world-model** **[3090]** *(being taken)* — `ca1→concept/composer` pathway + NMDA-supported concept assemblies (reuse the P0.3-validated slow-NMDA attractor) + concept-selective replay.
4. **A4a on-bridge V1 self-org** **[3090]** *(being taken)* — retinal-wave rate-Hebbian lift of the numpy-B1 GO onto the bridge.
5. **On-bridge spiking realizations of the Phase-0 GOs + wire into the develop-loop teacher hook** **[CPU→3090]** — DR-1/DR-2/P0.3/DR-3/P1.2/W3 as persistent co-resident slices driven by the teacher's turn-by-turn contingency (Kuhl); this is what turns the keystones into a developing brain.
...
**Queue to a free GPU lane so idle compute is never wasted:** P3.3 causal forward-model probe **[3090]** (unlocks causal + counterfactual + complete deliberation); P2.1 teacher-develop-loop **[3090]**; the LM width-ladder (162M/267M) continues on AWS underneath.
