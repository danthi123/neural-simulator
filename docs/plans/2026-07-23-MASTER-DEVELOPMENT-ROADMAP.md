...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v3 are NO-GO and host-timed v4 is retired. V5 learned a separate action-local expectation without plastic leakage on NumPy and CuPy, but dopamine suppression was only `5.56%/8.86%` and omission stayed silent. V6's four-point weight ladder and v7's `24/64/128/200` single-route convergence ladder both produced no pre-outcome expectation spikes and are retired. Formal phases remain sealed. | Preregister a dual-afferent MSN engagement smoke based on the recovered N9 distinction: fixed subthreshold convergent state plus separate plastic action context. Require learned pre-outcome expectation and causal learning lesion before testing graded GABA-B suppression, omission, cross-backend agreement, or formal seeds. | S1-S2 |
...
- **GPU, primary:** Gate B v5 learned action-local expectation but failed its prediction-error and omission outputs. V6 and v7 show that neither sparse-route weight nor single-route population size creates pre-outcome expectation. Research and preregister the distinct dual-afferent MSN mechanism before another reserved-seed GPU smoke; formal seeds remain sealed.
