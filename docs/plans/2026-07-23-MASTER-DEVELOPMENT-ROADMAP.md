...
| **executed-action credit** [mechanism: vocal-action-credit] | delayed dopamine reinforces arbitrary actions under unrelated reward | Gate A neural selection is 4-seed GO. Gate B v1-v12 established useful pieces but did not earn usable credit. V13 adds the missing tonic GPi/SNr-like output substrate, but calibration seed `1013` violated its locked CPU-before-GPU order; it and dependent replication seed `1019` are consumed and procedurally undefined. The large backend split was traced to initialization. A default-off correction now passes exact NumPy/RTX step-zero parity for all 15 checked arrays. The smaller runtime split was traced to fused multiply-add rounding; a default-off strict device kernel is implemented and focused tests pass. Held-out and learning seeds remain sealed. | Prove exact voltage, recovery, and spike trajectories in a full 1,200-step matched-state replay. Only then derive replacement calibration and replication seeds through the fail-closed controller. | S1-S2 |
...
- **CPU/GPU, primary:** V13 deterministic compatibility earned GO, but calibration seed `1013` violated its locked CPU-first order. It and dependent replication seed `1019` are consumed and procedurally undefined; held-out seed `1021` and Stage-1 seed `1031` remain sealed.
  The backend-origin diagnostic traced the large response split to initialization. The default-off initialization correction has now passed byte-exact NumPy/RTX parity across all 15 checked arrays at step zero. The smaller runtime split was localized to fused multiply-add rounding, and the implemented default-off strict device kernel passes focused tests. The next gate is a full 1,200-step matched-state replay with exact voltage, recovery, and spike trajectories. This is engineering validation, not a new brain capability, and corrected calibration stays sealed until it passes.
...
1. **Validate V13 backend corrections, then close executed-action credit** **[CPU + 3090]** — initialization now matches exactly at step zero, and the arithmetic difference has a tested strict implementation. Run the required 1,200-step matched-state replay next. If it stays exact, derive fresh calibration and replication seeds under the controller; only after Stage 0 earns GO return to contingent learning, yoked neutrality, and causal lesion collapse.
