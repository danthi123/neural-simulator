...
- **CPU/GPU, primary:** V13 deterministic compatibility earned GO across six seeds and both backends. The subsequent seed-`1013` calibration is procedurally undefined because CPU and GPU started concurrently despite the locked CPU-first order; dependent seed `1019` is diagnostic only, and both seeds are consumed. Held-out seed `1021` and Stage-1 seed `1031` remain sealed. Backend-native parameter sampling is the leading confound in the observed inhibitory-response difference. The exact next run is the preregistered byte-identical state-transplant matrix at diagnostic seed `7606856`, followed by any required backend-neutral initialization or arithmetic correction, then mechanically derived replacement calibration and replication seeds. RAG retrieval, evidence manifests, and the fail-closed stage controller support research and process integrity; they do not count as brain capabilities.
...
1. **Resolve V13's backend confound, then close executed-action credit** **[CPU + 3090]** — run diagnostic seed `7606856` without opening held-out seeds; apply any preregistered backend-neutral initialization correction; rerun calibration and replication with fresh derived seeds under the controller; only after Stage 0 earns GO return to contingent learning, yoked neutrality, and causal lesion collapse.
