...
| **learned binder over structured codes (gap#2 depth)** [mechanism: coincidence-binding] | FHRR exact-inverse algebra is a host idealization; multi-attr bundling not learnable-from-scratch on point neurons | single-attr on-spikes GO | **SlotBinderComposer** (competitive WTA slots per (fact,role) + on-bridge Hebbian write + pointer indirection; 6-seed GO, "FHRR fully retirable" — 2026-09-04 composer roadmap `78cdaedf`; SUPERSEDES the ruled-out dendritic-multiplicative-binding lever, the worst generalizer). First de-risk: SlotBinder at 404-fact scale vs the 3090. | S3 (frontier) |
