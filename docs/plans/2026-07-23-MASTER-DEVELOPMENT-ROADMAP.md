...
- **HOST-REMOVAL (the RETIRED step) in integrated verify NOW.** Branch (merge c29c72bb) DELETES the novelty/anaphor/qroute host fallback paths → spiking is the SOLE path. AWS differential verify running (branch-default vs main-default over the 26 probes; the one flagged ripple is the qroute `_extract_route` dead-margin TIE → GENERIC). **NEXT: if clean → merge to main + set the 3 ledger rows scaffold_retired:YES/RETIRED (1→4) + Vikunja sync, THEN terminate the AWS instance + delete SG.** RETIRABLE_NOW ages out 2026-09-30 (the forcing gate).
