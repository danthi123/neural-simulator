...
3. **Multi-bridge 2,048** (32 × 64) — the lifted-ceiling demonstration AND now the only path past the production tier. **✅ MECHANISM DE-RISKED (2026-06-12, finding `2026-06-12-multibridge-graded-derisk-GO.md`):** the cheap-first 3-bridge × 64 de-risk is GO 3-seed — cross-bridge composition (M3: target over the other 63 at 17–24× signal/floor) AND the no-confab moat (M4: zero breaches) **survive on correlated graded codes** (the load-bearing risk), with valid anti-cheats (the M7 control was mis-implemented, diagnosed + fixed + re-run before any GO claim). **Held until the owner green-lights the scale** (it folds into piece iii's integration + the ~2–4 week push). Remaining (build-time, not de-risked): the 32-bridge fan-out (cross-bridge SNR + moat at 6.4× the validated 5-bridge fan-out), semantic-cluster sharding (production), and the full conversational matrix (M5) at multi-bridge fan-out. Compute-feasible because each bridge is small (64 concepts, ~3.46M synapses).
