...
| **closed** (a capability) | the capability's spiking replacement is **`integrated`** (wired + on-by-default + scaffold-retired — see below), not merely a 6-seed GO; **OR** an honest-negative is recorded and the capability explicitly parked. A "closed" capability whose production default still runs the host shortcut is a MIS-USE (2026-08-11: ~40 faculties "closed/GO" while `/api/brain-chat` ran a host pipeline). | "de-risked", "validated opt-in", "GO on a proxy", "banked, not wired" |
| **wired** (a faculty) | it is **reachable from the production endpoint** — a call path from `webapp/server.py` `/api/brain-chat` → the `ChatBrain` it builds → the faculty's code, on SOME request. Reachable only from a `research/runners/*` runner is NOT wired. (`gates/production_integration` Check B) | "de-risked", "runner-only", "GO at runner level" |
| **on-by-default** (a faculty) | it runs in the **DEFAULT config the owner gets** — no opt-in flag, not a `default False`/default-off kwarg, not behind `rich=True`/an unchecked UI box. The default turn exercises it. | "opt-in", "default-off", "wired but gated off", "flag-guarded" |
| **scaffold-retired** (for a faculty replacing a host shortcut) | the host shortcut is **gone from the default path** (deleted or demoted to a test-oracle/verify-only), grep-verified — the spiking faculty, not the host code, computes the result on the default turn | "spiking version alongside the host default", "shadowed by a host fast-path" |
| **integrated / production-default** (a faculty) | **all three hold at once**: `wired` AND `on-by-default` AND `scaffold-retired` (and the de-risk is a GO). **Level-3 "spiking on-by-default" credit additionally needs a LESION test** (disable the spiking path → the default answer CHANGES; a byte-identical flip earns zero). Any one missing ⇒ name the highest level actually reached. | "de-risked", "wired (default-off)", "on-by-default (scaffold not retired)", "byte-identical flip" |
