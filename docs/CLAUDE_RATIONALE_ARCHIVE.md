# CLAUDE.md rationale archive

**What this is.** On 2026-09-25 (owner-approved) `CLAUDE.md` was condensed so that both Claude and the 64K-context
local model (`tools/local_llm/llm.sh claude`, Qwen3.8-27B) start each session with less always-loaded context. Nothing
was deleted. The operative rules stay in `CLAUDE.md`; this file keeps the FULL pre-condense `CLAUDE.md` VERBATIM (as of
commit `577eddb1e`, 25,670 characters): the rationale, the incident history, the owner's directive wording and the
worked examples behind each rule. It is NOT auto-loaded. It sits in `docs/`, which the RAG index covers, so retrieve
from it (`.venv-rag/bin/python tools/rag/rag_search.py "<q>" 5 --corpus doc`) rather than reading it whole.

**Where each original section's operative rule now lives in `CLAUDE.md`:**

| Original section (verbatim below) | Now in the condensed `CLAUDE.md` |
|---|---|
| ACTIVE MISSION (2026-07-23 PIVOT): north-star, plan/board/skim pointers, 5-gap subsumed | "Mission" |
| ACTIVE MISSION: HARD RULES (1) no-defer, (2) speed secondary, (3) one substrate; honesty boundary | "Non-negotiables" 2-5 |
| ACTIVE MISSION: THE LAW | "Non-negotiables" 3 |
| ACTIVE MISSION: SESSION START heartbeat, `parallel_audit.py`, never wait without a heartbeat | "Workflow essentials": session start |
| ACTIVE MISSION: COST-ROUTING + MODEL-TIER | "Non-negotiables" 11 |
| ACTIVE MISSION: cross-session continuation is MANUAL | "Mission" |
| Keep the SUMMARY docs synced | "Workflow essentials": sync |
| One term, one meaning | "Workflow essentials": terms |
| Document structure (W1/W2) | "Workflow essentials": W1/W2 |
| THE WORKFLOW IS NOW ENFORCED, NOT REMEMBERED | "Non-negotiables" 7; "Workflow essentials": failure log, biology bindings |
| THE DEEPEST LESSON (the wall reframe) | "Workflow essentials": at a wall |
| Drift prevention is MECHANICAL | "Workflow essentials": research, `tools.lab`, fail-loudly principle |
| Parallelize ACROSS ROADMAP LANES (`lane_check.py`) | "Workflow essentials": session start |
| Evolve the workflows | "Workflow essentials": evolve |
| When Compacting | "When Compacting" (same PRESERVE list, tightened) |
| Research the record BEFORE building | "Workflow essentials": research, at a wall |
| Standing standard: BRAIN-BASED ONLY | "Non-negotiables" 1 |
| Common Commands, Engine reference, Units, GPU Memory, Agent Style | same headings, shortened |
| Reproducibility (the `cfg.seed` trap) | "Reproducibility" |

The verbatim copy follows, unchanged. Its relative links were written for `CLAUDE.md` at the repository root, so read
`docs/...` and `research/...` links as paths from the root, not from this file's folder.
