...
- **NEXT (research-gated, EMERGE-59): "simulate Broca, don't bolt on an LLM."** The generator is still an ANN scaffold; the read-only research gate (`2026-07-03-simulate-broca-generator-replacement-research-gate.md`) isolated the residual to ~25% (the closed-class furniture — function words, 3sg inflection, neural frame-selection = exactly Broca's catalogued job G.12) and ranked Rung A cheapest-first: extend the already-GO `FrameCQ` competitive-queuing serial-order generator with LEARNED function-word + inflection SLOTS, spell each via the validated A→W read-out over the emergent lexicon, gate with the moat, grade held-out with permuted-slot/cross-frame/function-word-ablation/no-learning controls, 6-seed — renders EMERGE's frames fluently ON SPIKES + retires the 21M ANN for those frames. R4 (open arbitrary prose) honestly deferred (the ~4-orders-too-small wall; the dendritic credit-assignment lever runs in parallel).
