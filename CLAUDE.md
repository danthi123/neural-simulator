...
- **Open frontier (the DEEP residual, master-directive core, research-gate FIRING):** all inference so far rides on HOST-DESIGNED shared/is-a codes; **R-c = the STRUCTURE must EMERGE from experience** (co-occurrence/perception statistics), not be hand-assigned — a new mechanism class (research gate + cheap-first EMERGE-30, path flagged: the PPMI stream cortex + replay + the validated on-bridge rate-Hebbian co-occurrence learning). Also open (build): fold transitivity/production into the console; shared-verb high-order content+context binding (EMERGE-25 sub-problem — the shared verb's cells must be context-specific by subject).
- **CI:** `tests/test_emerge2{1,3,4,5,6,7,8,9}_*.py` (30 tests, CPU/numpy, offline).
