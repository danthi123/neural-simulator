...
- **⇒ the emergent cortex now goes from raw experience (co-occurrence / perception) → discovered categories → inference (inheritance/transitivity) → grammatical grounded conversation, all emergent + unsupervised on one spiking brain, transformer-free, NO `sim/` edit.** Still open (build): fully-spiking kWTA pooler on the bridge; cancellation on emergent codes; couple perception-grounded emergence into the experiential console; shared-verb high-order content+context binding (EMERGE-25 sub-problem).
- **CI:** `tests/test_emerge2{1,3,4,5,6,7,8,9}_*.py` + `test_emerge3{0,1,2,3,4}_*.py` (~44 tests, CPU/numpy, offline).
