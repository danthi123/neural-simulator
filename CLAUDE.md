...
- **Tracked shortcuts / deferred** (per `feedback_end_state_fully_spiking_one_brain_path_by_efficiency`): the generator runs as an ANN (spiking-forward conversion deferred, a validated-mechanism reuse); growth is over pre-allocated concept codes (new CODES = the dendritic frontier); the webapp Interact wire-in is pending (needs owner UI verification). **Open frontier:** OPEN-domain (non-fact) conversation remains the field's genuine wall — managed via domain-constraint + retrieval-augmentation + abstention, not solved.
