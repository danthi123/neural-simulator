...
mechanism that was mis-scoped in isolation. About thirty of these faculties now
run on by default in the live chat; a 2026-08-19 audit lesioned all thirty-one
and found twenty-three genuinely change the reply and none inert, with roughly
fifteen genuinely spiking and lesion-load-bearing. No host scaffold has yet been
fully retired, so this remains co-residency of many faculties in one loop rather
than one true substrate, and much load-bearing cognition still runs in host
numpy. The open problems are fluid open-ended conversation, a single true
substrate, and mechanisms that emerge rather than being hand-wired.
