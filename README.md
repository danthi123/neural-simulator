...
| Area | Honest status |
|---|---|
| Shared brain | Multiple regions and pathways can occupy one simulation and update together. The recall composer now runs on the spiking one-brain store by default, and proven organ pools are merged onto shared bridges by default (surprise and the forward world-model on one bridge; metacognition and pragmatics on a second), with cross-region synaptic interaction proven for one pathway. This is still co-residency of several faculties sharing simulation steps, not one true substrate: organ routing is still host Python. As of the 2026-08-26 landing the tracked production-integration ledger lists 54 faculties in total, 26 of them genuinely spiking and on by default, and zero with their host scaffold fully retired (`scaffold_retired: 0`) — most of the live chat's load-bearing cognition is still host, Python, or NumPy, not the shared spiking substrate. |
...
| Language | The repository contains simple question answering and bounded spiking language experiments, plus a live conversational loop that composes honest grounded replies and honest silences. On an open-ended prompt the brain now volunteers a novel, grounded, plausibility-gated, moat-verified proposition flagged as a guess, and multi-sentence replies are planned by a neural discourse planner. As of 2026-08-26, simple bounded recall sentences ("the dog chases the cat") can be spoken by the brain's own spiking word-order "Broca" render by default; open-ended, free-form prose is still produced by an external Qwen2.5-0.5B transformer acting as a declared articulation "mouth" — that scaffold is not retired, and learning a from-scratch spiking read-out for open prose is a mapped boundary (the spiking few-spike read plateaus at roughly a third of the quality a copied read reaches; the limit is read signal-to-noise, not training data). Fluent, open-ended conversation grounded in ongoing life is not yet achieved. |
...
lesion-load-bearing. As of the 2026-08-26 landing the tracked
production-integration ledger counts 54 faculties in total, 26 of them
genuinely spiking and on by default. No host scaffold has yet been fully
retired (`scaffold_retired: 0`), so this remains co-residency of many faculties
in one loop rather than one true substrate, and most of the live chat's
load-bearing cognition still runs in host Python or NumPy. The open problems
are a fluent open-ended mouth, a single true substrate, and mechanisms that
emerge rather than being hand-wired.
