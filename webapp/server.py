...
def brain_reply(chat, req, source, cache_key) -> JSONResponse:
    ...
    if use_rich:
        try:
            ...
            # fact on a HIGH read (the safe direction). Default-OFF (`BRAIN_CONFIDENCE_FORTHCOMING`; a
            # 2026-08-27 default-ON attempt was reverted the same day, hollow on real traffic -- see
            # webapp/confidence_forthcoming_chat.py) -> this whole block is skipped -> byte-identical to
            # pre-wiring.
            _cf_on = False
            _cf_floor = None
            _cf_saved_plan = None
