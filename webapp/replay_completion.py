...
WHAT HAPPENS (only with a flag ON, only inside that route's replay event):
  ...
  2. ITEM COMPETITION (spiking, assembly-coded; Amendment 7 addendum 7a). The same substrate read gives each concept
     ...
     WHY assemblies (measured before this change, disclosed in addendum 7a): with ONE cell per item the bank's
  ...
  3. IGNITION REQUIRES EVERY CONTENT ROLE (addendum 7a; unanimous, not a majority). The burst reinstates only when
     ...
     order, both disclosed in addendum 7a: (i) NO ignition threshold (discrimination alone) reinstated the bare
...
THE IGNITION POINT (corrected in addendum 7a; two constants, not none). Per role, an item is resolved while the
...
# ASSEMBLY CODING (addendum 7a). Cells per candidate item in the concept bank. Chosen a priori from the bank itself, not
...
# IGNITION REQUIRES ALL CONTENT ROLES (addendum 7a; unanimous -- every value tried below 3 was measured and
...
def record(recs) -> list:
    ...
    role's own competition winner (addendum 7a) whether or not the burst ignited; `items` is what was actually
...
def select_items(inner, sc: dict) -> dict:
    ...
    never enough on its own: the burst needs ALL `IGNITION_MIN_ITEMS` of them to resolve (addendum 7a; unanimous,
