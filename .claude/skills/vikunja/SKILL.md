...
- **⭐ THE BOARD IS A LIVING VIEW, NOT A LANDING-LOG — mark work IN-FLIGHT when you START it, not only when it lands (2026-08-26, owner-flagged: "Vikunja needs to be properly utilized and updated; I feel like you may have been failing to do so").** The prior loop only synced on LANDINGS, so anything in progress (a launched agent/workflow, a long run, a multi-step build/fix) was INVISIBLE on the board and it drifted out of date. Fix: **the moment you launch an agent/workflow/long-run or begin a multi-step build/fix on a roadmap item, IMMEDIATELY create-or-open its task** (priority 3-4, area + ladder label, `(Ref: workflow/run id)`) so the board shows what is ACTIVELY happening at any instant; flip it done/`de-risked` when it lands (the SYNC rule below). **And RECONCILE on the OWNER'S CHECK-IN (and session start), the WHOLE board FIRST:** every open item = really-open, in-flight work marked, resolved items closed, stale items retired — so when the owner asks "what's the status" the board ALREADY answers it. **A status question you cannot answer straight from the board is the exact drift this rule exists to prevent.**
