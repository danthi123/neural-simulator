...
6. **Vikunja board (the human pane) — reconcile it WITH these docs, do NOT leave it to a separate budget.** Earned 2026-09-03: the board drifted "quite out of date" while GAP_CLOSURE stayed fresh all session, because the board sync lived ONLY in the separate `vikunja` skill + the `vikunja_freshness` (VF) threshold-budget gate, which tolerated up to 5 findings of drift and never tripped — I kept the docs current here but treated the board as a separate chore. **Fix: whenever THIS Check I updated a capability's STATUS / FRONTIER / next-action above, run the `vikunja` skill's reconcile for the SAME change (update/close/add the roadmap-LEVEL task, plain language), THEN stamp `docs/.vikunja_sync` (append a dated line) + `git add` it** — the stamp resets the VF budget and proves the sync. The board now moves WITH the docs on the same landing, not on its own budget. (Roadmap-level only: a pure contribution finding that changes no capability status needs no board row, exactly as it needs no roadmap-ledger edit — that is the anti-noise line the VF threshold was protecting, now enforced by *what* triggers the sync rather than *how often*.)
