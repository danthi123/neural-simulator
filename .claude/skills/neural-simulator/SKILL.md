...
- **NEVER let a SUBAGENT run a multi-seed sweep** — it runs it single-threaded AND usually ORPHANS it (a detached child dies when the subagent returns → zero output, the exact 2026-07-06 failure). Division of labor: a subagent BUILDS the runner (+ a cheap 1-seed smoke); the **CONTROLLER fans out + runs the sweep** itself and aggregates.
  ...
  - **2026-08-18 REFINEMENT — the worktree-per-agent BUILD+run+commit case (10 clean landings in one wave).** The orphan premise above ("a detached child dies when the subagent returns") did NOT hold this wave: the agents' background multi-seed runs SURVIVED their turn-ends (one 6-seed ran for HOURS across the agent parking + being resumed). So when an agent BUILDS a NEW runner it must commit NARROWLY in its OWN worktree (the cherry-pick-onto-main consolidation flow), letting it run its own decisive multi-seed IS viable — the runner+artifact+finding+commit stay coherent in one worktree, then the controller cherry-picks. Two costs bit, both preventable up front: (1) agents ran the 6 seeds SERIALLY in one process (a lane ground for hours) → SPEC the agent to FAN OUT per-seed in parallel WITH the BLAS caps `OMP_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 MKL_NUM_THREADS=4` (the same discipline as a controller sweep applies to the AGENT's run too — omitting it spiked load to ~15 this wave). (2) the agent WILL park after launching a long run (arms a Monitor + stops); this recurred 3× DESPITE explicit anti-park prose, so do NOT rely on the prose to prevent it — EXPECT the park and RESUME the agent on its completion notification ("poll YOUR run to completion, then finalize+commit; there is no external resumer"). The one resume round-trip is the accepted cost of the clean single-worktree narrow commit. **Controller-runs-the-sweep (above) still wins whenever the CONTROLLER already owns the runner** (no new build, no cross-worktree commit) — the EMERGE-5 90×-parallel case; the refinement is only for the DELEGATED-build case.
