...
- **NEVER let a SUBAGENT run a multi-seed sweep** — it runs it single-threaded AND usually ORPHANS it (a detached child dies when the subagent returns → zero output, the exact 2026-07-06 failure). Division of labor: a subagent BUILDS the runner (+ a cheap 1-seed smoke); the **CONTROLLER fans out + runs the sweep** itself and aggregates.
  - **The agent's 1-seed smoke MUST run SYNCHRONOUSLY (foreground, in-process) so its NUMBERS land in the report — SPEC every build agent to NEVER launch a background smoke and STOP-WAIT on it** (2026-08-01: every one of ~5 build agents did exactly this, returning an empty *"yielding until the monitor fires"* result → I had to hunt the smoke JSON myself AND SendMessage the agent to resume it for its analysis, a wasted round-trip each time). Two acceptable shapes only: **(i)** run the smoke in-process and report the numbers + design in the SAME return; or **(ii)** return the exact command WITHOUT launching anything — the controller runs + banks. Put this line verbatim in the dispatch prompt: *"run your smoke in the foreground and report its numbers; do NOT launch a background run and stop-wait on it."*
