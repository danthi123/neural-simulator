"""Direction 4 verdict module - adversarial test matrix (>=12 cases).

Mirrors the Direction Q + Direction 3 verdict-test patterns. Tests:
...
def _load_verdict_module():
    spec = importlib.util.spec_from_file_location(
        "direction_4_verdict", _VERDICT_PATH,
    )
...
def test_threshold_tamper_detection():
    ...
    assert isinstance(mod._DIRECTION_4_OB_MIN, float)
    assert isinstance(mod._DIRECTION_4_OI_MIN, float)
    assert isinstance(mod._DIRECTION_4_LOADS, tuple)
    assert isinstance(mod._DIRECTION_4_MIN_SEEDS, int)
...
def test_pass_when_all_cells_clear_bar():
    ...
    assert mod.compute_verdict(per_seed) == mod.DIRECTION_4_PASS
...
def test_negative_when_no_cell_clears_bar():
    ...
    assert mod.compute_verdict(per_seed) == mod.DIRECTION_4_NEGATIVE
...
def test_partial_when_some_cells_clear():
    ...
    assert mod.compute_verdict(per_seed) == mod.DIRECTION_4_PARTIAL
...
def test_void_on_none_input():
    ...
    mod = _load_verdict_module()
    assert mod.compute_verdict(None) == mod.DIRECTION_4_VOID_MALFORMED


def test_void_on_empty_list():
    """Empty list (0 seeds) -> VOID."""
    mod = _load_verdict_module()
    assert mod.compute_verdict([]) == mod.DIRECTION_4_VOID_MALFORMED
...
def test_void_on_fewer_than_min_seeds():
    ...
    for n in (1, 2):
        ...
        assert mod.compute_verdict(per_seed) == mod.DIRECTION_4_VOID_MALFORMED, (
...
def test_void_on_string_per_seed_arg():
    ...
    mod = _load_verdict_module()
    assert mod.compute_verdict("bad input") == mod.DIRECTION_4_VOID_MALFORMED
...
def test_below_threshold_eps_fails():
    ...
    assert verdict == mod.DIRECTION_4_NEGATIVE
...
def test_verdict_never_raises_on_malformed_garbage():
    ...
    for g in garbage_inputs:
        ...
        assert verdict == mod.DIRECTION_4_VOID_MALFORMED, (
