...
def test_case_verdict_and_reason(case, casedir):
    ...
    r = cc._scan(p)
    got = "FAIL" if (r["missing"] or r["unsupported"] or r["low_coverage"]) else "PASS"
...
def test_case_wrong_on_is_re_derived(case, casedir, history):
    ...
    for tag, mod in history.items():
        try:
            ...
            got = None                              # a crash is also "not the expected verdict" -> wrong
        if got != case["expect"]:
            wrong.add(tag)
...
@pytest.mark.parametrize("name,body,flagged_expected", [
    ...
    # a table whose derived rows EACH carry the marker passes -- no table-awareness needed, it is just text
