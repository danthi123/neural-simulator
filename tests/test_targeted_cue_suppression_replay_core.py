...
def test_clean_pass():
    ...
    assert targeted_cue_suppression_replay_verdict(rungs)["gate"] == "PASS"


def test_uniform_ctrl_not_collapsing_is_fail():
    # capability NOT attributable to targeted cue-suppression-during-replay
    # + amplified tag stim + persistent PFC-frame + higher n_replays_per_tag -> FAIL
    rungs = [_rung(2, uniform=0.30)]
    v = targeted_cue_suppression_replay_verdict(rungs)
...
def test_direct_retain_below_floor_is_fail():
    ...
    assert targeted_cue_suppression_replay_verdict([_rung(2, direct=0.50)])["gate"] == "FAIL"


def test_abstain_below_bar_is_fail():
    assert targeted_cue_suppression_replay_verdict([_rung(2, ab=0.50)])["gate"] == "FAIL"


def test_full_below_bar_is_fail():
    assert targeted_cue_suppression_replay_verdict([_rung(2, full=0.10)])["gate"] == "FAIL"


def test_small_load_only_is_works_small():
    rungs = [_rung(2, full=0.88), _rung(3, full=0.60), _rung(5, full=0.40)]
    assert targeted_cue_suppression_replay_verdict(rungs)["gate"] == "WORKS-AT-SMALL-LOAD-NO-SCALE-CONFIDENCE"


def test_below_min_seeds_is_void():
    v = targeted_cue_suppression_replay_verdict([_rung(2, n_seeds=2)])
    assert v["gate"] == "VOID" and v["gate"] != "FAIL"


def test_ladder_mismatch_is_void():
    # 4 is not in _TC_LADDER == (2,3,5)
    assert targeted_cue_suppression_replay_verdict([_rung(4)])["gate"] == "VOID"


def test_nonfinite_is_void():
    assert targeted_cue_suppression_replay_verdict([_rung(2, full=float("nan"))])["gate"] == "VOID"
    assert targeted_cue_suppression_replay_verdict([_rung(2, uniform=float("inf"))])["gate"] == "VOID"
    assert targeted_cue_suppression_replay_verdict([_rung(2, direct=float("nan"))])["gate"] == "VOID"


def test_missing_key_is_void():
    bad = {"N": 2, "n_seeds": 3, "full_acc": 0.9}
    assert targeted_cue_suppression_replay_verdict([bad])["gate"] == "VOID"


def test_empty_nonlist_none_is_void():
    assert targeted_cue_suppression_replay_verdict([])["gate"] == "VOID"
    assert targeted_cue_suppression_replay_verdict("nope")["gate"] == "VOID"
    assert targeted_cue_suppression_replay_verdict(None)["gate"] == "VOID"


def test_bool_not_numeric_is_void():
    assert targeted_cue_suppression_replay_verdict([_rung(2, full=True)])["gate"] == "VOID"


def test_duplicate_N_is_void():
    assert targeted_cue_suppression_replay_verdict([_rung(2), _rung(2)])["gate"] == "VOID"


def test_precomputed_verdict_ignored():
    r = _rung(2); r["verdict"] = "PASS"; r["full_acc"] = 0.10
    assert targeted_cue_suppression_replay_verdict([r])["gate"] == "FAIL"


def test_degenerate_always_abstain_is_fail():
    rungs = [_rung(2, full=0.0, uniform=0.0, direct=0.0, ab=1.0)]
    assert targeted_cue_suppression_replay_verdict(rungs)["gate"] == "FAIL"


def test_degenerate_always_answer_is_fail():
    assert targeted_cue_suppression_replay_verdict([_rung(2, ab=0.0)])["gate"] == "FAIL"


def test_void_and_fail_distinct_with_metadata():
    void = targeted_cue_suppression_replay_verdict([])
    fail = targeted_cue_suppression_replay_verdict([_rung(2, full=0.10)])
    assert void["gate"] == "VOID" and fail["gate"] == "FAIL"
    assert void["gate"] != fail["gate"]
    for d in (void, fail):
        assert "reason" in d and "frozen_bars" in d
    fb = targeted_cue_suppression_replay_verdict([_rung(2), _rung(3), _rung(5)])["frozen_bars"]
