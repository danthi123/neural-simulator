"""Capture a byte-exact golden snapshot of a NON-adapting 2-region bridge run.

Run this ONCE on the UNEDITED bridge (before the input-mean-adapt edit), then the
regression test `test_input_mean_adapt.py::test_byte_identical_when_off`
re-builds the identical bridge, runs the identical steps, and asserts the arrays match this
golden EXACTLY. This is the load-bearing guard that the default-off input-mean-adapt edit is
byte-identical.

The bridge shape mirrors the graded-edit golden (src -> mid (inh) -> dst, src -> dst) so the
snapshot exercises the same E/I-split matvec + g_i path the new step block sits next to. NO
region sets input_mean_adapt=True anywhere — so cp_input_mean_ema / cp_input_mean_adapt_mask
must stay None and the new step block must be unreached.

Usage:  SIM_BACKEND=numpy python -u tests/_capture_input_mean_golden.py
"""
import os
import sys
...
def build_nonadapt_bridge(seed=42):
    """A small inhibitory-feedback bridge: src (exc) -> mid (inh) -> dst (exc), and src -> dst (exc).
    No input_mean_adapt region anywhere. Same shape the input-mean-adapt edit will touch (an
    input-current path), so the snapshot is sensitive to any perturbation of total_input_current_pA."""
...
if __name__ == "__main__":
    ...
    # confirm the new attributes are absent / None on the unedited build (sanity for the edit later)
    has_ema = getattr(sb, "cp_input_mean_ema", "MISSING")
    has_mask = getattr(sb, "cp_input_mean_adapt_mask", "MISSING")
    snap = run_and_snapshot(sb)
    out = os.path.join(_HERE, "_input_mean_golden.npz")
    np.savez(out, **snap)
    print(f"[golden] cp_input_mean_ema attr on unedited bridge = {has_ema!r}")
    print(f"[golden] cp_input_mean_adapt_mask attr on unedited bridge = {has_mask!r}")
