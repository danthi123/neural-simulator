...
2. **A deeper learning rule.** Some ceilings (how much structure it can compose, how deep it can nest ideas) come down to one missing capability: a dendritic learning rule that lets a *deep* network teach itself on spikes. We have this working in an idealized form; getting it to train a real spiking network end-to-end is the live frontier. **New (this cycle):** the *long-range* piece of this gap turned out to be far more tractable than it looked — long-range language is bottlenecked on learning the **input representation on a fixed recurrent scaffold**, not on the hard deep-recurrent rewrite; the fully biology-legal version (local rules, no weight-transport) reaches ~78% of the full-backprop reference and is now being put on spikes with no engine edit (see §9.1).
