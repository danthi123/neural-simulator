...
(3) **Workspace-combines-two-reads GO is now threshold-proof.** The global-workspace result (the substrate fuses two below-threshold organ reads by ignition, an AND not a host if/else) was re-run across the whole below-threshold window (three drive levels) and holds 6/6 at each with every anti-cheat clean — closing the last "it's a fitted threshold" objection.
