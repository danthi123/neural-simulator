...
sleep onset) passed its six-seed test on every check, independently re-checked. Two gaps named with it are now worked:
**resting before sleep** (a fact told 4 hours before bed, with quiet rest in between) is kept on 5 of 6 seeds but not
the sixth, so it does not pass. On that seed the memory started weakest and faded during rest instead of being
refreshed. The rest-time refresh scales with how strong the trace already is, so a weak trace fades. The next attempt,
now being built on practice seeds, makes one replay event fill in the whole pattern (as the hippocampus does) instead of
scaling with the trace. **Letting unimportant facts fade:** the old nightly rule removed a fixed share every night and
forgot even an untouched fact within three nights. It is replaced (off by default) by a rule where each night's
scaling-down matches how much the day added, so forgetting comes from later learning. On the first seed an untouched
fact lasts 7 nights and one followed by 3 new facts a day is lost on day 4; its six-seed test is queued.
**Also tonight:** the brain's own fact-binder passes its production-size check on six seeds at 32 facts. Loading all
404 example facts is not yet practical, so a faster teaching step comes next. The emotion-marker "settle" timing
check came back undecided: the settle step itself adds 0.13 s, inside the 0.3 s budget, but whole-reply times vary too
much to judge, so a sharper measurement is being registered. A local assistant model for terminal work (Qwen3.8-27B)
passed all three agent tasks in the local-model comparison and is set as the default. A pool bug that ran seven
...
own work costs about 0.13 s per emotional reply, inside the limit. But two copies of the brain doing the same planned
...
## 2026-09-24 (late night) — the fact-provenance test passes even when the fallback voice is forced to speak; the shipped default still learns nothing

**Whether a fact in a reply was learned by the brain itself, scored on all six seeds, two ways.** The test built and merged earlier tonight was run to completion and scored: a taught fact is correctly attributed to the brain's own connection change, the "didn't learn it" and "just heard about it" controls correctly stay silent, and nothing the fallback voice already knew leaks into 20 unrelated questions.
This held on all six seeds, both with the fallback voice only stepping in when the brain's own voice fails, and with the fallback voice forced to speak every recalled fact (the harder, adversarial version of the same question). The one miss anywhere is a small early trial run outside the official count, where the fallback voice dropped the taught fact rather than saying anything wrong.
The share of what's said that was actually written by a connection change stays at 0% in the shipped default (nothing there can currently learn that way); it reaches 60-75% once the still-switched-off learning connection is explicitly turned on for the test. Turning that connection on by default is not yet warranted: a broader regression check with everything else running, and a check on the real production configuration, are both still missing. (Finding: `2026-09-24-content-provenance-learned-facts-prod-and-qwenforced-GO-6seed`.)
...
**The #1 metric, measured once across everything with every fix switched on:** 24 of the 26 exercised abilities drive the reply on all six seeds (average 0.949); the shipped default is still about 0.59, because the fixes and the better probes are opt-in. A re-run that also switches on the emotion-marker "settle" fix is finishing on AWS. (Finding: `2026-09-23-allfixes-adequate-battery-6seed-robust-core-24`.)
**A brain-learned word-category circuit** (which of a sentence's words names a thing) passed all eight pre-registered checks on six seeds, replacing a host lookup; it is a route to measuring one of the two abilities the battery cannot yet exercise. Two other lines came back honest no's: open-ended talk on a real production turn (effect 0.094, below the 0.10 bar) and the emotion ladder as a twelfth organ on the shared pool (3 of 6 seeds). (Findings: `2026-09-23-cpu-lane-harvest-language-lexicon-referent-v2-S2-scored-6seed-GO`; the open-ended and D3 verdicts.)

## 2026-09-23 — the #1 metric moves again (episodic memory fully proven), a homeostat hardens the weak spots, and a live version of it honestly falls short

**The #1 metric — how many of the brain's abilities provably drive what it says — is now 23 out of 26 on the adequate-probe measure (was 22).** Episodic memory was the last seed-dependent holdout: one of its six test runs used to flicker between "drives the reply" and "doesn't," and that flicker turned out to be a graphics-card measurement quirk (repeated reductions on the GPU aren't perfectly reproducible), not a fragile ability — reading the same case seven times and taking the majority vote settles it cleanly at "drives the reply" on all seven repeats.
Separately, a new mechanism lets each individual brain instance find its own best internal operating point (rather than everyone sharing one hand-set number), and this measurably firms up the three weakest, closest-to-the-edge abilities in the robust core (one of them by +65%). Trying to make that same idea run live and self-adjusting DURING a conversation (instead of computed once in advance) was tried next and came back an honest no on 5 of 6 test seeds — a real limit of this particular design, not a bug; the next attempt is a detector that notices the danger zone before the brain reaches it, rather than trying to hold a moving target.
A large all-fixes confirmation run cross-checking everything above was still finishing at the time of writing (185 of 186 pieces done), so treat "23" as preliminary until the last piece lands. (Findings: `2026-09-23-episodic-s100-cupy-repeats-7of7-loadbearing-robust-core-23`, `2026-09-23-operating-point-stabilizer-go-6seed`, live-homeostat NO-GO `@67b7fbc9c`.)

## 2026-09-23 — mood still can't darken the mouth's tone by nudging words or by a brain-only neural hookup; the next attempt changes how the mouth GENERATES, not how it picks words afterward

Following up on the "mood only ever brightens, never darkens" result below: a second, brain-based attempt (a direct neural hookup between the feeling-state and the word-choice competition, bypassing any word-nudging formula) was built and tested — and it also fails to make a reply sound colder on a sad or angry mood (measured honestly on all 6 test runs, trustworthy instrument). Both tried methods share the same shape: they nudge WHICH word gets picked, after the fact. With that whole approach now ruled out twice, this specific route is being set aside for outside research rather than a third in-house tweak (the project's own rule against repeated attempts at the same wall without checking the outside literature first).
A different angle is now built instead: let the brain's mood feed into the mouth's generation process itself, from the start, rather than editing its output afterward. It is switched off by default and has not yet been put through the same strict 6-seed pass/fail test. (Findings: `2026-09-23-affect-tone-neural-coupling-6seed-subdelta-NOGO`; the new approach's merge commit `@5e9a7955`.)

On a related note, giving the brain a short pause to settle before answering, plus a longer rest between conversation turns, was tested on the full production brain (not just the small circuit it was first shown on): with that setting on, one specific feeling-related signal now DOES drive the reply on all 6 test runs — but only 2 of those 6 actually needed the new setting to make that happen; the other 4 already worked without it. So this is an honest partial win on the full brain, not the clean "off without it, on with it" result the small-circuit version predicted.

## 2026-09-23 — the brain now writes its OWN synaptic tag when a fact feels surprising, instead of a background formula deciding what to keep

Follow-up to the "remembers surprising news the next day" result below: that mechanism has been rebuilt so the brain's own neurons make the save-or-forget call, instead of an ordinary comparison formula watching the dopamine signal from outside. Concretely, a spiking population that reads dopamine now sets an actual tag directly on the connection carrying that memory, and a separate slower process later decides whether to make that tag permanent — much closer to how real brains are thought to stabilize a memory chemically.
The test result is unchanged and, if anything, more convincing: a fact told as surprising news is still recalled a day later, cutting the dopamine connection makes the brain honestly say "I don't know" instead of guessing, and a plainly-told fact is forgotten either way regardless of the setting — on all 6 test seeds, with a wide safety margin this time. It is still switched off by default and still not part of what the live chat actually uses day to day — hooking it into the real conversation memory and adding a "did we already know this was true a day later" check to the automatic test battery are the next concrete steps. (Finding: `2026-09-23-da-encoding-natural-drive-v3-synaptic-capture-6seed-GO-runner-level`; merge `@c660d535`.)
...
- **"Will scaling up (a bigger GPU / more parameters) unblock our main wall — the brain speaking fluently and broadly in its own voice?" We did the deep research before spending, and the honest answer is: NOT from raw scale.** A 10-part adversarially-checked review of our OWN past scaling runs found that raw model/compute scale is *falsified* as the fluency lever — our token-supply run flattened out (tiny gain across a big data increase, and it started losing to a simple baseline), and the one realistic "buy compute and run it big" test we own actually *regressed*. The external research agrees: fluency is a DATA-QUALITY problem and breadth is a DATA-THROUGHPUT problem on top of a modest one-time size floor — not raw size.
  The owner's instinct ("scale never got us anywhere") is vindicated by our own data. The mouth IS the right wall to attack (it gates ~half the remaining retirements), but the lever there is data, not a bigger card. **Recommendation: do NOT buy hardware yet.** We ran the FREE no-GPU test first (fitting the standard scaling curve to data we already have) — and honestly, it came back inconclusive: it straddles the fluency threshold because our largest model was never trained on enough data to pin the curve's floor. Usefully, that same fit proved the *cheap* follow-up we'd planned wouldn't settle it either — so the decisive test is a longer training run of the largest model on the EXISTING card (no purchase), then a check that the result survives in the deployable biological form.
  Only if those pass is a compute purchase justified. Honest fork recorded: if the *deployable, biological* voice can't reach fluency within a single consumer GPU's envelope, keeping the temporary Qwen "mouth" scaffold is the honest call.
- **The one-brain default-on flip: real cause found (last week's fix was wrong).** The flip was held because turning it on shifted the brain's "engagement" read. Last week's proposed cause (an RNG flag) was *disproven* by direct measurement this session; the real cause turned out to be two build-config mismatches (a heterogeneity seed-trap + a homeostasis setting) between the merged and standalone paths. Fixed additively (no core-code change), and the fix brings the two paths' readings into agreement. The final 38-ability battery re-runs to confirm before the flip lands.
- **Two more host shortcuts wired for retirement** (off by default, proven-unchanged-when-off): the two duplicate hand-written "extractor" steps now route through the brain's own learned parser; and a production-scale spiking-recall de-risk is built and queued.
...
**Later the same night (deep-research-grounded frontier harvest, all on pool/rented compute + local numpy; the owner's GPU kept running the voice-scaling work):**
- **The last hand-written piece of understanding a sentence now has a LEARNED replacement.** Deciding where each phrase begins and ends (sentence segmentation) was the final hand-coded step in the comprehension path — everything after it already learns on the neurons. A new spiking circuit now *discovers* those boundaries from the statistics of the word stream (it predicts the next word and flags the surprising transitions as boundaries): passes 5 of 6 tests, an untrained version collapses to chance (the learning is doing the work), a scrambled-stream version also collapses (the structure, not a word-list cue, carries it), and it parses phrases the old word-list couldn't. Off by default; de-risks retiring that shortcut.
  Grounded in the actual statistical-learning + predictive-coding literature (sources read, not skimmed).
- **Gendered pronouns (he/she/him/her) now recognized by the same neural mechanism** as it/that/they — 6 of 6, off by default, provably unchanged behavior on the original pronouns. Not switched on live yet (needs a gender-agreement check in the resolution step first).
- **Emotion noise-robustness: the fix mechanism is found.** Following the "more data isn't it" result: the fix is combining several independent body/context signals and *learning which to trust* when they're noisy. In a strict matched-budget test this decisively beats a single signal (and beats equal-weighting), the learned trust-weights track the true reliabilities, and it clears the bar on 5 of 6 seeds — one seed short of the strictest all-6 bar, so the *mechanism* is validated even though the strict gate reads partial. The biologically-correct grounding surpass, not text-cleverness.
- **Vision: the decisive lever is now isolated.** A clean single-variable test shows the object-recognition-anywhere lift is carried by "saturating normalization" (a canonical neural computation) — turning it off collapses the result — NOT by feature-binding, which a matched plain pool equals. Binding is banked as not-the-lever; the earlier "binding signal" was real but not binding-specific.
- **The one-brain merge is being switched ON (final safety check running).** The 11-organ shared-substrate default-on flip passed its answer-preservation + one-brain-coherence check on all 6 seeds; the full 38-ability no-regression battery is running now to clear it for default-on (with a one-switch byte-identical escape kept).
...
- **Deleting the old shortcut code (the actual "retirement") is DONE — three more shortcuts fully retired.** Turning the neural versions on is one thing; removing the dead hand-written fallbacks so the neural path is the *only* path is what actually counts on the #1 metric. The deletion landed: the never-forgetting set, the pronoun word-list, and the question-routing priority list (plus their on/off switches) are gone; the brain's own neurons are the only path for all three.
  The whole-brain before/after check on the rented high-memory machine confirmed it: the brain's replies are **byte-for-byte identical** to before across the entire probe set, and none of its 38 checked abilities regress (the one behavior that *could* have changed — how a perfectly-tied question-routing decision breaks — never triggers on any real input). **The project's #1 metric (shortcuts fully retired) moved from 1 to 4** — the first movement since early September. (The safety-net escape switches are also gone with the code; the lesion controls that prove the neural circuits are load-bearing were kept.)
...
- **The brain's own voice (the #1 priority): the "more reading makes it better" lever is confirmed again, across two text sources and three brain sizes, all six seeds — its skill keeps improving with more reading and never plateaus in the range tested.** But the run only reached about 9 million words before stopping short: the loader was told to hold too few passages in memory, so the biggest reading amounts were silently skipped. So we have strong confirmation the lever works, but not yet the answer to "does it eventually level off?" — the actual question. A corrected, much larger run (up to ~74 million words) is already re-queued to answer it.
- **An emotion-circuit tuning sweep tuned a dial that wasn't connected.** The 42-job sweep meant to test whether stronger competition between the two emotion sub-populations sharpens them turned out to move a knob that does nothing in the two-column setup being tested (every job gave identical numbers — the tell). It did re-confirm, now across all six seeds, that the circuit sits at its known limit at the default setting. The corrected sweep — turning the dial that actually controls the competition — is re-queued on the mini-PCs.
- **Deep credit assignment (the learned-interneuron microcircuit):** four of six seeds are in, all currently negative; the last two expensive seeds kept getting cut short by gaming pauses and are finishing now. Verdict finalized once all six land.
- **Lesson banked for the queue tooling:** a parameter sweep has to confirm its dial is actually live in the code path being run, and a scaling run's memory setting has to be big enough to reach its largest step — both now checked on the first job of any sweep.

## 2026-09-09 (continued) — an AWS mistake caught, two more host-shortcut retirements de-risked, vision at its best mechanism, and the local RAM wall being dissolved
- **Caught + reversed an AWS spend mistake (mine).** The cloud "grid next step" named in the entry below was re-running a test we had ALREADY concluded — the deployable-size 2-billion-token voice run, which came back negative on Sep-8. I terminated the duplicate instance (~$5 lost), corrected the record, and banked a check so a concluded result is never paid for twice. The voice's real forward path is NOT more of the same reading at the deployable size (that regressed); it needs a broader reading mix or more capacity — so the outside-model stand-in holds for now.
- **Three more hand-written shortcuts retired onto spiking cells (the owner's #1 priority), all proven 6/6:** (1) "how novel is this word" — a Python set that never forgot — became a spiking habituation circuit whose novelty genuinely fades AND recovers over time; (2) the pronoun / "it, that, they" detector — a bare word-list — became a spiking pattern-completion memory that recognizes those words even from a corrupted cue and goes silent when lesioned; (3) choosing how to interpret a question — a fixed host priority list — became four neural assemblies competing (winner-take-all), a sensible default winning unless a specific pattern fires, an ambiguous question decided by drive strength.
  All three de-risked (board #213/#214/#215), not yet switched on (the switch-on step is RAM-blocked locally — see below).
- **The brain's vision "binding" found its best mechanism yet.** Letting the binding units COMPETE (a winner-take-all selection) beats every prior approach — learning now matters on all six seeds and it clears the task on four of six (the best so far). Both tuning it further and a hard attention-gate made it worse; the next lever (a soft, graded attention with divisive normalization, per the attention literature) is named.
- **The mini-PC compute lane was silently broken (running stale code) — now fixed + proven.** Jobs sent to it had been failing on outdated code; re-syncing the nodes restored it and live-confirmed the fix that lets brain-chat-importing jobs run there.
- **The local RAM wall is being dissolved, likely for free.** The integrated brain-chat checks that ran the machine out of memory were holding the whole brain in system RAM; running it on the GPU's near-empty memory instead should cut the footprint enough to unblock them locally (test in progress). If not, a cheap high-RAM cloud CPU box (now authorized) is the fallback. This is the one thing gating turning the shortcut-retirement de-risks into actual switched-on retirements.
- **Deep credit assignment (a long-standing mapped wall):** the learned-interneuron microcircuit — the genuinely-untested fix on the feedback-signal side, not the already-ruled-out dendrite/read side — is being built and de-risked on the GPU.
