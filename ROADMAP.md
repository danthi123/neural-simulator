...
- **Two of our own claims were withdrawn the same evening — worth knowing about.** Twice we reported that the reader had *learned* which place each cell should respond to, and twice that was wrong: the
  ...
  claiming the opposite). Both were caught by the same one-line check — measure the untrained network too. So: reading direction is solved; **learning what to read it from is not, and is the current job.**
- **Next:** teach the reader which place each cell should respond to, using competition between cells rather than plain co-activation (three candidate mechanisms are being tried); wire the composed-memory→cortex consolidation pathway (so a whole structured fact, not just a route, moves from hippocampus to a lasting cortical memory — the single highest-value memory build. As of 2026-07-29 the shared-population design is largely settled and partly tested on real neurons: storing unrelated items works perfectly, storing facts that share words is much harder, and scrambling each word differently depending on its role recovers most of the loss.
