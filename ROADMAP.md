...
Verified through the real chat handler: with the coupling ON, teaching the same fact under a high-dopamine turn writes a gain of 2.48 vs 0.5 under a low-dopamine turn (the dopamine levels are the real spiking SNc read, not a host number), and a lesion that pins the gain to 1.0 makes the difference vanish entirely (the whole effect rides the dopamine read). OFF by default → byte-identical to today. NOT flipped: the stronger trace only bites on the magnitude-carrying production composer, so the flip needs a no-regression soak there. Finding 2026-08-21-da-gated-encoding-wired-into-chat-GO; production-integration ledger row `da-gated-encoding`.

**2026-08-21 (autonomous) — LEARN-THROUGH-USE IS NOW CONVERSATION-VISIBLE at the real production encode, and WIRED into the live recall — but the on-by-default flip is HELD because a soak found a real leak.** The "next: wire the recall→strengthen loop into the live loop and prove it makes a later turn measurably better, then flip on" step below is now done up to the flip.
The fix that unlocked visibility: the old yes/no recall read was saturated (so using a memory changed nothing visible — the prior blocker), and a GRADED dendritic read (the plateau depth, which is the brain's own strengthening signal) surfaces a rising recall-STRENGTH number instead. Through the real recall + reply at full production encode strength, using a memory makes it recall visibly stronger on every seed whose memory completes (5/5), the effect vanishes exactly when switched off, and the honesty gate + a simulated mid-crash rollback both hold (6/6).
WHY NOT FLIPPED: the no-regression soak is clean when two memories are stored in separate neurons (4/5 builds byte-identical), but on ~1-in-6 builds where two memories happen to SHARE neurons, strengthening one slightly shifts the OTHER's recalled strength — a visible change to a memory the user never touched. So it ships WIRED but OFF BY DEFAULT (the default reply is byte-identical), and the remaining work before on-by-default is the same keep-memories-separate fix already tracked (board #73), or a leak-robust surfaced read. Finding 2026-08-21-d5-graded-apical-read-conversation-visible-...-flip-blocked-on-emergent-assembly-crosstalk; board #71.
