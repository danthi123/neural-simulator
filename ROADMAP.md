...
## 2026-08-27 (continuation) — two merged brain regions now actually LEARN from each other for the first time; a fake "on by default" was found and fixed; a testing bug that hid a real problem recurred five more times; a chat-honesty gap named a week ago is closed

Six more things landed in the same overnight/continuation session, after the items below:

- **Two merged brain regions influencing each other by LEARNING, not by a hand-wired shortcut, for the first time.** Earlier work proved merging two regions onto shared neurons doesn't change their answers (safe, but the regions still ignored each other). This session built the first case where one region's answer genuinely shapes another's: the working-memory region that tracks "who was mentioned" now feeds the sentence-understanding region's guess at what a pronoun refers to — and the connection between them GREW ON ITS OWN through repeated experience, the same kind of learning rule real synapses use, rather than being typed in by hand.
  A follow-up pass made that learning rule more biologically faithful (it now only strengthens a connection when the guess turns out to be REWARDED, not just whenever the two regions happen to fire together) and let the brain try several candidate connections and let the useful one win, instead of being told in advance which one to grow.
- **A capability the project's own tracking sheet said was "on by default" turned out to be silently doing nothing.** A three-way cross-check mechanism (three different brain regions have to agree before a conscious workspace commits to an answer) was switched on in the official tracker back on 2026-08-21, but a second, independent on/off switch buried one layer deeper had been left in its OFF position the whole time — so the mechanism never actually ran in the live system despite the paperwork saying it had. Found by accident while auditing a different bug (see below); fixed; and a sweep of every other capability the tracker calls "on by default" found no other case of this.
- **A testing shortcut that made a bug invisible (found last time on the mood-coloring check) turned out to be a repeated pattern, not a one-off.** Five other automated checks had the same flaw — their way of testing "is this OFF" secretly matched what "ON" now looks like, so they were comparing on-vs-on and always looked fine. All five are now fixed the same way.
- **Why the brain sometimes says "I don't know" to a fact it actually has stored, diagnosed.** Asking about a small, obscure fact (a football club's country) got an honest-sounding "I don't know" even though the fact was in the knowledge base. Turned out to be two separate, unrelated bugs stacked on top of each other, both of which have to clear for a plain question to get answered: one was a straightforward word-order mistake in understanding the question (now fixed); the other is a deeper design gap where one of the brain's cross-checking mechanisms was built expecting to see only recently-discussed facts, so it automatically distrusts anything pulled from the large background knowledge store, right or wrong.
  A fix for the second one is built and tested but not yet turned on by default — that is a call for the project owner.
- **A dishonesty gap flagged a week ago, when the free-form chat mode was first wired up, is now closed.** That mode (still off by default) lets the brain answer in its own free-flowing words instead of a rigid template, with a safety check afterward that strips anything it can't back up. The safety check's "does this contradict what we actually know" logic was a placeholder that never actually checked anything — so on a topic the brain DOES know about, it could add a wrong specific detail (the wrong bordering country, a made-up date) right alongside the correct facts, and nothing would catch it.
  A previously-tested fix for exactly this is now wired in: it correctly strips 10 out of 10 known wrong details in testing, while leaving unrelated correct content alone, and turning the fix back off (to check the fix is really what's responsible) makes all 10 wrong details come back — confirming the fix is actually doing the work. One honest limitation carries over unchanged from the earlier testing: if a correct detail and a wrong detail happen to be stated in the very same sentence, both get removed together (the checker works sentence-by-sentence, not clause-by-clause) — this was already true before today's change, just now confirmed still true.

Findings: `2026-08-27-onebrain-integration-R1-wm-to-comprehension`, `2026-08-27-onebrain-integration-R2-threefactor-selforganized`, `2026-08-27-gnw-3organ-hollow-flip-fixed-plus-ledger-audit`, `2026-08-27-flip-soak-off-arm-staleness-audit`, `2026-08-27-knowledge-in-live-chat-veto-comprehension-and-gnw-organb-expectation-gap`, `2026-08-27-organb-ltm-exempt-derisk-6seed-GO`, `2026-08-27-known-supplement-contradiction-filter-wired-into-open-ended-chat-postfilter-GO`.
