...
- **Two more hand-written shortcuts retired onto spiking cells (the owner's #1 priority), both proven 6/6:** (1) "how novel is this word" — a Python set that never forgot — became a spiking habituation circuit whose novelty genuinely fades AND recovers over time; (2) the pronoun / "it, that, they" detector — a bare word-list — became a spiking pattern-completion memory that recognizes those words even from a corrupted cue and goes silent when lesioned. Both de-risked, not yet switched on (the switch-on step is RAM-blocked locally — see below).
