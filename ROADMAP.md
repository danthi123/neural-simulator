...
- **Next:** give the replay a *neural* reader (the population decode is a measurement instrument for now); wire the composed-memory→cortex consolidation pathway (so a whole structured fact, not just a route, moves from hippocampus to a lasting cortical memory — the single highest-value memory build — as of 2026-07-29 its next step is to build the shared-population (sparse distributed) store on the real substrate, replacing the dedicated-slot design that hit a capacity ceiling); build a theta pacemaker.
