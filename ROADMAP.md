...
- **A biological deep-learning rule — now demonstrated on the spiking brain (2026-08-01).** Some ceilings (how much structure the brain can compose, how deeply it can nest ideas, a truly self-taught movement policy) traced back to one missing capability: a learning rule that lets a *deep* spiking network teach itself, without backpropagation or biologically-impossible weight-copying.
  Several candidate rules were tried and honestly retired; this cycle a transport-free biological rule — **e-prop** (forward eligibility traces + a fixed feedback signal) — was shown to train deep credit **on the production spiking brain**, and with **population coding** it climbs *past* the idealized reference ceiling (the more neurons vote on each unit, the cleaner the credit — a clean √K trend, K=16 above the ceiling). The substrate now genuinely *learns* by a biological rule; this was the project's central "must-solve" dependency, and it is unlocked.
  The remaining refinement is making the teaching feedback itself *learned* — the cortex's own self-predicting microcircuit, replacing the fixed-random feedback — which is actively worked now, in parallel with wiring this learning substrate into a teacher-driven developmental loop.
