...
## [Unreleased] — 2026-09-25 — The #1 metric became lesion-verified load-bearing fraction; Qwen reframed as a permanent mouth; a new sleep-replay CAPTURE pass and an AI-teacher memory path reach 6-seed GO (neither shipped by default; basic offline sleep-replay consolidation has been default-on since 2026-08-26)

### The integration metric changed, and the mouth's role was redefined

- **Owner-ratified pivot (2026-09-19): the #1 tracked metric is now the lesion-verified integrated LOAD-BEARING FRACTION of the live conversation**, replacing "% scaffold retired." The question is no longer how much host code has been deleted but whether the spiking brain, not the LLM mouth, provably drives the produced reply. A 2026-09-25 re-scored 6-seed battery (superseding the earlier 2026-09-20/21 numbers), reported per `research/findings/2026-09-25-production-default-battery-B2a-rescored-PASS.md`: on the shipped default (thin) probes, mean 0.603 (robust core 15/26, union 16); with all verified-adequate measurement-only probe fixes enabled (not shipped), mean 0.949 (robust core 24/26, union 25), SD 0.018 for both. Per the owner's reporting standard, these two numbers are always reported as a pair, never a single cherry-picked figure.
- **Qwen2.5-0.5B is now framed as a permanent articulation mouth, not a scaffold to be retired (owner note, 2026-09-19).** The earlier "retire the transformer mouth via spiking fluency" arc is closed as falsified; the roadmap now targets proving the spiking brain is load-bearing *under* that permanent mouth, rather than replacing the mouth itself. (A narrow, bounded-scope spiking mouth that renders single-role transitive-SVO recall sentences directly remains default-on; it is a different, older mechanism from the broad-fluency arc that was closed.)
- **Scaffold-retirement count reached 5 of 69 tracked faculties (2026-09-16).** Four faculties — selective-attention biased competition, spiking novelty-habituation, spiking anaphor-detection, and spiking Q-route selection — had their host fallback code fully deleted and verified byte-identical against `main`, joining the earlier one-brain-substrate retirement.

### Memory: an AI-teacher path and a new sleep-replay CAPTURE pass both reach 6-seed GO, neither shipped

- **A chat-only AI-teacher pathway passed a clean 6-seed GO with a sham-lesion control (2026-09-24, late evening).** Facts taught only in conversation are recalled after a sleep gap via the brain's own synaptic change, are lost when that write is frozen/erased, and the brain preferentially keeps a counterfactual or lie it was actually told over an unstated "true" fact. This is not yet switched on by default — it needs a full production no-regression battery and a real-production-config check first.
- **A new sleep-replay CAPTURE pass (`BRAIN_SLEEP_REPLAY_CAPTURE`, default OFF — a night-load-dependent synaptic-tagging mechanism distinct from the basic offline sleep-replay consolidation that has been default-on since 2026-08-26) passed 6-seed GO on its core `rc` gates, independently re-checked (2026-09-25).** A harder awake-rest extension of the same route (a fact told hours before sleep, not immediately before it) passes on 5 of 6 seeds, and a separate later-learning-interference variant is NO-GO on 3 of 6 seeds — on the 2 failing seeds the fact was told and stored with a normal synaptic weight (checked directly; it was not "never stored"), but its night-1 reactivation missed the capture margin. A load-based (rather than fixed-percent) nightly forgetting rule for unimportant facts is designed and built but not yet default-on or 6-seed-validated end to end.
- **The measurement suite grew from 38 to 49 tracked cognitive abilities (2026-09-24)**, with nine more faculties merged off-by-default pending their own 6-seed tests, including theory-of-mind false-belief tracking, multi-step reasoning, and a spiking fact-binder in chat.
- **The glued-on feeling word in replies (e.g. "Wonderful!") was retired from default output (2026-09-25)**, per owner directive that affect should shape *how* the brain speaks rather than insert marker words; mood is still computed and recorded internally.

### Deep credit and open constraints (honest, not shipped as resolved)

- **The gap#4 deep-credit-assignment sliding-baseline fix, which stops synaptic weights piling up at their bound, has merged (2026-09-23/24).** Whether it carries over to full network size is still open: a full-size-network run landed exactly at chance on every repeat as of 2026-09-24.
- **Memory capacity remains an open constraint.** The fact store's recall holds to roughly 2,000 facts by construction (a disjoint one-block-per-fact design, not itself a capacity law), but per-chat-turn READ-BACK TIME crosses from usable to unusable somewhere between 50 and 500 stored facts — measured entirely on shared pool-CPU nodes (peak memory stayed under 233 MB throughout the whole grid, nowhere near a VRAM limit; it is a time ceiling, not a memory one). The project's single-consumer-RTX-3090 reference case is explicitly UNMEASURED here, so whether a GPU moves that ceiling is still open (2026-09-25 capacity-curve work, `research/findings/2026-09-25-d6-capacity-curve-N2000-6seed-CEILING-BETWEEN-50-AND-500.md`).

### Honest state of the whole (do not read the wins as more than they are)

The honesty boundary continues to apply project-wide: self-reports remain framed as functional read-outs of measurable correlates ("my familiarity monitor reads this as novel, so I'm uncertain"), never as phenomenal or subjective experience. The ledger distinguishes isolated-organ lesion-load-bearing (a component correctly reads/reacts in isolation) from integrated load-bearing (that reaction actually changes the produced reply on the live default chat path) — episodic memory is the ledger's own worked example, isolated-lesion-load-bearing since 2026-08-12 but found integrated-hollow until a probe fix on 2026-09-20.
