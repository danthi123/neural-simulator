...
## [Unreleased] — 2026-04-28 — Cheat #5 CLOSED + throughput investigation + webapp polish
...
- **Cheat #5 v4 (`--developmental-pretraining`) — NO-GO; cheat #5 closed by design.** Pre-training cross-projections during a 5K-trial critical period (all gates open) and freezing for eval is *worse* than v3.1: 3-seed Tier 2 sum 11.34 ± 1.85, P0 4.88 (even initial goal acquisition degrades), P1 6.46. Tier 3 (overnight 6-seed) skipped — Tier 2 unanimous past the > 6.0 NO-GO threshold. Pretraining DID develop cross weights consistently (mean ~11, std ~0.5 across seeds) — the topology, not the training, is the limit. **Cheat #5 closed: v3 MSN lateral inhibition + same-action-only routing IS the functional WTA equivalent in our reduced model.** Cross-projections corrupt the cascade at any non-zero weight regardless of training regime. Real BG anatomically dense + functionally same-action-dominant = our reduced model's same-action-only + lateral inhibition. Finding: `research/findings/2026-04-28-cheat5-v4-results.md`.
