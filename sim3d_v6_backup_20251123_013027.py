...
class CoreSimConfig:
    ...
    total_simulation_time_ms: float = 1000.0
    dt_ms: float = 1.000
...
class SimulationBridge:
    ...
    def get_latest_simulation_data_for_gui(self, force_fetch=False):
        ...
        gui_data_dict = {
            ...
            "neuron_types_list_for_viz": self.sim_config.neuron_types_list_for_viz.copy(), # Stays as Python list
            "neuron_model_type_str": self.sim_config.neuron_model_type,
        ...
        # Synapse info for GUI is CPU-based and sampled - keep as is for now
    ...
    def get_initial_sim_data_snapshot(self):
        ...
        if not self.is_initialized:
            ...
            n_cfg = self.sim_config.num_neurons if self.sim_config else 0
            model_type_str_cfg = self.sim_config.neuron_model_type if self.sim_config else NeuronModel.IZHIKEVICH.name
            types_list_cfg = self.sim_config.neuron_types_list_for_viz.copy() if self.sim_config and self.sim_config.neuron_types_list_for_viz else []
...
def update_gl_data():
    ...
    # CPU data (neuron types list is Python list of strings)
    ...
    if current_filters.get("type_filter_enabled", False):
        ...
        if selected_type_str_cpu != "All":
            # This part still involves CPU list comprehension and transfer for the mask
            # For full GPU, neuron_types_list_for_viz would need to be a CuPy array of type IDs.
            type_mask_np = np.array([all_neuron_types_str_list_cpu[i] == selected_type_str_cpu for i in range(num_neurons_in_snapshot)], dtype=bool)
            visible_mask_cp &= cp.asarray(type_mask_np)
    ...
    # --- Synapse Data (Still largely CPU based for filtering, but uses CuPy arrays for final positions) ---
    ...
    if global_gui_state.get("show_connections_gl", False) and "synapse_info" in sim_data_snapshot:
        ...
        visible_synapse_indices_in_sample_cpu = apply_synapse_filters_to_indices(all_synapse_data_list_cpu, current_filters)

        if visible_synapse_indices_in_sample_cpu: # If there are synapses to draw
            src_indices_list, tgt_indices_list = [], []
            chosen_global_indices_set_cpu = set(cp.asnumpy(chosen_neuron_indices_cp)) # For quick CPU-side lookup

            for local_syn_idx in visible_synapse_indices_in_sample_cpu:
                if local_syn_idx < len(all_synapse_data_list_cpu):
                    syn_data = all_synapse_data_list_cpu[local_syn_idx]
                    src_global_idx, tgt_global_idx = syn_data["source_idx"], syn_data["target_idx"]
                    if src_global_idx in chosen_global_indices_set_cpu and tgt_global_idx in chosen_global_indices_set_cpu:
                        src_indices_list.append(src_global_idx)
                        tgt_indices_list.append(tgt_global_idx)

            if src_indices_list: # If any valid synapses found after filtering
                src_indices_cp = cp.asarray(src_indices_list, dtype=cp.int32)
                tgt_indices_cp = cp.asarray(tgt_indices_list, dtype=cp.int32)

                pos_src_all_cp = all_neuron_positions_3d_cp[src_indices_cp]
                pos_tgt_all_cp = all_neuron_positions_3d_cp[tgt_indices_cp]

                # Interleave source and target positions: [src1, tgt1, src2, tgt2, ...]
                temp_gl_connection_vertices_cp = cp.empty((len(src_indices_list) * 2, 3), dtype=cp.float32)
                ...
                current_num_synapse_lines_to_draw = len(src_indices_list)
    ...
    if gl_neuron_pos_vbo is not None and gl_neuron_pos_vbo > 0 and gl_neuron_pos_cp.size > 0:
        ...
        # Convert to NumPy array before passing to glBufferData
        glBufferData(GL_ARRAY_BUFFER, gl_neuron_pos_cp.nbytes, cp.asnumpy(gl_neuron_pos_cp), GL_DYNAMIC_DRAW) 
    if gl_neuron_color_vbo is not None and gl_neuron_color_vbo > 0 and gl_neuron_colors_cp.size > 0:
        ...
        # Convert to NumPy array before passing to glBufferData
        glBufferData(GL_ARRAY_BUFFER, gl_neuron_colors_cp.nbytes, cp.asnumpy(gl_neuron_colors_cp), GL_DYNAMIC_DRAW)
    ...
    if gl_synapse_vertices_vbo is not None and gl_synapse_vertices_vbo > 0 and gl_connection_vertices_cp.size > 0:
        ...
        # Convert to NumPy array before passing to glBufferData
        glBufferData(GL_ARRAY_BUFFER, gl_connection_vertices_cp.nbytes, cp.asnumpy(gl_connection_vertices_cp), GL_DYNAMIC_DRAW)
    ...
    if gl_pulse_vertices_vbo is not None and gl_pulse_vertices_vbo > 0 and gl_pulse_vertices_cp.size > 0:
        ...
        # Convert to NumPy array before passing to glBufferData
        glBufferData(GL_ARRAY_BUFFER, gl_pulse_vertices_cp.nbytes, cp.asnumpy(gl_pulse_vertices_cp), GL_DYNAMIC_DRAW)
...
def simulation_worker_loop(sim_bridge, local_shutdown_event, command_q, data_q):
    ...
    DATA_UPDATE_INTERVAL_STEPS = 60 # e.g., send data every 2ms of sim time if dt=0.1ms
