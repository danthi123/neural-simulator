        ...
        try:
            ...
            if effective_connections_matrix.nnz > 0 and _prev_any:
                ...
                if cfg.enable_inhibitory_neurons and self.cp_traits is not None:
                    ...
                    # See the E/I-split branch above: deterministic transpose SpMV when
            ...
            if (getattr(cfg, "enable_coincidence_detection", False)
                ...
                if self.cp_connections.nnz > 0 and _prev_any:
                    ...
                    # conductance matvec above): materialize the transpose as CSR so the per-step op is a
                    # reproducible one-thread-per-output-row SpMV instead of the bit-non-reproducible csc
                    # atomic scatter. Default off ⇒ `_co_matT` is just `_co_mat.T` and the matvec expression
