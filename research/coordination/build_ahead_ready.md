...
**1. affect→neural default-on validation** — ✅ **DONE / RETIRED 2026-09-08** (was stale-"READY"): the full fan-out ran + landed `7e5836a56` (harness `a9c29d10e`, correction `5efa0d62b`); verdict **NOT-READY to promote** (`BRAIN_WKV_MOUTH_AFFECT_NEURAL` stays default-OFF — negative direction only reaches 6/6 at neg_pa_scale≥2.5 where output loops; two objectives in tension on one knob). Finding `2026-09-04-affect-neural-default-on-negpa-sweep-NOT-READY.md`. NEXT MECHANISM (originally proposed): convert the congruence source from a host fixed-lexicon lookup into a LEARNED word-identity↔valence association driving the neuromodulator. ⚠️ **2026-09-09 verify-first — DO NOT re-attempt as a text-only build:** this hits the SAME text-boundary `2026-09-05-affect-learned-gate-retry-register-confound-BOUNDARY.md` already characterized (4 mechanistically-different learned attempts, ALL boundary — every candidate reads the SAME co-occurrence graph the confound lives in, so no text statistic separates genuine-affect from incidentally-warm words). The owner's standing multimodal-grounding guidance scopes this boundary as EXPECTED (surpass via interoception/prosody/context grounding, NOT text-cleverness; prosody/facial deferred as principled scope). So it is a DEFERRED multimodal boundary, not an open text lever. — original stale text below —
