...
current_status: "FORWARD-LEARNING REAL, DEEP-CREDIT ATTRIBUTION NOT CLOSED AT 6 SEEDS (2026-08-01, corrects this morning's closure claim). The 6-seed bar — named by the prior current_status as the SOLE remaining residual — has now been RUN (noise-OFF, depth-2, K=8 and K=16, seeds 42/43/44/100/101/102), and the runner returns SIGNAL=False on 11/12 runs because the shuffle-DFA DEEP-CREDIT control LEAKS on 4/6 seeds at each K (mean shuffle-DFA 0.438 at K=8, 0.494 at K=16, vs the GO bar chance+0.10=0.433). What IS real and holds at 6 seeds: e-prop trains the forward task on the production Izhikevich bridge, the teacher signal is load-bearing (permuted control clean, mean 0.247 -> chance), and `inherit` climbs monotonically with the population factor K (0.685 at K=8 -> 0.852 at K=16). What is NOT established: that the climb is DEEP CREDIT (correct routing) rather than reservoir expressivity that grows with K — scrambling the DFA credit route only partially degrades it (0.33-0.59, not chance 0.333), so the √K inherit gain and the √K reservoir-capacity gain are confounded on this task. RETRACTED: 'the K=8 residual is CLOSED and the population lever surpasses the reference ceiling' (it read the `inherit` field past the run's own HONEST-NEGATIVE verdict — silent-failure rule #1). The √K inherit curve (K=1 0.37 -> K=16 0.85) is real as a CAPACITY curve, not as proof of clean credit. REAL RESIDUAL (a method, not more seeds): a control the reservoir CANNOT defeat — a frozen-hidden reservoir baseline at an operating point where credit routing is load-bearing; deep credit is earned only when e-prop beats frozen-hidden AND shuffle-DFA collapses to chance. Arc-A's shallow atom reached the same conclusion independently (its shuffle-DFA was reported-not-gated for exactly this reason)."
sources:
