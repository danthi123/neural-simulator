...
current_finding: research/findings/2026-09-23-vision-attention-gated-soft-readout-spiking-port-collapse-NOGO-banked.md
current_status: "HARD k-WTA (--readout attention-gated) BANKED as a REGRESSION (PARTIAL-beat2/6-lb3/6 vs the competitive-selection baseline's beat4/6-lb6/6; research/findings/2026-09-09-vision-configural-binding-attention-gated-readout-NEXT-MECHANISM-PREREGISTERED.md). Its named next rung, the GRADED/soft attention-gain mode (--readout attention-gated-soft, built 2026-09-16, commit b7108ca04), is now BANKED NO-GO on 12 of 12 seed-runs across two front-end operating points (research/findings/2026-09-23-vision-attention-gated-soft-readout-spiking-port-collapse-NOGO-banked.md; the prior PREREGISTERED framing was ⛔ RETRACTED -- see docs/RETRACTED.md): LEARNED_spkwta_held at exact chance (0.25) on 11 of 12 seed-runs (one exception, default front end seed 101 at 0.2604) across both the default z-norm front end (2026-09-17) and the satdiv-GO front end (2026-09-23), while LEARNED_linscore_held (a separate, never-gated pathway) retains real signal on 11 of 12 (one exception, default front end seed 100 at 0.3229, below this runner's own nogo_floor of 0.34) -- the plain --readout linear spiking port is NOT broken at the same satdiv-GO operating point (mean 0.5729, real signal). The banking finding's own diagnosis -- that the collapse is specific to attention-gated-soft's per-class, per-trial host satdiv normalization compressing between-class contrast before the LIF sign-split read -- is a HYPOTHESIS, not yet tested by an ablation; a COMPETING explanation (LEARNED_spkwta_TRAIN pinned at exactly 0.25 on all 6 satdiv-GO seeds, a constant-output/degenerate-port signature the runner's own docstring separately names) has not been ruled out. The NEXT mechanism (research/findings/2026-09-23-vision-configural-binding-spiking-feedback-divisive-gain-control-readout-PREREGISTERED.md, --readout attention-gated-soft-fbgain) replaces that specific host-computed normalization stage with a SPIKING, pooled-across-classes, feedback divisive gain-control loop realized inside the LIF read itself (Wilson & Cowan 1972; Heeger 1992); its gain-only arm (--fb-strength 0.0, satdiv stage removed, feedback loop off) is the REGISTERED TEST of the per-class-satdiv hypothesis above, not another retune of the same host formula. A fresh automated selftest (tests/test_vision_attention_gated_soft_selftest.py) pins byte-identical-off + non-degenerate-on + dispatcher-routing for the (now banked-NO-GO) soft mode; tests/test_vision_fbgain_readout_selftest.py does the same for the new fbgain mode."
sources:
