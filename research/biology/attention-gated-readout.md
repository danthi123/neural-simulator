...
current_finding: research/findings/2026-09-24-vision-configural-binding-spiking-feedback-divisive-gain-control-readout-NEUTRAL-6seed.md
current_status: "HARD k-WTA (--readout attention-gated) BANKED as a REGRESSION (PARTIAL-beat2/6-lb3/6 vs the competitive-selection baseline's beat4/6-lb6/6; research/findings/2026-09-09-vision-configural-binding-attention-gated-readout-NEXT-MECHANISM-PREREGISTERED.md). Its named next rung, the GRADED/soft attention-gain mode (--readout attention-gated-soft, built 2026-09-16, commit b7108ca04), is BANKED NO-GO on 12 of 12 seed-runs across two front-end operating points (research/findings/2026-09-23-vision-attention-gated-soft-readout-spiking-port-collapse-NOGO-banked.md; the prior PREREGISTERED framing was ⛔ RETRACTED -- see docs/RETRACTED.md): LEARNED_spkwta_held at exact chance (0.25) on 11 of 12 seed-runs (one exception, default front end seed 101 at 0.2604) across both the default z-norm front end (2026-09-17) and the satdiv-GO front end (2026-09-23), while LEARNED_linscore_held (a separate, never-gated pathway) retains real signal on 11 of 12 (one exception, default front end seed 100 at 0.3229, below this runner's own nogo_floor of 0.34) -- the plain --readout linear spiking port is NOT broken at the same satdiv-GO operating point (mean 0.5729, real signal). The NEXT mechanism (--readout attention-gated-soft-fbgain, research/findings/2026-09-23-vision-configural-binding-spiking-feedback-divisive-gain-control-readout-PREREGISTERED.md) replaced the host-computed per-class satdiv normalization stage with a SPIKING, pooled-across-classes, feedback divisive gain-control loop realized inside the LIF read itself (Wilson & Cowan 1972; Heeger 1992); its decisive 6-seed run is now SCORED (research/findings/2026-09-24-vision-configural-binding-spiking-feedback-divisive-gain-control-readout-NEUTRAL-6seed.md): NEUTRAL, an EXACT zero-variance null (mean(d)=0, SD(d)=0 across all 6 seeds -- the full-mechanism and gain-only arms are bit-identical on LEARNED_spkwta_held, all at exact chance, on every seed). This RULES OUT the per-class-satdiv hypothesis as the whole story (the gain-only arm removes that stage entirely and still collapses identically) and does not support the feedback-loop-as-fix hypothesis either (adding it back changes nothing). The constant-output/degenerate-port signature (LEARNED_spkwta_held == LEARNED_spkwta_train == RANDOM_spkwta_held == scramble_learned_held == 0.25 on every one of the 12 runs) is confirmed, not merely un-ruled-out. Sharpened diagnosis (code-reading-derived, NOT yet tested by an ablation): the collapse point likely precedes BOTH the satdiv stage and the feedback-loop stage, at the top-down per-unit gain multiply itself (gated = r * A_c**exponent, applied before the already-fitted discriminant w=V/sd, which was calibrated on the un-gained r) -- rescaling by a per-class template the discriminant was never trained against is a plausible route to `net` being dominated by the trial-independent class bias rather than the trial's own signal. Candidate next lever (NOT yet pre-registered or built): a per-class SCALAR response-gain (rescaling the already-computed linear score, holding the discriminant's own input calibration intact) rather than the per-unit contrast-gain (input-reweighting) form every arm in this lane has tried so far -- the distinction Reynolds & Heeger (2009) themselves draw between response-gain and contrast-gain attentional modulation. A fresh automated selftest (tests/test_vision_attention_gated_soft_selftest.py) pins byte-identical-off + non-degenerate-on + dispatcher-routing for the soft mode; tests/test_vision_fbgain_readout_selftest.py does the same for the fbgain mode."
sources:
...
findings:
  ...
  - research/findings/2026-09-23-vision-configural-binding-attention-gated-soft-readout-PREREGISTERED.md ⛔ RETRACTED, see docs/RETRACTED.md
