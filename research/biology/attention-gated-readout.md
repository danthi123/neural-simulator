...
current_status: "HARD k-WTA (--readout attention-gated) BANKED as a REGRESSION (PARTIAL-beat2/6-lb3/6 vs the competitive-selection baseline's beat4/6-lb6/6; research/findings/2026-09-09-vision-configural-binding-attention-gated-readout-NEXT-MECHANISM-PREREGISTERED.md). Its named next rung, the GRADED/soft attention-gain mode (--readout attention-gated-soft, built 2026-09-16, commit b7108ca04), is now BANKED NO-GO on 12 of 12 seed-runs across two front-end operating points (research/findings/2026-09-23-vision-attention-gated-soft-readout-spiking-port-collapse-NOGO-banked.md; the prior PREREGISTERED framing was ⛔ RETRACTED -- see docs/RETRACTED.md): LEARNED_spkwta_held pinned to exact chance (0.25) on every seed of both the default z-norm front end (2026-09-17) and the satdiv-GO front end (2026-09-23), while LEARNED_linscore_held (a separate, never-gated pathway) retains real signal on every seed -- the plain --readout linear spiking port is NOT broken at the same satdiv-GO operating point (mean 0.5729, real signal), so the collapse is specific to attention-gated-soft's own per-class, per-trial host satdiv normalization compressing between-class contrast before the LIF sign-split read. The NEXT mechanism (research/findings/2026-09-23-vision-configural-binding-spiking-feedback-divisive-gain-control-readout-PREREGISTERED.md, --readout attention-gated-soft-fbgain) replaces that specific host-computed normalization stage with a SPIKING, pooled-across-classes, feedback divisive gain-control loop realized inside the LIF read itself (Wilson & Cowan 1972; Heeger 1992), not another retune of the same host formula. A fresh automated selftest (tests/test_vision_attention_gated_soft_selftest.py) pins byte-identical-off + non-degenerate-on + dispatcher-routing for the (now banked-NO-GO) soft mode; tests/test_vision_fbgain_readout_selftest.py does the same for the new fbgain mode."
sources:
