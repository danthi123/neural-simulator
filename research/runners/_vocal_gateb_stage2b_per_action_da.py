"""Gate B Stage 2b: per-action compartmentalised dopamine on the BG selector.

Surpasses the Stage-2 global-DA wall
(`research/findings/2026-08-06-gateB-stage2-local-reward-credit-NO-GO.md`,
`STAGE2_NO_GO`): a single GLOBAL dopamine scalar times eligibility converted
BOTH channels' traces, so reward decoupled from the action (yoked) learned
IDENTICALLY to reward contingent on it (D_contingent == D_yoked on all 6 seeds).
The substrate already ships the fix (Cluster C v2, `sim/neuromodulators.py`,
`2026-04-24-session-c.md` sec 4).

Stage 2b swaps the credit signal for **per-action compartmentalised DA**: four
per-action DA modulators (`dopamine_{N,E,S,W}`; we use N->action0, E->action1)
whose `from_action_specific_reward` production fires ONLY for the channel the
body just executed (`core_config.last_selected_action`, the neural motor
read-out), and whose per-synapse DA signal
(`compute_per_synapse_da_signal`) routes each channel's DA to ONLY its
action-tagged synapses (`region.action_index` on `str_d1_c` ->
`cp_synapse_action_tag`). The three-factor rule then converts
`per_synapse_da[route] * eligibility[route]`, so reward for action A potentiates
ONLY A's eligible D1 route, not B's -- the action-local credit the global scalar
could not do.

    Delta_w = reward_learning_rate * per_synapse_DA(action) * eligibility_trace

Eligibility is NEURAL (coactivity on real pre/post spikes on `proposal_c ->
str_d1_c`, scoped). Reward is DELIVERED as an environmental scalar from the
body's motor read-out; `last_selected_action` is the SAME body read-out (which
motor pool fired -- identical to how the nav body moves the agent,
`g11_bg_runner.py:7694`; CLAUDE.md body boundary). The host never assigns credit
to synapses, never edits eligibility/weights, never argmaxes spikes to LABEL the
credited route -- the neural action tag + neural eligibility + neural DA
production/decay do the credit assignment; the host only reports the executed
action + reward, exactly as Stage-2 reported reward.

A neural EXPLORATION process (elevated OU membrane-potential noise on the
proposal populations -- background synaptic-bombardment variability) de-locks
pre-learning action choice so the disfavoured action is sampled enough to be
credited (4/6 Stage-2 dev seeds were seed-locked -> no variance for reward to
shape). The reward-OFF build at the Stage-1 noise level is asserted
byte-identical (weights + raster hash) to `run_stage1`, so "built on the Stage-1
selector" stays a mechanical fact; the scored circuit differs from Stage-1 in
exactly two DECLARED substrate knobs -- D1-route plasticity + the per-action DA
subsystem (the mechanism under test) and elevated proposal OU noise (the
exploration process) -- no host-designed wiring/weight change.

Acceptance criteria are FROZEN from the Stage-2 preregistration
(`research/findings/2026-08-06-gateB-stage2-local-reward-credit-PREREGISTRATION.md`):
bias-free swap differential D vs a reward-count-matched yoked control, the
acquisition/expression lesions, same-brain convention reversal, across the same
6 dev seeds.
"""
...
OUT_DIR = ROOT / "research/findings/raw/gateb_stage2b_per_action_da"
...
def main(argv=None):
    ...
    if args.mode == "explore":
        ...
        artifact = {"probe": "gateB_stage2b_explore_calibration",
    ...
    if args.mode == "calibrate":
        ...
        yok = run_condition(args.seed, condition="yoked", target=args.target,
                            ...
                            reward_learning_rate=args.reward_lr, ou_sigma=args.ou_sigma)
        artifact = {"probe": "gateB_stage2b_calibration", "backend": backend["backend"],
        ...
        outcome = ("STAGE2B_GO" if verdict["go"] else "STAGE2B_NO_GO")
        if verdict["verdict_status"] == "UNDEFINED":
            outcome = "STAGE2B_UNDEFINED"
        artifact = {"probe": "gateB_stage2b_per_action_da", "stage": "stage2b_learning",
                    "backend": backend["backend"], "device": backend["device"],
