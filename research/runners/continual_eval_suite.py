...
def benchmark_interference(args, rng):
    """TODO: train interleaved 8 words vs sequential A->B.
    Compare final accuracy on full vocab. Score = interleaved /
    sequential (>1 means interleaving helps; we want >= 1.2)."""
    return {
        "name": "interference",
        "score": 0.0,
        "pass": False,
...
def benchmark_long_tail(args, rng):
    """TODO: train 8-word vocab with imbalanced frequency.
    Pass if rare words still get >= 30% accuracy (vs 25% chance)."""
    return {
        "name": "long_tail",
        "score": 0.0,
        "pass": False,
