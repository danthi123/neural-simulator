...
def main():
    ...
    ap.add_argument("--homeostatic", action="store_true",
                    help="LEVER-2: derive the ON arm from the PRODUCTION homeostatic companion (Turrigiano "
                         ...
                         "instead of the RAW clamped map. This is the shipped default-ON write path.")
    ...
    out = {
        ...
        "config": {"seeds": seeds, "D": dim, "k_da": K_DA, "da_baseline": DA_BASELINE, "battery_m": m,
                   ...
                   "da_values": _DA, "gains": gains, "gain_mode": mode, "unstored_moat_cue": list(UNSTORED)},
    ...
    print(f"  MODE: {mode}  (homeostatic = Turrigiano multiplicative scaling + recall-safe floor; raw = the "
          f"pre-lever-2 clamped map)", flush=True)
