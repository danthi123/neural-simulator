"""Hands-off self-sweep + aggregator for source-monitor v9 generalization.

Launch one self-contained process, get one earned verdict. This runs EVERY seed
of a phase against the FROZEN v9 mechanism (imported unchanged from
``_laneC_source_monitor_coresidency_gate_v9``), collapses per-seed PASS/FAIL into
one GO/NO-GO, and writes a single aggregate verdict artifact under
``research/findings/raw/source_monitor_v9_generalization/``.
...
rule, or inhibitory-STDP operating point (that would be gaming the gate).
...
Run (hands-off; provenance is auto-stamped via ``research.runners.__init__``):
    ...
    SIM_BACKEND=numpy python -m research.runners.aggregate_source_monitor_v9_seeds \
        --phase development
"""
...
from research.runners._laneC_source_monitor_coresidency_gate_v9 import (
    GENERALIZATION_DIR,
    PHASE_SEEDS,
    _development_is_go,
...
def _seed_summary(row: dict) -> dict:
    ...
    istdp = row.get("istdp", {}).get("intact_settling", {})
    return {
        "seed": row["seed"],
        "status": row["status"],
        ...
        "recall_rival_burden": metrics.get("recall_rival_burden"),
        ...
        "istdp_weight_delta_l1": istdp.get("weight_delta_l1"),
        "istdp_per_source_l1_gain": istdp.get("per_source_l1_gain"),
        "istdp_before_per_source_l1": istdp.get("before_per_source_l1"),
        "istdp_after_per_source_l1": istdp.get("after_per_source_l1"),
...
def run_phase(phase: str, out_dir: Path = GENERALIZATION_DIR) -> dict:
    ...
    earned = Verdict(f"source-monitor v9 {phase} generalization")
    earned.require(
    ...
    earned.require(
        "the frozen v9 acceptance criteria were applied unchanged on every seed",
    ...
    aggregate = {
        ...
        "acceptance_rule": "GO iff every seed satisfies the frozen v9 criteria",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Self-sweep a source-monitor v9 phase and write one aggregate verdict."
    )
    ...
    print(
        f"[aggregate-source-monitor-v9] phase={aggregate['phase']} "
    ...
    for s in aggregate["per_seed"]:
        print(
            ...
            f"rival_burden={s['recall_rival_burden']} "
            f"istdp_dl1={s['istdp_weight_delta_l1']} "
            f"istdp_gain={s['istdp_per_source_l1_gain']} "
            f"learning_off={s['learning_off_source_spikes']} "
            f"failing={s['failing_components']}",
            flush=True,
        )
    print(f"[aggregate-source-monitor-v9] wrote {aggregate['verdict_path']}", flush=True)
