"""Hands-off self-sweep + aggregator for source-monitor v8 generalization.

Launch one self-contained process, get one earned verdict. This runs EVERY seed
of a phase against the FROZEN v8 mechanism (imported unchanged from
``_laneC_source_monitor_coresidency_gate_v8``), collapses per-seed PASS/FAIL into
one GO/NO-GO, and writes a single aggregate verdict artifact under
``research/findings/raw/source_monitor_v8_generalization/``.
...
and collapses verdicts -- it never changes the mechanism, thresholds, acceptance
rule, or homeostasis operating point (that would be gaming the gate).
``held_out`` stays sealed until ``development`` records a GO here, enforced by
the runner's ``validate_phase_seed``.

Run (hands-off; provenance is auto-stamped via ``research.runners.__init__``):

    SIM_BACKEND=numpy python -m research.runners.aggregate_source_monitor_v8_seeds \
        --phase development
"""
...
from research.runners._laneC_source_monitor_coresidency_gate_v8 import (
    GENERALIZATION_DIR,
    PHASE_SEEDS,
    _development_is_go,
...
def _seed_summary(row: dict) -> dict:
    ...
    # The redistributive-win question: a lift fraction of 0 means competition did
    # not raise the weakest source above the competition-off arm at all (the v6
    # seed-654 failure). Attributing it out loud is the class-AT discipline.
    ...
    scaling = row.get("scaling", {}).get("intact_settling", {})
    return {
        "seed": row["seed"],
        "status": row["status"],
        ...
        "scaling_weight_delta_l1": scaling.get("weight_delta_l1"),
        "weakest_source_before": scaling.get("weakest_source_before"),
        "weakest_source_share_gain": scaling.get("weakest_source_share_gain"),
        "before_share": scaling.get("before_share"),
        "after_share": scaling.get("after_share"),
...
def run_phase(phase: str, out_dir: Path = GENERALIZATION_DIR) -> dict:
    ...
    earned = Verdict(f"source-monitor v8 {phase} generalization")
    earned.require(
    ...
    earned.require(
        "the frozen v8 acceptance criteria were applied unchanged on every seed",
        fixed_criteria is not None,
        expect=True,
    )
    earned.require(
        "every seed ran the full preregistered control set (20 components)",
        bool(component_counts) and min(component_counts) == 20,
        expect=True,
    )
    ...
    aggregate = {
        ...
        "acceptance_rule": "GO iff every seed satisfies the frozen v8 criteria",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Self-sweep a source-monitor v8 phase and write one aggregate verdict."
    )
    ...
    print(
        f"[aggregate-source-monitor-v8] phase={aggregate['phase']} "
    ...
    for s in aggregate["per_seed"]:
        print(
            ...
            f"lift_frac={s['weakest_lift_attributable_to_competition']} "
            f"scaling_dl1={s['scaling_weight_delta_l1']} "
            f"weak_share_gain={s['weakest_source_share_gain']} "
            f"learning_off={s['learning_off_source_spikes']} "
            f"failing={s['failing_components']}",
            flush=True,
        )
    print(f"[aggregate-source-monitor-v8] wrote {aggregate['verdict_path']}", flush=True)
