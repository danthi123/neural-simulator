...
class SpikingWTASampler:
    ...
    def draw_from_weights(self, weights, candidates, max_retries=3):
        ...
        takes the CALLER's already-computed weights, so the likelihood ablation `_weights()` applies on the seed-word
        path (`_draw`/`draw_svo`, used by the de-risk's LESION verify) NEVER reached this method -- the lesion was
        INERT on the production `_generate_hypothesis` draw (a sampler built with ablate_likelihood=True drew
        IDENTICALLY to the intact one here; the wiring gap diagnosed in 2026-09-20-hollow-open-ended-generation-drive
        and RE-CONFIRMED on the real brain by gap-open-ended-generation-v2). Honor it here too, with the EXACT
        `_weights` ablation semantics: replace the caller's weights with a UNIFORM vector (`np.ones`, no likelihood
        signal). ablate_likelihood=False (default / not lesioned; only BRAIN_SPIKING_DRAW_LESION=1 sets it True on the
        sampler) -> untouched -> BYTE-IDENTICAL to the pre-fix path. Earned by the open-ended-generation load-bearing
        wiring gap (2026-09-20)."""
