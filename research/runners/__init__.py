...
def _record_start():
    ...
    rec = {
        ...
        # BRAIN_/LB_ (2026-09-24, research/lbf-row-registry-hook): the load-bearing-fraction shards' own PER-ROW
        # env (which BRAIN_<X>_LESION / LB_<X>_PROBE flags a given shard actually ran under) was invisible here --
        # every OTHER env family this record already tracks has burned the project via a silent, unrecorded flag.
        "env": {k: v for k, v in os.environ.items()
                if not k.startswith(_PRIVATE_PROVENANCE_PREFIX)
                and (k.startswith(("SIM_", "GAP5_", "HEBB_", "POOL_", "GAP4_", "BRAIN_", "LB_"))
