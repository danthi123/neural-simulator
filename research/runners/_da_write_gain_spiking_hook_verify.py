...
unset / set / set+lesioned, on a minimal fake `chat` carrying only what `da_level_of()` reads
...
WHAT THIS PROVES (GO = A and B and C and D and E):
  (A) OFF (`BRAIN_DA_ENCODING_SPIKING_GAIN` unset) -- `encoding_gain_for()` returns EXACTLY (float ==) what the
      pre-existing `_gain_map()` closed form returns, on BOTH leaf branches (the substrate/default floor=1.0
      branch and the raw/ablation floor=0.5 branch). Byte-identical asserted IN THE DATA (exact float equality),
  ...
  (E) LAZY IMPORT: `research.runners._da_write_gain_spiking_derisk` is NOT in `sys.modules` after exercising the
      OFF arm in a fresh subprocess -- the new mechanism's substrate is never even built when the flag is off,
...
def main():
    ...
    _QUIET = {"BRAIN_DA_ENCODING_SPIKING_GAIN": None, "BRAIN_DA_ENCODING_SPIKING_GAIN_LESION": None,
             "BRAIN_DA_ENCODING_LESION": None, "BRAIN_DA_ENCODING_SUBSTRATE": None,
             "BRAIN_DA_ENCODING_HOMEOSTASIS": None}

    # ── (A) OFF: byte-identical on BOTH leaf branches (substrate floor=1.0, raw/ablation floor=0.5). ──
    _set(_QUIET)
    ...
    # ── (E) lazy import: run the OFF arm in a FRESH subprocess, check sys.modules never gained the new module.
    ...
    probe_src = (
        ...
        "logging.getLogger().setLevel(logging.ERROR)\n"
        "import webapp.da_encoding_drives_chat as DAE\n"
        "os.environ.pop('BRAIN_DA_ENCODING_SPIKING_GAIN', None)\n"
        "\n"
        "class _C:\n"
        "    def __init__(self, da):\n"
        "        self._last_da_drives = {'da_level': da}\n"
    ...
    go = bool(go_a and go_b and go_c and go_d and go_e)

    from tools.verdict import Verdict
    v = Verdict("LEVER-4 (BRAIN_DA_ENCODING_SPIKING_GAIN) production-hook dispatch, default-OFF")
    v.require("(A) OFF byte-identical on the substrate/default leaf (floor=1.0)", go_a_substrate, expect=True,
              note=f"off={off_substrate} == expected={exp_substrate}")
    v.require("(A) OFF byte-identical on the raw/ablation leaf (floor=0.5)", go_a_raw, expect=True,
              note=f"off={off_raw} == expected={exp_raw}")
    ...
    v.require("(E) the spiking module is never imported while the flag is OFF", go_e, expect=True,
    ...
    out = {
        ...
        "A_off_byte_identical": {"substrate_leaf": go_a_substrate, "raw_leaf": go_a_raw,
        ...
        "E_lazy_import": {"subprocess_stdout": proc.stdout.strip(), "subprocess_stderr": proc.stderr.strip(),
                         "GO": go_e},
    ...
    bar = "=" * 100
    print("\n" + bar)
    print("  LEVER-4 PRODUCTION-HOOK VERIFY -- BRAIN_DA_ENCODING_SPIKING_GAIN")
    print(bar)
    print(f"  (A) OFF byte-identical: substrate={go_a_substrate} raw={go_a_raw}")
    ...
    print(f"  (E) lazy import confirmed (never imported while OFF): {go_e}")
