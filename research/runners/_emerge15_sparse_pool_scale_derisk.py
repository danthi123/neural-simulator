...
def _run(seed, n_subj, epochs, k_syn, sparse, k_win=4, act_th=3):
    ...
    if sparse:
        ...
        n_syn = len(row)
    lr = OnBridgeLearner(b, row, col, cells_idx, vocab_n, nE, k_win=k_win, act_th=act_th, lesion=False)
    for _ in range(epochs):
        for cs in col_seqs:
            lr.train_sequence(cs)
    bp = branch_pos - 1                                        # predict s[bp+1] = the branch word at branch_pos
    htm = htm_nextword_acc(lr, col_seqs, bp)
    return {"htm": float(htm), "n_syn": int(n_syn), "vocab_n": int(vocab_n), "nE": int(nE)}
...
def main():
    ...
    ap.add_argument("--k-syn", type=int, default=40, help="sparse: pre-synapses sampled per post cell")
    ...
    d_htm, d_syn = agg("dense"); s_htm, s_syn = agg("sparse")
    go = (s_htm is not None and d_htm is not None and s_htm >= d_htm - 0.05
    ...
    out = {"probe": "emerge15_sparse_pool_scale", "n_subj": a.n_subj, "vocab": len(vocab),
