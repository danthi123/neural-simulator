"""Generator-S pre-registered MULTI-SEED capability gate. Decides
whether a subword spiking LM trained by surrogate-grad BPTT on a real
public corpus generates coherent held-out text. Anti-cheat: FIXED bars
in subword_lm_gate_core (0.20/1.5/0.5/0.20, >=3 seeds, NEVER tuned
here); the load-bearing control is a WORD-SHUFFLE of the train split
(BPE-invariant -> control model has the identical tokenizer/token
distribution, ONLY sequence order destroyed). Kill-safe resume
(skips completed seeds via <ckpt>.resume.json). Honest propagation is
the CONTROLLER's post-run job; main() ONLY computes+prints+writes JSON
(exit 0 == verdict computed [PASS or honest FAIL]; exit 2 == not
runnable). ASCII only."""
...
def _heldout_nll(layers, tok, text, T, xp, max_positions):
    """Teacher-forced per-token NLL over held-out ids. REUSES the
    validated forward_unroll_xp + cross_entropy_loss_np (DRY); SAME
    logits semantics as training (last-layer spike sum over T-window).
    Returns list[float] nll."""
    import numpy as np
...
def main():
    ...
    ap.add_argument("--out", type=str,
                    default="research/findings/raw/g11_bg/"
                            "generator_s_gate.json")
    ap.add_argument("--ckpt", type=str,
                    default="research/findings/raw/g11_bg/"
                            "generator_s_gate.ckpt")
    a = ap.parse_args()
    ...
    print("=" * 64, flush=True)
    print("GENERATOR-S PRE-REGISTERED MULTI-SEED CAPABILITY GATE",
          flush=True)
    print("(subword spiking LM via surrogate-grad BPTT on a real "
          "public corpus;", flush=True)
    print(" FIXED bars 0.20/1.5/0.5/0.20 in subword_lm_gate_core "
          "NEVER tuned here;", flush=True)
    ...
    # Corpus fetched ONCE (idempotent cache); shared across seeds.
    ...
    tr_file = str(cdir / "gen_s_train.txt")
    ...
    for seed in seeds:
        ...
        # WORD-SHUFFLED control corpus (BPE-invariant; only order
        # destroyed). Identical model/config.
        ctl_file = str(cdir / ("gen_s_ctl_s%d.txt" % seed))
        ...
        # Generation metrics from held-out prompts.
        ...
        per_seed_records.append({
            ...
            "train_ppl": tr_ppl, "distinct_trigram": distinct,
        ...
        print("[SEED %d] ho_ppl=%.3f ctl_ppl=%.3f tr_ppl=%.3f "
              "distinct=%.3f copy=%.3f -> %s"
              % (seed, ho_ppl, ctl_ppl, tr_ppl, distinct, copy_frac,
                 v["GATE"]), flush=True)

    agg = gs_aggregate_multiseed(per_seed_verdicts)
    result = {
        "task": "Generator-S pre-registered MULTI-SEED capability gate",
        "mechanism": ("subword spiking LM, surrogate-grad BPTT, real "
                      "public corpus, self-contained at runtime; order "
                      "is learned, not read out of an order-blind pool"),
        ...
        "anti_cheat": {
            "fixed_bars_in_gate_core": "0.20/1.5/0.5/0.20; >=3 seeds; "
                                        "NEVER tuned in the runner",
    ...
    print("\n" + "=" * 64, flush=True)
    print("GENERATOR-S GATE VERDICT", flush=True)
    print("=" * 64, flush=True)
    for r in per_seed_records:
        ...
        print("  seed %s: %s (ho_ppl=%s ctl_ppl=%s tr_ppl=%s "
              ...
              % (r["seed"], vv["GATE"],
                 ...
                 r.get("train_ppl"), r.get("distinct_trigram"),
    ...
    if agg["GATE"] != "PASS":
        print("  NOTE: a maxed FAIL is an HONEST finding -> propagate "
              "+ proceed to pre-staged Generator-D; do NOT "
