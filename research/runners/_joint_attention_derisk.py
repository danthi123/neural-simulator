...
from tools.verdict import Verdict  # noqa: E402  -- earned preconditions carried into the artifact
from tools.lab import attributable_to  # noqa: E402  -- attribute the alignment to its manipulations
...
def main():
    ...
    # Earned preconditions carried INTO the artifact (tools.verdict.Verdict -> a `preconditions` block that
    # tools/gates/verdict_preconditions.py enforces). These mirror the four gate checks in _verdict, so a GO
    # travels with what earned it. No lambda in `expect` (its repr embeds a memory address -> would make the
    # artifact non-deterministic); the booleans come straight from `checks`, the magnitudes ride in `note`.
    v = Verdict("joint attention: STS-TPJ other-attention-schema -> spiking one-of-K spotlight", chance=1.0 / a.K)
    v.floor("intact align vs chance", agg["align_acc"], floor=1.0 / a.K)
    v.require("align_acc >= 0.85 (GATE a)", checks["a_align"], expect=True,
              note="agg align=%.3f" % agg["align_acc"])
    v.require("lesion collapses: align_lesion <= chance+0.10 (GATE b, STS-TPJ read load-bearing)",
              checks["b_lesion"], expect=True,
              note="agg lesion=%.3f band=%.3f" % (agg["align_acc_lesion"], band))
    v.require("scramble -> chance: align_scramble <= chance+0.10 (GATE c, rides ACTUAL partner gaze)",
              checks["c_scramble"], expect=True,
              note="agg scramble=%.3f band=%.3f" % (agg["align_acc_scramble"], band))
    v.require("not-a-copy: align_blind <= chance+0.10 (GATE d, needs gaze x layout)",
              checks["d_notacopy"], expect=True,
              note="agg blind=%.3f band=%.3f" % (agg["align_acc_blind"], band))
    v.disabled("STDP / Hebbian / homeostasis / reward-mod / STP / structural plasticity",
               why="the gaze ring is a pure direction-encoder and the STS-TPJ read uses fixed synapses; the "
                   "lesion holds by construction (no plasticity to regrow the severed read within a trial)")
    decided = v.decide(go=go, verbose=False)

    # Attribution (tools.lab.attributable_to): what fraction of the intact alignment is NOT present in each
    # control? Each control collapses to ~chance, so the alignment is (near-)entirely attributable to the
    # gaze-derived STS-TPJ read -- not to a fixed response (scramble) or a copied coordinate (layout-blind).
    attribution = {
        "vs_lesion": attributable_to("STS-TPJ read (intact vs lesion)",
                                     agg["align_acc"], agg["align_acc_lesion"]),
        "vs_scramble": attributable_to("actual partner gaze (intact vs scrambled)",
                                       agg["align_acc"], agg["align_acc_scramble"]),
        "vs_blind": attributable_to("gaze x layout inference (intact vs layout-blind)",
                                    agg["align_acc"], agg["align_acc_blind"]),
    }
