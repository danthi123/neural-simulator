"""Calibration-only v5 gate for co-resident episodic source monitoring.

The v2 local fast-spiking GABA-A biased-competition MECHANISM is unchanged here.
Only the acceptance rule and the seed partition change.  V2 was recorded NO-GO
solely because a per-source zero-degradation control rejected a 0.0092 loss on
an already-strong source (seed 217).  The P3 functional-role spec
(2026-08-06-source-monitor-P3-functional-role-and-tradeoff-spec.md) shows the
whole-brain role sets reliability by the WEAKEST source, so the correct control
is a bounded-loss, guard-the-floor, max-min rule, not zero degradation.  This
gate applies that rule (the same one the v3 preregistration filed on fresh
seeds) to the unchanged v2 circuit.

Only calibration seeds 650 and 651 are open.  Development seeds 652, 653, and
654 and held-out seeds 655, 656, and 657 are named but mechanically rejected.
Seed 649 is implementation smoke only and can never be formal evidence.  The
runner does not choose words, calculate confidence, or decide whether to
respond.
"""
...
def validate_calibration_seed(seed: int) -> int:
    """Open the two v5 calibration seeds, plus the implementation-smoke seed.

    Seed 649 runs the full pipeline for implementation verification only; it is
    preregistered as never-formal-evidence, so a smoke run cannot consume a
    calibration, development, or held-out seed.
    """

    seed = int(seed)
    if seed not in CALIBRATION_SEEDS and seed != SMOKE_SEED:
        raise ValueError(
            f"seed {seed} is not a v5 calibration seed; allowed={CALIBRATION_SEEDS}, "
...
def evaluate_calibration_seed(
    ...
    """Run one v5 calibration seed and all preregistered controls."""
    ...
    earned = Verdict("source-monitor co-residency v5 calibration")
    earned.require(
    ...
    earned.disabled(
        ...
        why="v5 isolates Hebbian source association plus fixed local GABA-A competition",
    )
    ...
    return {
        ...
        "honest_scope": (
            "V5 keeps v2's fixed local fast-spiking GABA-A competition unchanged and applies the bounded-loss "
            "max-min acceptance rule justified by the whole-brain role. "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one calibration seed for source-monitor co-residency v5."
    )
    ...
    if args.json:
        ...
        print(f"[source-monitor-coresidency-v5] wrote {out_path}", flush=True)
