...
Pre-registered in research/findings/2026-09-24-open-ended-gated-turn-PREREGISTRATION.md (Part B; Amendments 2, 3 and 4).
...
MEASUREMENT CONDITION. The `open_ended_gated` response key exists only when BRAIN_OPEN_ENDED_GATED=1. This module
declares `REQUIRED_ENV = {"BRAIN_OPEN_ENDED_GATED": "1"}` below, which `research/runners/lbf_rows/__init__.py`'s
registry-merge hook honours (commit `eed3652a0`, merged from main on this fix round): at production defaults (the
env unset) it PARKS every key this module would otherwise contribute, so `open-ended-turn-affect-drive` and
`open-ended-turn-gnw-drive` are OUT of `load_bearing_fraction.FACULTY_LESIONS`/`FACULTY_PROBES` and every headline
battery reads NOT-EXERCISED for them -- an opt-in faculty, never "not load-bearing". Confirmed by direct import:
a fresh process with the env unset has neither key in `FACULTY_LESIONS`; a fresh process with
`BRAIN_OPEN_ENDED_GATED=1` set before import has both. (`open-ended-turn-faculty-drive` is EXCLUDED from the
registry either way -- see its own `PARKED` entry below.) The two live rows are measured in tag b2b-caps with
`tools/lb_shard.py jobs ... --extra-env BRAIN_OPEN_ENDED_GATED=1 --faculties <rows>` (LLM disabled, stub renderer),
a DEDICATED invocation whose `--faculties` lists exactly `FACULTIES` below (Amendment 3: extra-env applies to every
job of an invocation, so any other row generated with them would be measured with the gated turn ON).
...
integrity check, not a decision field.
  * open-ended-turn-faculty-drive -- the lesion sets the INPUT saliences of the BG speak/abstain race to an equal
    ...
    PARKED FROM THE REGISTRY, not just DESCRIPTIVE_ONLY (fix round after the 2026-09-24 re-review): DESCRIPTIVE_ONLY
    is read only by this module's own `score_row()` dev-smoke helper -- `load_bearing_fraction.py`'s own scoring
    never reads it, so once REQUIRED_ENV is met a coin-flip verdict would silently enter the real headline
    load-bearing counts. The smallest honest fix: this row is also listed in the module-level `PARKED` dict below,
    which the registry hook DOES honour unconditionally (never modifying load_bearing_fraction.py itself) -- it
    keeps this key out of `FACULTY_LESIONS`/`FACULTY_PROBES` whether or not BRAIN_OPEN_ENDED_GATED is set. It stays
    measurable only as the `--bg-curve` descriptive read (PREREG Amendment 1) and via this module's own
    `score_row()` for dev-seed smokes; it can never contribute to a load-bearing fraction number.
...
EXTRA_LESIONS = {
    "open-ended-turn-faculty-drive": dict(
        ...
        note="PARKED (Amendment 4): never enters FACULTY_LESIONS/FACULTY_PROBES, BRAIN_OPEN_ENDED_GATED set or not "
             "(see the module-level PARKED dict). Kept here for score_row()'s dev-smoke use and the audit trail. "
             "Cuts the afferents from the brain reads (GNW route, Gate-B affect) into the gated turn's BG "
             "speak/abstain race: both channels get the same baseline salience (0.5, 0.5); organs stay installed. "
             "Measured on the off-KB 'unknown' turn, where the intact saliences are (0, 1). DESCRIPTIVE ONLY (PREREG "
             "Amendment 3): a single-shot read of a race the cut moves to P(SPEAK)~1/2 is a per-seed coin flip; "
             "never counted toward a load-bearing claim. Decision fields bg_action / reply_kind (marker_level is "
             "trace: the marker WTA's own lesion mode makes it None by construction)."),
...
# Fix round after the 2026-09-24 re-review: load_bearing_fraction.py does not read DESCRIPTIVE_ONLY (only this
# module's own score_row() dev-smoke helper does), so a coin-flip verdict would otherwise enter the real headline
# load-bearing counts once REQUIRED_ENV is met. PARKED is honoured unconditionally by the registry hook
# (research/runners/lbf_rows/__init__.py), regardless of BRAIN_OPEN_ENDED_GATED -- the smallest honest fix, and it
# does not touch load_bearing_fraction.py. The row stays measurable only via --bg-curve (descriptive) and this
# module's own score_row() (dev-seed smokes); it can never contribute to a load-bearing fraction number.
PARKED = {
    "open-ended-turn-faculty-drive": "non-discriminating as a single-shot LBF row (PREREG Amendment 1/3): the "
        "pooled dev bg-curve at the intact salience (0, 1) reads SPEAK 0 / STAY_SILENT 23 / none 1 of 24 races, and "
        "at the cut salience (0.5, 0.5) SPEAK 12 of 24 (bg_curve_dev_s7_11_13.json, pooled) -- one LBF build per arm "
        "is one race, so the verdict is a per-seed coin flip whatever the afferent coupling. Kept out of "
        "FACULTY_LESIONS/FACULTY_PROBES entirely (not merely DESCRIPTIVE_ONLY, which the real scorer never reads); "
        "read via --bg-curve instead.",
}

# PREREG Amendment 3: rows whose registered single-shot verdict cannot discriminate (reported, never counted toward a
# load-bearing claim), with the reason. The LBF runner itself does not read this; the lane's scorer (score_row(),
# used by the dev-seed smoke only) and the finding do. Kept for that dev-smoke path even though the row is also
# PARKED above -- PARKED and DESCRIPTIVE_ONLY answer different questions (is it IN THE REGISTRY vs does the row's
# OWN VERDICT count), and score_row() still runs this row directly (never through the registry) for the smoke.
DESCRIPTIVE_ONLY = {
    "open-ended-turn-faculty-drive": "single-shot race: intact P(SPEAK) 0/24 at (0, 1) vs cut 12/24 at (0.5, 0.5) on "
...
# the keys the dedicated b2b-caps invocation must be limited to (tools/lb_shard.py --faculties). Two, not three: the
# module also declares EXTRA_PROBES for the PARKED "open-ended-turn-faculty-drive" (kept for score_row()'s dev-smoke
# use and the module docstring's audit trail), but it never enters FACULTY_LESIONS/FACULTY_PROBES, so passing it to
# `load_bearing_fraction.py --only` would fail; it is read via --bg-curve instead, not this invocation.
FACULTIES = tuple(r[0] for r in EXTRA_PROBES if r[0] not in PARKED)
