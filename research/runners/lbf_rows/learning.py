...
Two new load-bearing-fraction rows for the shipped, default-ON offline-learning faculties:
  ...
  * D5 learn-through-use consolidation (`BRAIN_D5_CONSOLIDATE`, webapp/continuous_engine.py, default-ON since
    ...
    weights. `BRAIN_D5_CONSOLIDATE=0` is the LESION (the shipped byte-identical escape).
  * Offline sleep-replay (`BRAIN_SLEEP_REPLAY`, webapp/continuous_engine.py, default-ON since 2026-08-26, 6-seed
    ...
    last sleep is reactivated in store order through the SAME BTSP kernel. `BRAIN_SLEEP_REPLAY=0` is the LESION.
...
EXTRA_LESIONS = {
    "d5-consolidate": dict(
        flag="BRAIN_D5_CONSOLIDATE", value="0", kind="neural-lesion",
        note="webapp/continuous_engine.py: BRAIN_D5_CONSOLIDATE=0 makes consolidate_used_memory a no-op (the "
             "tick still runs -- n_sessions_ticked>0 -- but the substrate's plateau-gated BTSP reactivation "
             "never fires), so a later recall's graded strength (episodic.graded_cue.depth_hold) never rises "
             "and its reply never surfaces 'recall strength'. Default-ON (BRAIN_D5_CONSOLIDATE unset -> True); "
             "this is the shipped byte-identical escape, not a new knob."),
    "sleep-replay": dict(
        flag="BRAIN_SLEEP_REPLAY", value="0", kind="neural-lesion",
        note="webapp/continuous_engine.py: BRAIN_SLEEP_REPLAY=0 makes consolidate_sleep_replay an immediate "
             "no-op (the deep-idle tick still runs) -- the recently-stored batch is never reactivated, so a "
             "later recall never reads a risen depth_hold and never surfaces the 'I also replayed it offline' "
             "clause + when-rank/batch-size. Default-ON (unset -> True, post 2026-08-26 6-seed soak GO); this "
             "is the shipped byte-identical escape, not a new knob."),
}
