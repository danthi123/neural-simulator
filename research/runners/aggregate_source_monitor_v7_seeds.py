"""Hands-off self-sweep + aggregator for source-monitor v7 generalization.

Launch one self-contained process, get one earned verdict. This runs EVERY seed
of a phase against the FROZEN v7 mechanism (imported unchanged from
``_laneC_source_monitor_coresidency_gate_v7``), collapses per-seed PASS/FAIL into
one GO/NO-GO, and writes a single aggregate verdict artifact under
``research/findings/raw/source_monitor_v7_generalization/``.
...
Run (hands-off; provenance is auto-stamped via ``research.runners.__init__``):
    ...
    SIM_BACKEND=numpy python -m research.runners.aggregate_source_monitor_v7_seeds \
        --phase development
"""
...
from research.runners._laneC_source_monitor_coresidency_gate_v7 import (
    GENERALIZATION_DIR,
    PHASE_SEEDS,
    _development_is_go,
...
def _seed_summary(row: dict) -> dict:
    ...
    homeo = row.get("homeostasis", {}).get("intact_settling", {})
    return {
        "seed": row["seed"],
        "status": row["status"],
        ...
        "homeostasis_threshold_delta_l1": homeo.get("threshold_delta_l1"),
        "per_source_threshold_means": homeo.get("per_source_threshold_means"),
...
def run_phase(phase: str, out_dir: Path = GENERALIZATION_DIR) -> dict:
    ...
    earned = Verdict(f"source-monitor v7 {phase} generalization")
    earned.require(
    ...
    earned.require(
        "the frozen v7 acceptance criteria were applied unchanged on every seed",
    ...
    aggregate = {
        ...
        "acceptance_rule": "GO iff every seed satisfies the frozen v7 criteria",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Self-sweep a source-monitor v7 phase and write one aggregate verdict."
    )
    ...
    print(
        f"[aggregate-source-monitor-v7] phase={aggregate['phase']} "
    ...
    for s in aggregate["per_seed"]:
        print(
            ...
            f"thr_delta_l1={s['homeostasis_threshold_delta_l1']} "
            f"learning_off={s['learning_off_source_spikes']} "
            f"failing={s['failing_components']}",
            flush=True,
        )
    print(f"[aggregate-source-monitor-v7] wrote {aggregate['verdict_path']}", flush=True)
