...
## ⛔ CORRECTION (2026-09-01, same day — `research/findings/2026-09-01-confidence-forthcomingness-default-on-flip-GO.md`)

The "NOT byte-identical-off" verdict below (and its `verify_confidence_ltm_loadbearing.json` artifact's `off_n_sentences: 4` / `off_matches_no_ltm_tier: false` readings) is RETRACTED — that JSON was STALE, generated before the margin-norm code in this branch's own commit (`metacog_production_organ.py` / `rf_phasor_composer.py` / `rich_answer_composer.py`) was finalized, and never regenerated before this finding was committed. A fresh, unmodified re-run of the identical `verify_confidence_ltm_loadbearing.py` script (in an isolated worktree pulling these exact files) passes `byte_identical_off` on all 6 seeds, matching the no-LTM-tier reference verbatim (same `n_sentences`, same answer text). The 6/6 discrimination claim (`vary_lesion_all_GO: true`) below is UNCHANGED and stands. See the superseding finding for the full correction and the resulting AUTO-FLIP.
