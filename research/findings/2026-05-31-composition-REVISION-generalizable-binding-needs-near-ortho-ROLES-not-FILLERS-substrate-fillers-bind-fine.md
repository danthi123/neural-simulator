...
3. ABSTRACT +-1 VSA isn't directly biological (neurons fire >=0). The biological bind is a >=0 MULTIPLICATIVE GAIN-FIELD (apical role gates basal filler), which lacks the clean +-1 self-inverse (role (x) role = 1). The >=0 gain-field realization + its unbinding need separate validation.
