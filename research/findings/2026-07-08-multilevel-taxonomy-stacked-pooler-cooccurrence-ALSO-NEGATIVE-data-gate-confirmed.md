...
## Addendum — WikiText (encyclopedic) is NOT-EVALUABLE (degenerate super grouping)
Tested the same co-occurrence-super mechanism on WikiText (encyclopedic Wikipedia, more taxonomic than children's stories) at K=1024/24-fine/6-coarse: all 6 seeds "too few usable supers" -- the fine-cluster co-occurrence super grouping is DEGENERATE on encyclopedic text (nearly everything co-occurs -> imbalanced supers, no ≥2 usable supers). So the co-occurrence-super mechanism doesn't form a clean hierarchy on WikiText either. ⇒ the taxonomy boundary is comprehensively characterized: 2 mechanisms (centroid + stacked-pooler co-occurrence) × 2 corpora (flat TinyStories NEGATIVE, encyclopedic WikiText not-evaluable) + dictionary genus (noisy) -- ALL fail. Multi-level taxonomy from UNSUPERVISED real-corpus co-occurrence is a genuine deep boundary; the is-a signal lives in definitional TEXT ("X is a Y"), whose extraction needs POS-clean NLP (or a synthetically-clean is-a corpus like EMERGE-44's). The mechanism (stacked pooler) is validated on clean is-a data; real unsupervised extraction is the open problem.