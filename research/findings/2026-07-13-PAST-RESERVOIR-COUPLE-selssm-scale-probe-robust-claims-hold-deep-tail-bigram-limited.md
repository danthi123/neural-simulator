...
## Next
- The mission-central next is the ON-BRIDGE (spiking) coupling (all 3 pieces exist), not more numpy scale (the deep-tail-vs-bigram is a known scale wall, not a coupling failure). A genuine deep-tail-clears-bigram test needs the 50–200× scale the reservoir-scale finding closed as tractable-infeasible here.
