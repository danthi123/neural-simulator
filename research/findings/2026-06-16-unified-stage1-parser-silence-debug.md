...
**GPU behavioral confirmation:** the full `_unified_stage1_merged.py` gate on the fixed builder got all the way through — byte-identity **True**, H5 concept-cat **0.75** (margin +0.780), H6-hybrid **0.75**, MOAT **ABSTAIN** — and the parser is **un-silenced** (`KeyError: 'action'` gone; the parse now decodes agent/action/patient correctly). The run then surfaced a *separate* test-construction bug: `KeyError: 'chase'` inside `composer.store`, because the no-regression check used "dog chase cat" — "chase" is the generalization-test `ACTION_WORD`, not in the composer's `DEFAULT_VOCAB`. Fixed the check to the shipped in-vocab sentence ("dog go north") that `tests/test_nav_conv_step2b_coresident.py` validates (commit `b7e3401b`). The parser-silence root cause is resolved.

## FOLLOW-UP — the proper `sim/` fix (owner-approved): gate the weight clips

The runner-side snapshot/restore is a workaround; the underlying foot-gun is in `sim/bridge.py`. The per-step weight **decay** is gated by `cp_plasticity_rate_gain` (a frozen pathway at gain 0 does not decay), but **all three weight CLIPs were ungated** — the Hebbian clip (~6509), the reward/STDP clip (~6829, the "5a characterized gap"), and the homeostasis clip (~7092/7094). With owner approval, all three are now gated the same way the decay is: clip **only gain>0 synapses**, leaving frozen (gain 0) weights verbatim. Uniform gain (all 1.0) reproduces the un-gated clip exactly; `cp_plasticity_rate_gain is None` (the default for non-gated configs) takes the **byte-identical** un-gated branch. Operand sizing mirrors the gated decay directly above it (proven-correct). Commit `2d0ae99a`.

Regression test `tests/test_plasticity_gate_clip.py` (numpy backend, CPU): a frozen (gain 0) synapse at weight 50 **survives** when `hebbian_max_weight=20` under firing, while a plastic (gain 1) synapse is clipped to ≤20; and the `gain=None` default path still clips (byte-identity). The 5 numpy-backend failures in `test_regions`/`test_global_plasticity_gain` are **pre-existing** (`profile_name_for_conn` init bug), confirmed by reverting `bridge.py` (still fail). This makes the runner-side workaround redundant (kept as belt-and-suspenders).
