...
status: no-go
date: 2026-09-16
original-verification-date: 2026-09-05 (see rescued-2026-09-16-from below -- date bumped to the LANDING date so
  gates/deep_research_at_wall windows against the ADDENDUM's own fresh external check, not the original run)
...
rescued-2026-09-16-from: unmerged branch research/metacog-spiking-margin-prodflip-verify, commit 09153561d (never
  merged to main; this finding + its runner + raw artifact did not exist on main until this rescue — see the
  ADDENDUM below)
---
...
false — this file, its runner, and its raw artifact were reachable from NO branch main ever merged. A separate
task this session was briefed on the premise "an already-6/6-seed-GO spiking replacement exists and is
UNWIRED," sourced from `research/coordination/scaffold_retirement_backlog.md`'s rank-9 entry, which cites only
the composer-level `...-derisk-PARTIAL.md` finding and never mentions this integrated NO-GO — because this
finding was never on `main` for that backlog entry (or anyone) to cite. Both halves of that premise are wrong:
the mechanism is already wired end-to-end behind `BRAIN_METACOG_SPIKING_MARGIN` (confirmed by inspection of
`research/runners/rf_phasor_composer.py`'s `spiking_recall_margin` and
`research/runners/metacog_production_organ.py`'s `mean_role_confidence`/`spiking_recall_margin_enabled`, both on
`main` since `2ca5370e8`), and the flip was already integration-verified NO-GO, not GO. This finding, its
runner, and its raw artifact are rescued onto `main` by this session (unmodified except this addendum + the raw
JSON/prov-JSON copied byte-for-byte) specifically so a future session cannot re-derive the same wrong premise
from the same stale backlog entry; `scaffold_retirement_backlog.md` rank 9 is corrected in the same commit.
...
new fact. It does NOT establish that real traffic never enters the band: the 2026-08-27 reference characterized
CONFIDENT real turns only, the LTM tier (where weaker/partial matches over a much larger candidate vocabulary
are structurally more likely) was explicitly not exercised by this sweep (see "Scope notes" above), and the
residual's own mechanism (two substrates measuring correlated-but-not-identical decisiveness) is a property of
degraded/marginal recalls specifically — the exact traffic slice the honesty-hedge exists to protect. The
directionally one-sided (4-for-4 overconfident) empirical rate stands as the load-bearing reason for NO-GO
independent of this band question.

**Staleness risk, now concretely named.** `2ca5370e8` (the wiring this NO-GO tested) predates `eca75a3f1`
("retire host rf composer + numpy-KB LTM → onebrain composer + substrate-store default-on; no-regression GO"),
a default-composer change landing AFTER this integrated verification. The wiring itself is confirmed unchanged
by inspection, but the composer this verification built against may no longer be what a real turn resolves
through by default. A 6-seed re-run of the UNMODIFIED runner (`_metacog_spiking_margin_prodflip_verify.py`,
which forces its own env — no external flag flip needed) against present `main` is queued on the pool (see the
board / pool queue at commit time) to confirm the NO-GO verdict still holds rather than trusting an
8-days-stale-by-changed-composer result.

**Verdict for the wire-in de-risk task that surfaced this:** NOT safe to flip default-ON on current evidence.
The wiring is sound, additive, and already byte-identical-off (confirmed unchanged). No further build work is
warranted until the queued re-run either reconfirms or overturns this NO-GO on present `main`.

**External grounding for the "next rung" above (2026-09-16 literature check, `gates/deep_research_at_wall`):**
the proposed directionally-aware recalibration is not an ad hoc patch — it is the standard signal-detection-
theory result for asymmetric error costs. Under SDT (Green & Swets 1966, already this finding's own framing;
Landy, NYU SDT lecture notes, https://www.cns.nyu.edu/~eero/math-tools24/Handouts/sdtchapter.pdf), the
Bayes-risk-minimizing
criterion `c` is NOT the naive midpoint between two distributions when the two error types carry unequal cost —
it shifts toward the response that avoids the COSTLIER error. Here the honesty-boundary mission treats false
confidence as costlier than extra-hedging (CLAUDE.md's own framing: the hedge exists specifically to prevent
overclaiming), so the theory directly endorses biasing `SPIKING_MARGIN_LO`/`HI`'s effective threshold crossing
away from the empirically-observed 4-for-4 overconfident split rather than treating a re-centered fit as
arbitrary. This does not change today's verdict (the flag stays default-OFF), only grounds the next lever
already named above in an established result rather than leaving it a hunch.
