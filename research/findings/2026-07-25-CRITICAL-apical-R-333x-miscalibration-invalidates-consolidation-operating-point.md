...
### 2026-07-29 ⭐⭐ PRIMARY-SOURCE READ (O'Keefe-Nadel 1978) RE-DIAGNOSES THE DEFECT — and a corpus-access bug was hiding the sources

**Prompted by an owner challenge on whether I had been doing the research properly. Honest audit: NO.** Two RAG
queries all session (both decisive — they found catalog B.16 and BTSP), the findings docs read at moderate depth,
but **ZERO primary sources**, `tools/before_you_build.sh` never run, `tools/lab.py` never imported (and the two
retractions of the night are exactly what its `before_after` exists to force), and the research gate fired only
AFTER 11 agents had been launched on three mechanisms.

**⛔ INFRASTRUCTURE DEFECT FOUND — plain `grep` returns NOTHING on the primary sources.** They are **ISO-8859**
encoded with CRLF (`file` says so), so grep treats them as BINARY and silently reports no matches: `grep -c place
okeefe-nadel.txt` prints nothing, while **`grep -ac place` prints 830** (and `hippocamp` 2353). **Any session or
subagent searching these texts without `-a` concludes the source does not cover the topic.** This plausibly
explains a standing pattern of leaning on findings docs instead of sources. Also note the paths: the texts are at
`~/Projects/sim-catalog/.catalog-work/{okeefe-nadel,buzsaki}.txt`, NOT the documented
`references/textbooks/*/full-book.txt` (that glob returns nothing). **Use `grep -a` and the `.catalog-work` path.**

**THE MECHANISM, read from O'Keefe & Nadel 1978 (Physiology, ~p.211):**

> "the field is generated by **active inhibitory processes** acting on neurones with a tendency to fire
> spontaneously or **with a diffuse source of excitatory drive**. This inhibition **restricts the area** of an
> environment where the place unit fires. It derives from environmental stimuli either directly or indirectly
> **through the mediation of other place units**. When the **appropriate number** of these inhibitory inputs
> converge on a place unit, **a discrete place field results. When there are too few environmental inputs, the
> unit fires over a large part of the environment or in several parts of the environment.** On the other hand, too
> many such inputs will **totally inhibit** the place unit and result in the absence of a field."

**THIS INVERTS MY ARCHITECTURE, and my four named levers all missed it.** I was POTENTIATING excitatory place→read
weights and measuring the excitatory profile; O'Keefe-Nadel say the field is the **un-inhibited COMPLEMENT** of a
diffuse drive, its width set by the **NUMBER of converging, CUE-DERIVED inhibitory inputs**. My soft-WTA is a
shared FS pool driven uniformly by all readers ⇒ **position-INDEPENDENT inhibition**, which can only lower global
gain — precisely what I measured earlier ("uniform FS inhibition reduced firing 450→40 but produced NO
differentiation"). All four levers I had named (inhibition strength, more readers, `btsp_hetero_dep`,
`btsp_elig_hard_thresh`) concern MAGNITUDE or WITHIN-CELL competition. **None makes the inhibition itself
place-specific.**

**THE 1978 PREDICTION TESTED AGAINST MY OWN 2026 DATA — CONFIRMED.** "Too few inputs ⇒ fires ... **in several
parts of the environment**." Counting contiguous above-half-max runs per cell in the 6-seed BTSP weights:

| seed | 42 | 43 | 44 | overall |
|---|---|---|---|---|
| peaks per cell | 5.25 | 2.83 | 4.42 | **4.17** |
| fraction multi-peaked | 1.00 | 1.00 | 1.00 | **1.00** |

**My fields are not BROAD — they are FRAGMENTED, 4.17 discrete peaks per cell, 100% of cells.** That re-diagnoses
the defect entirely: it is NOT weak learning, it is a **missing convergent inhibitory constraint**. It also
explains the low `circ_resultant` mechanically (multi-peaked vectors cancel) and explains the INTEGRATION FAILURE
quantitatively — a cell with 4.17 peaks has no meaningful `argmax`, so ordering by it is near-random, which is
exactly the measured scramble-equivalence (LEARNED 0.678 vs SCRAMBLED 0.683).

**⇒ NEXT MECHANISM, from the source rather than from my guesses: CUE-DERIVED CONVERGENT INHIBITION** — inhibitory
drive that is itself a function of position (derived from the environmental stimuli / other place units), with the
