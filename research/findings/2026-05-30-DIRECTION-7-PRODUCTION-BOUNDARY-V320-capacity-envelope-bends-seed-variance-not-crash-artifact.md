# Direction 7 PRODUCTION = DIRECTION_7_PASS by frozen mean-rule but a genuine BOUNDARY at V=320; OI L=5 mean 0.830 with one seed below bar (0.70); the capacity envelope SHATTERED at V=160 reasserts at V=320; crash-retrain confound ruled out; pillar n=110 BOUNDARY candidate

**Date:** 2026-05-30 ~11:50 EDT (production training completed ~08:14; inline probe completed; diagnostic + this doc shortly after)
**Status:** DIRECTION_7_PASS by the pre-registered frozen mean-rule, but HONESTLY a BOUNDARY result (one of three seeds below the 0.80 bar at L=5 OI). Pillar n=110 BOUNDARY candidate pending adversarial reviewer. NOT a clean PASS equivalent to pillars n=108 (D4 V=80) / n=109 (D6 V=160).
...
| bridge | seed 42 | seed 43 | seed 44 | seed 43 delta | retrained? |
|---|---|---|---|---|---|
| A_nouns | 0.048 | 0.054 | 0.061 | ~0 | no |
| B_verbs | 0.063 | 0.083 | 0.075 | +0.014 | no |
| C_adj | 0.074 | 0.107 | 0.065 | +0.037 | no |
| D_spatial | 0.055 | **0.101** | 0.043 | **+0.052 (worst)** | **no** |
| E_functional | 0.064 | 0.081 | 0.071 | +0.013 | **YES (retrained)** |

seed 43 is uniformly less orthogonal across ALL FIVE bridges (overall mean 0.085 vs 0.061 / 0.063 for seeds 42 / 44 -- ~40%% less orthogonal). Critically, the WORST-degraded bridge for seed 43 is **D_spatial (+0.052), which was NOT retrained**, while the retrained E_functional is among the LEAST affected (+0.013). If the crash had caused seed 43's drop, E_functional would be its worst bridge -- it is not. Therefore seed 43's below-bar L=5 OI is **genuine seed variance at the capacity boundary, not a crash artifact.**

## Biology-translatable insight

At V=160 the dedicated-pool grounded-symbol geometry was near-orthogonal and seed-robust (pillar n=109 diagnostic: all seeds abs cos ~0.04). At V=320, twice the concepts per bridge (64 vs 32), the per-bridge mean-centring (cortical pooled-inhibition analogue) has to separate twice as many concepts against the shared common-mode, and the orthogonality becomes seed-sensitive: some random topographic-prior draws (seed 42, 44) still separate cleanly (~0.06), others (seed 43) degrade (~0.085) enough to drop 5-way composition below bar. The dedicated-pool architecture's effective-orthogonality advantage has a vocabulary ceiling, and V=320 is where it begins to bind. This is a genuine, measured capacity-envelope result -- the first in this arc that LOCATES a ceiling rather than shattering a prediction.
