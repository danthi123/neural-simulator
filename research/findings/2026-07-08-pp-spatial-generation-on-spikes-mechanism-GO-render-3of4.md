...
**Verdict:** GO (4/4, 3-seed 42/43/44) — PP spatial generation on spikes: order + productive morphology + prepositions + moat, render-fidelity 4/4. The initial 3/4 miss (the EMERGE-75b A→W read-path residual) was **SURPASSED** by a larger pre-spell settling window (reset_steps 50→150).

## UPDATE — the EMERGE-75b A→W read-path residual SURPASSED (3/4 → 4/4)
The initial run rendered 3/4 (the subject "owl" misread on the first scored render — the characterized EMERGE-75b A→W read-path state-carryover). The fix, per the boundary-surpassing workflow: the A→W drive path already does a 50-step pre-spell settling with zero input, but 50 steps do NOT fully decay the Izhikevich ADAPTATION current (u) carried from the prior spell, so a word misreads on deep render history. Raising the pre-spell settling window to **reset_steps=150** fully decays the adaptation → **history-independent A→W reads → 4/4 exact** ("the owl runs to the pond" now correct on the first scored render). This confirms EMERGE-75b's named hypothesis (a settled-state read, not the post-build reset that made it worse) and realizes it cheaply (more settling steps). `ConceptFrameSpeaker.spell(reset_steps=)` is additive (default 50 = byte-identical); the `ProductiveMultiSpeaker` uses 150. **High-leverage: this lifts the render-fidelity cap on ALL spiking generation** (property/relational/PP), not just PP.
