...
## ✅ 6-SEED CONFIRMED (2026-08-08) — GO on 6/6, held-out mean 0.873
<!--derived-->
The parent 6-seed sweep (42/43/44/100/101/102) is **GO on all 6**: held-out compositional 0.72 / 0.92 / 0.88 / 0.92 / 0.88 / 0.92 (mean **0.873**, range [0.72, 0.92]) vs marginal-prior lookup 0.000 and nearest-neighbour retrieval 0.000 (chance 0.040) on every seed; `compositional_not_additive=True` every seed; both lesions (zero-W, input-silence) collapse held-out to the prior every seed; seeded byte-identical. Deterministically reproducible (each seed sets `cfg.seed`): `SIM_BACKEND=numpy python -m research.runners._forward_model_reservoir_derisk --seeds 42 43 44 100 101 102`. **A REQUIRED FIX landed to reach 6/6:** the reservoir builder (`_emerge82_onbridge_lsm_derisk._build_reservoir_bridge`) left `enable_structural_plasticity` at its default `True`, so the bridge remodelled its own synapse set mid-run and the post-init snapshot no longer matched the live per-synapse arrays — `final_state()`'s wash-out restore raised `could not broadcast (N,) into (M,)` on seeds 44/100/102 (crash, NOT a low score — the silent-failure trap: a run whose verdict is ERROR is not a negative). Disabling structural plasticity + pruning (a reservoir must be structurally FIXED; only the read-out learns) fixes the crash AND removes a reservoir-semantics confound; the three seeds that already ran are unchanged (42=0.72, 43=0.92, 101=0.88). The world-model SIMULATION faculty generalizes across all seeds — the crux that gates bridge integration.
