...
3. **NAV STAGE-B INTEGRATION + 6-SEED GATE — SUBAGENT IN FLIGHT `acf21d7caa128435d` (background; runner-side ONLY, hard sim/-protected STOP; DO NOT duplicate, DO NOT touch `research/runners/g11_bg_runner.py`).** The BRAIN-BASED-ONLY completion: make the existing `str_striosome_X` pools a NEURAL value critic (perceived-state drive + plastic →snc weight learned by the SNc δ), route `str_striosome_X→snc` `receptor="gaba_b"` + `cfg.enable_gabab=True`, and DROP the host `−snc_value_gain·_V_scaffold` term in `_I_snc` (the neural GABA_B inhibition now supplies −V). New flag `--enable-neural-critic` (Stage-A byte-unchanged when off). Cheap-first: 2-seed SMOKE (does the critic learn + run?) GATES the 6-seed deterministic A/B (neural vs Stage-A; acceptance = neural summed-reward ≥ Stage-A; honest negative = valid deliverable). Returns the runner diff (sim/ MUST be byte-empty) + results WITHOUT committing, for controller review. **CONTROLLER ON ITS RETURN:** verify sim/ untouched, review the runner diff + the smoke/6-seed verdict, commit + push both remotes, report. THEN (nav fully biologized + audited) the parked single-instance unification. This is the LAST subtraction piece of Stage B (the neural critic both LEARNS V and SUBTRACTS it onto the DA cell via real GABA_B/GIRK biology).
