...
## >>> EXACT NEXT (2026-06-08 — GABA_B/GIRK BUILD IN FLIGHT; read THIS FIRST; supersedes every block below) <<<

**⚙️ A PROTECTED-EDIT BUILD SUBAGENT IS RUNNING IN THE BACKGROUND — `a9bcc05afdfc193b7`. DO NOT spawn a duplicate, DO NOT start new work, DO NOT touch `sim/` (the subagent owns the working tree: `sim/bridge.py`, `sim/config.py`, `sim/kernels.py`, `sim/regions.py`, + `research/runners/snc_stageb_critic_probe.py` are mid-edit/uncommitted).** Owner APPROVED the GABA_B→GIRK protected edit with **Option (a): per-`RegionPathway` `receptor` field** (verbatim: "Approve — per-pattern receptor (recommended)"); the design (`2026-06-08-gabab-girk-conductance-design.md`) was byte-reviewed + presented + approved. The subagent is implementing the edit EXACTLY per the design (additive, default-off, byte-identical-when-off — the NMDA pattern inverted: a 2nd inhibitory conductance `g_gabab`, `E_K≈−90 mV`, `tau≈150 ms`, routed only through `receptor="gaba_b"` synapses), verifying byte-identity (CPU test suite + a direct-mode before/after), extending the probe with `--gabab` + the two anti-cheats (conductance-lesion, GABA_A-only A/B), and running the de-risk on seeds 42/43/44. It was instructed to **return the full `sim/` git diff + the de-risk result WITHOUT committing** (leave changes uncommitted for my byte-review).

**CONTROLLER'S EXACT NEXT — on the subagent's completion notification (do this, in order):**
1. **Byte-review the protected `sim/` diff myself** (trust-but-verify, do NOT delegate): (a) `fused_conductance_decay_and_current` is **byte-unchanged** + the new `fused_gabab_decay_and_current` is the additive sibling; (b) Izhikevich/HH/AdEx dynamics kernels + call sites **untouched**; (c) the `receptor` field on `RegionPathway` mirrors `transmission_gate` EXACTLY with default `"gaba_a"` (+ the `_build_pathway` return dict carries `"receptor": pw.receptor`, consumed in `inject_explicit_wiring` only under the `enable_gabab` guard) — verified the clean baseline this session (regions.py:236 field, :620 dict key); (d) the B4 per-step GABA_B current block is fully guarded (`enable_gabab` False + `cp_conductance_g_gabab is None` ⇒ block skipped ⇒ `total_input_current_pA` **bit-identical** to today); (e) the 4 new config fields default OFF/zero. The protected set MUST be additive-only with the only existing-line edits being new dataclass fields / new `None` decls / the one guarded appended current term.
2. **Review the de-risk result:** gate (i) **STATE-SPECIFIC SNc GAP** (`unpredicted_rate > 1.30 × predicted_rate`, sign-consistent ≥3 seeds) — the gate the GABA_A direct projection FAILED 0/3 — must now PASS, WHILE gates (ii) V cue-gated, (iii) omission-dip, (iv) US-burst-shrink are RETAINED; AND the anti-cheats hold (conductance-lesion → the gap VANISHES; GABA_A-only A/B → `receptor="gaba_a"` still FAILS, `"gaba_b"` PASSES). **An honest negative IS a valid deliverable** (it maps a substrate limit) — do not hide a partial.
3. **Commit + push BOTH remotes** (origin + gitea) — the protected edit + the probe extension + the de-risk finding. Only `git add` the reviewed files (NOT the pre-existing unrelated `raw/*.json` working-tree noise).
4. **Report to the owner** (the byte-review verdict + the de-risk table). If the de-risk PASSES → the nav 6-seed regression gate (flagship A+E+G v2.5 + `--spiking-snc --enable-neural-critic` + GABA_B critic→SNc; summed reward ≥ Stage A; an honest negative is still a deliverable). THEN (only once nav is fully biologized + audited) the parked single-instance unification.

Optional pre-registered follow-on (owner-surfaced, AFTER GABA_B lands): the **conduction-delay** protected edit (the shortcut-audit's #2; cheap; gates temporal/phase coding) as the next proactive fidelity investment; + 2 minor naming-polish docstring rewordings. Do NOT start these while GABA_B is in flight.

## >>> EXACT NEXT (2026-06-08 LATEST — superseded by the GABA_B BUILD block above) <<<
