...
**Phase 1 (B cleanup) DE-RISK GO** (commit d4cd1521, `2026-06-05-phase1-tpam-cleanup-derisk-GO.md`): the biology-grounded TPAM cleanup (complex Hopfield / CA3 pattern completion; W=SS* IS the bridge's complex-synapse matvec) == the numpy argmax at D≥256 multi-seed (27/27; D=128 21/27 = dense-phasor capacity wall, lifted by the D dial — same as the 2-attribute lift). **NEXT = Phase 1 ON-BRIDGE INTEGRATION** (route a: spiking matched-filter cleanup — a concept-neuron bank, `rec` through conj(code) complex synapses → |correlation| → WTA; the NEF-cleanup analogue on phasors, simplest, IS the argmax) → re-validate the agent's full suite at parity → commit. THEN Phase 2 (C-A phasor weight-store) → Phase 3 (A grounded codes) → Phase 4 (D substrate assoc graph). Each phase: de-risk parity GATE → opt-in integration → agent full suite at parity → commit BOTH remotes; honest finding (GO or BOUNDARY). Watchdog: continue the cheat-conversion per the plan.
