...
**NEXT (the original cheat-D integration target): wire this LEARNED recurrent into the conversational agent's dlPFC association memory** — replace the set-from-Python `c2d` edges (the SET-not-LEARNED residual) with this learned sparse recurrent, so the agent's dialogue planning spreads over a substrate-LEARNED association graph. Watchdog: integrate the learned heteroassociative into the agent's dlPFC per cheat D.
