...
3. **NAV STAGE-B NEURAL-CRITIC — 2-SEED SMOKE NEGATIVE (honest; committed). DIAGNOSIS NEXT (diagnostic subagent dispatched).** The BRAIN-BASED-ONLY nav completion BUILT (runner-side only, **sim/ byte-empty**, reviewed sound, committed): `--enable-neural-critic` adds a GABAergic `striosome_value` critic (cortex_it-driven, plastic `value_input`, SNc-δ-trained), routes `striosome_value→snc` `receptor="gaba_b"` + `cfg.enable_gabab=True`, DROPS the host `−snc_value_gain·_V_scaffold` (Stage-A byte-unchanged when off). **SMOKE seed 42 FAILED decisively:** SNc fires **0 Hz** (Stage A = 7.16 Hz), critic frozen (2.990→2.990, never fires in logged windows), nav collapses (sumFinalQ **133** vs flagship ~2.6, never reaches goal). Root: SNc silenced → dopamine δ dead → no three-factor learning → nav dies. PARADOX = neural `_I_snc`(220+reward) is LARGER than Stage-A (220+reward−value) yet fires 0 → something adds STRONG hyperpolarization. **PRIME SUSPECT:** the critic fires during regular nav steps (NOT the reward-window-only `striov_rate_log`, so its 0 is misleading), pumping the SLOW GABA_B conductance (τ=150ms, E_K=−90mV) on the SNc → sustained silencing. The CPU de-risk missed this (clean train-then-test Pavlovian, not a continuous free-running loop with persistent cortical drive to the critic). Finding: `2026-06-08-neural-critic-nav-smoke-NEGATIVE.md`. **NOT** proceeding to the 6-seed gate (smoke gate failed). **DIAGNOSTIC SUBAGENT (runner-side/probe only, NO sim/ edits):** instrument a short neural-critic run logging cortex_it/striosome_value/snc firing EVERY step + the SNc GABA_B conductance + a NaN check, vs `cfg.enable_gabab` forced off (critic on) vs Stage A → localize the SNc silence → propose a runner-side fix (faster GABA_B τ for the nav loop / weaker `gabab_propagation_strength` / gate the critic→SNc GABA_B to the reward window / rebalance SNc tonic) OR an honest "slow GABA_B incompatible with the continuous nav loop" deliverable. **CONTROLLER ON ITS RETURN:** verify sim/ untouched, review root cause + fix, re-smoke, then (only if it passes) the 6-seed A/B gate. The GABA_B `sim/` edit itself is UNAFFECTED (its CPU de-risk PASSED, byte-identical-when-off). THEN (nav fully biologized + audited) the parked single-instance unification.
