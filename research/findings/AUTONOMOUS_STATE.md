...
>>> PRODUCTION BUILD STARTED (2026-06-02): the GPU visual-text recognition bridge is BUILT + running (research/runners/text_visual_grounding.py). Constructs the full visual hierarchy retina(configurable, un-capped per owner)->V1_simple->V1_complex->V2->IT on CuPy/RTX3090 with SCALED Gabor V1 (freqs/sigmas/RF scaled by retina/32 for letter scale). At retina 64: 49,472 neurons, 13.7M synapses, builds in ~80s. Reuses the g11 visual region/pathway pattern + the standard region-framework bridge construction + sim.visual_cortex Gabor. Renders word-as-pixels -> image_to_retina_drive -> retina region -> steps -> reads V1/V2/IT firing. Scaled-Gabor cheap probe: retina 64 ~doubles 32 recognition (0.18->0.37) but V1-simple+linear still ceiling'd ~0.37 -> the full V1->V2->IT hierarchy (which THIS bridge has) is needed. >>> EXACT NEXT (step 2): verify V1/V2/IT fire to text (b7m6z7xsk running); then STDP-train an IT word-recognition readout (open visual_cortex_v2/it gates, drive words, let V2/IT learn word-form features) + wire IT->concept pools (replacing set_token_drive orthogonal lang_input) -> EARNED visual word recognition. This is the input-side-fidelity fix realized on the GPU. Both remotes; biology-faithful; no shortcuts. (Owner flagged stalling twice -- production build now underway with continuous concrete GPU progress, not promises.)
