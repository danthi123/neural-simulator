...
**EXACT NEXT CONCRETE ACTION for the watchdog (DEFAULT if owner doesn't steer within a cycle or two):** begin the interneuron-diversity de-risk — the **matched-topographic-FS toy test** (the cheap-first of the one remaining on-bridge lever). Build the toy decorrelation bridge (16 concepts, as `_A_spiking_decorrelation`) but REPLACE the single all-to-all FS pool with MATCHED TOPOGRAPHIC FS: each FS_k connects (BOTH it→fs AND fs→it) to a LOCAL IT window `[k·stride : k·stride+W]` (overlapping windows tiling all IT), so inhibition is LOCAL/SPECIFIC not global — the realizable proxy for Földiák's pairwise W_ik. Realize via post-build `cp_connections` topographic masking (zero the non-local it↔fs entries after build) OR K windowed FS sub-regions. **GATE:** does the worst-pair MAX coherence drop below the single-pool ~0.9 (toy)? If YES → the diversity direction works → justify the full build (then scale to the 320 functional gate, target > 76.9% toward ZCA). If NO → genuine multi-TYPE (PV fast-perisomatic / SST slow-dendritic / VIP disinhibition) dynamics diversity is needed (not just topographic) — a documented deeper boundary. NOTE: random-sparse it↔fs does NOT work (must be MATCHED — FS_k must receive from AND inhibit the SAME window). If owner steers to (2) dataset or (3) pivot, follow that instead. Watchdog: this is a clean fresh-context build — do NOT jam mid-build; start it as the cycle's task.
