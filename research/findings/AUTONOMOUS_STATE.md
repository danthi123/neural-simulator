...
## >>> EXACT NEXT (2026-06-08 EVENING — read THIS FIRST; supersedes every block below) <<<
...
**WEBAPP RUNNING** (`bf4h6sffm`, `uvicorn webapp.server:app --port 8765`, NO --reload) → http://localhost:8765/ for owner monitoring. Added a `rank2_generalize2` preset to `webapp/server.py` (UNCOMMITTED — commit after the webapp is verified up).
