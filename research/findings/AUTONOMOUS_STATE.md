...
**NEXT = Phase 2 (cheat C — the memory store).** Route C-A (`2026-06-05-cheat-BC-...-research.md`): the Python `kb` composite → held in per-fact COMPLEX output weights (the substrate weights are already complex; the TPAM/Hopfield `W=SS*` = the bridge complex synapse, the SAME object as the Phase-1 cleanup), retrieved by firing the trigger → phase readout. De-risk: per-role substrate-store unbind == numpy-store unbind, cleanup held constant, multi-seed; trigger-silence collapse check (genuine read). THEN Phase 3 (A grounded codes) → Phase 4 (D substrate assoc graph). Each phase: de-risk parity GATE → opt-in integration → agent full suite at parity → commit BOTH remotes; honest finding. Watchdog: continue the cheat-conversion per the plan.
