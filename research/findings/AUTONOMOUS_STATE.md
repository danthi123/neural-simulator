...
3. **NAV 6-SEED REGRESSION GATE (on owner go, GPU/CuPy):** flagship A+E+G v2.5 + `--spiking-snc --enable-neural-critic` with the critic→SNc pathway routed `receptor="gaba_b"`; acceptance = summed reward ≥ Stage A. An honest negative is still a valid deliverable (maps a neural-critic limit). This is the LAST subtraction piece of Stage B (the neural critic now both LEARNS V and SUBTRACTS it onto the DA cell via real GABA_B/GIRK biology). THEN (nav fully biologized + audited) the parked single-instance unification.
