"""Direction 6 per-bridge builder wrappers (5 functions, CPU-only spec).

Each function builds a fresh SimulationBridge with ONE category's V=32
vocab on the validated v14/v16 bio_brain_regions concept-pool architecture.
This is the 2x-vocab extension of Direction 4 (V=16 per bridge) per the
pillar n=108 D4 dedicated-pool result, scaling to V=32 per bridge x
5 bridges = 160 cross-bridge concepts.
...
- research.runners.text_minimal_isolation.build_biological_brain_regions
  (the protected builder; this wrapper passes each bridge's V=32 category
...
Bridge -> pool-kind mapping (same as D4):
...
- Bridge-specific seed offsets MANDATORY (analog of D5 c4e18f2 + D4 fix);
  100k spacing per bridge to avoid byte-identical weight initialization
  across the 5 bridges.
"""
...
# + D4 _DIRECTION_4_BRIDGE_LABEL_SEED_OFFSETS). Each bridge needs UNIQUE
# random initialization of lang_input -> pool weight matrices; without
# offsets all 5 bridges share identical base_seed (e.g. 42) and the
# protected build_biological_brain_regions / SimulationBridge construction
# produces byte-identical pool weights across bridges. Since
...
_DIRECTION_6_BRIDGE_LABEL_SEED_OFFSETS: dict = {
    "A_nouns":      0,
    "B_verbs":      100000,
    "C_adj":        200000,
    "D_spatial":    300000,
...
def _build_bridge_core(
    ...
    if n_active_slots != 1:
        raise ValueError(
            "Direction 6 per-bridge builder requires exactly ONE pool "
            "slot non-None per call; got " + str(n_active_slots)
        )

    # CRITICAL BUG FIX (analog of D5 c4e18f2 + D4 fix 2026-05-26):
    # derive a bridge-specific seed from (base_seed, label) so each of
    # the 5 bridges initializes its lang_input -> pool weight matrices
    ...
    # probe operate on duplicate inputs.
    _bridge_seed_offset = _DIRECTION_6_BRIDGE_LABEL_SEED_OFFSETS.get(
        label, 0,
    )
    ...
    if verbose:
        ...
        print("[BUILD-D6-" + label + "] V=32 bridge: "
...
def build_direction_6_bridge_A_nouns(
    seed: int,
    ...
    """Bridge A: V=32 nouns on bio_brain_regions concept-pool substrate.

    32 noun pools (D4's 16 + 16 D6 extension) loaded via the dedicated
    ...
    return _build_bridge_core(
        ...
        noun_pool_names=DIRECTION_6_NOUN_NAMES,
        verbose=verbose, label="A_nouns",
    )


def build_direction_6_bridge_B_verbs(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge B: V=32 verbs on bio_brain_regions concept-pool substrate.

    32 verb pools (D4's 16 + 16 D6 extension) loaded via the dedicated
    verb_pool_names slot of the protected builder.
    """
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        verb_pool_names=DIRECTION_6_VERB_NAMES,
        verbose=verbose, label="B_verbs",
    )


def build_direction_6_bridge_C_adj(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge C: V=32 adjectives on bio_brain_regions concept-pool substrate.

    32 adjective pools (D4's 16 + 16 D6 extension) loaded via the dedicated
    adjective_pool_names slot of the protected builder.
    """
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        adjective_pool_names=DIRECTION_6_ADJECTIVE_NAMES,
        verbose=verbose, label="C_adj",
    )


def build_direction_6_bridge_D_spatial(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge D: V=32 spatial words on bio_brain_regions concept-pool
    substrate.

    32 spatial words (D4's 16 + 16 D6 extension covering front/back/
    here/there + above/below/inside/outside + behind/beside/between/across +
    along/around/beyond/within) loaded via the noun_pool_names slot of
    ...
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        noun_pool_names=DIRECTION_6_SPATIAL_NAMES,
        verbose=verbose, label="D_spatial",
    )


def build_direction_6_bridge_E_functional(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge E: V=32 functional words on bio_brain_regions concept-pool
    substrate.

    32 functional words (D4's 16 + 16 D6 extension covering
    we/they/it/me/us/them + who/where/why/how + but/if/then/now/is/was)
    loaded via the noun_pool_names slot of the protected builder. Same
    rationale as Bridge D (category-agnostic pool architecture).
    """
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        noun_pool_names=DIRECTION_6_FUNCTIONAL_NAMES,
...
DIRECTION_6_BRIDGE_BUILDERS = {
    "A_nouns": build_direction_6_bridge_A_nouns,
    "B_verbs": build_direction_6_bridge_B_verbs,
    "C_adj": build_direction_6_bridge_C_adj,
    "D_spatial": build_direction_6_bridge_D_spatial,
    "E_functional": build_direction_6_bridge_E_functional,
}
