"""Direction 7 Task 4: cross-bridge parallel-matching mode-unification
probe on the bio_brain_regions 5-bridge ensemble (320 cross-bridge
concepts = 5 categories x V=64; 2x-vocab extension of Direction 4 V=80).
...
substrate has 5 x V=64 = 320 concepts (vs D6's 5 x V=32 = 160).

CPU-only by design (operates on cached per-bridge activity .npz files
written by direction_7_5bridge_runner.py, Task 5). Does NOT import
...
5. Compute per-load (OB, OI) accuracy; emit verdict via the frozen
   direction_7_verdict module.

PRE-REGISTERED reading (frozen by direction_7_verdict module):
- DIRECTION_7_PASS: multi-seed-mean OB AND OI both clear 0.80 at every
  load in {2, 3, 5}.
- DIRECTION_7_PARTIAL: some cells above bar but not all.
- DIRECTION_7_NEGATIVE: NO load-cell on EITHER readout clears the bar.
- DIRECTION_7_VOID_MALFORMED: instrument-validity failure (per the
...
# Direction 7 modules (this arc's net-new code).
from research.findings.raw.direction_7_vocab_spec import (
    ...
    DIRECTION_7_BRIDGE_CATALOG,
    DIRECTION_7_TOTAL,
)
from research.findings.raw.direction_7_verdict import (
    compute_verdict,
    _DIRECTION_7_OB_MIN,
    _DIRECTION_7_OI_MIN,
    _DIRECTION_7_LOADS,
    _DIRECTION_7_MIN_SEEDS,
    DIRECTION_7_VOID_MALFORMED,
)
...
# -----------------------------------------------------------------------
# Constants (mirroring n=95 + OPTION 3 V=16 + Direction 3 V=64 +
# Direction 4 5-bridge patterns).
# -----------------------------------------------------------------------
SEEDS = [42, 43, 44]
LOADS = list(_DIRECTION_7_LOADS)
BAR_OB = _DIRECTION_7_OB_MIN
BAR_OI = _DIRECTION_7_OI_MIN
N_GAMMA_SLOTS = 7

# The frozen iteration order over the 5 bridges (matches
# DIRECTION_7_ALL_WORDS order in direction_7_vocab_spec). The cross-
# bridge probe takes the union in this exact order so word_idx in
# decoder = position in DIRECTION_7_ALL_WORDS.
BRIDGES: Tuple[str, ...] = (
...
_PER_BRIDGE_WORDS: Dict[str, List[str]] = {
    "A_nouns": DIRECTION_7_BRIDGE_A_WORDS,
    "B_verbs": DIRECTION_7_BRIDGE_B_WORDS,
    "C_adj": DIRECTION_7_BRIDGE_C_WORDS,
    "D_spatial": DIRECTION_7_BRIDGE_D_WORDS,
...
CACHE_DIR: str = os.path.join(_HERE, "direction_7_cache")
...
def _load_per_bridge_activity(
    ...
    if not os.path.exists(cache_p):
        raise FileNotFoundError(
            "Direction 7 activity cache missing: " + cache_p
    ...
    for w in words:
        if str(w) not in data.files:
            raise ValueError(
                "Direction 7 activity cache " + cache_p
...
def derive_global_grounded_symbols(
    ...
    Returns:
        all_words: list of (bridge_name, word) tuples in cross-bridge
                    union order (matches DIRECTION_7_ALL_WORDS).
...
# substrate (5 bridges x V=64 = 320; vs D6's 5 bridges x V=32 = 160).
# -----------------------------------------------------------------------
def run_one_seed_cross_bridge(
    ...
    Args:
        ...
        loads: composition loads; default _DIRECTION_7_LOADS.
    ...
    Returns:
        per_load: {load: {"order_bearing_accuracy": float,
                          ...
                          "n_trials": int}}
        V: total concepts in the union (=320 if all bridges have V=64)
...
def run_cross_bridge_probe(
    ...
    frozen Direction 7 verdict.

    Args:
        seeds: list of seeds to probe; default SEEDS=[42, 43, 44].
        loads: composition loads; default _DIRECTION_7_LOADS=(2, 3, 5).
        n_trials: trials per load; default N_TRIALS=200.
    ...
    Returns:
        Result dict with shape:
        {
          ...
          "V": 320, "n_bridges": 5,
          "per_seed": [{"seed": N, "V": 320, "per_load": {...},
                         "verdict_entry": {"L=2": {"OB":, "OI":}, ...}}, ...],
          "aggregate": {load: {"order_bearing_mean":, ...}, ...},
          "verdict": "DIRECTION_7_PASS" | "DIRECTION_7_PARTIAL" |
                      "DIRECTION_7_NEGATIVE" | "DIRECTION_7_VOID_MALFORMED",
    ...
    for seed in seeds:
        ...
        seed_results.append({
            ...
            # direction_7_verdict.compute_verdict).
            "verdict_entry": {
                ("L=" + str(l)): {
    ...
    return {
        ...
        "min_seeds": _DIRECTION_7_MIN_SEEDS,
        "V": DIRECTION_7_TOTAL,
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 7 cross-bridge parallel-matching probe "
                    "on bio_brain_regions 5-bridge ensemble "
                    "(V=64 per bridge x 5 = 320 cross-bridge concepts; "
    ...
    ap.add_argument(
        ...
        help="override per-bridge activity cache directory (defaults to "
             "research/findings/raw/direction_7_cache)",
    )
    ap.add_argument(
        "--out", default=None,
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_7_cross_bridge_" + tag + ".json",
    )
