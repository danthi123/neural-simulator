...
Empirically measures the actual cosine geometry of D6 V=160 cached
per-concept activity vectors to characterise WHY the FHRR algebra
capacity-ratio prediction was DECISIVELY SHATTERED at pillar n=109.
...
def main():
    ...
    print("  Reading cached D6 V=160 production activity (5 bridges x 3 seeds)")
    ...
    if diff_mc_agg.get("n_cells", 0) > 0:
        ...
        if abs(diff_mc_mean) < 0.10 and diff_mc_abs < 0.30:
            print("  - Mean-centred different-concept cosine NEAR-ORTHOGONAL "
                  "(mean %.4f, abs %.4f). Empirically validates n=109 SHATTER."
    ...
    out_path = os.path.join(_HERE, "direction_6_geometry_diagnostic.json")
    out = {
