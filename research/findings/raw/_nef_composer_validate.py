"""(A integration validation) Production no-regression confirm: the opt-in spiking NEF cleanup composer
(`CoreSimComposer(enable_spiking_cleanup=True)`) must answer the capability matrix IDENTICALLY to the numpy
...
Run AFTER the integration lands (it uses the enable_spiking_cleanup flag):
  python -u -m research.findings.raw._nef_composer_validate --proj-dim 2048 --seeds 42 43 44 \
      --out research/findings/raw/_nef_composer_validate.json
"""
...
def main():
    ...
    print(f"\n[VERDICT] spiking-cleanup composer vs numpy: {total_match}/{total} match across seeds -> {verdict}")
    if args.out:
