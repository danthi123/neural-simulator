"""Direction 8 Task 5: 5-bridge training runner (CONTROLLER-ONLY;
GPU-bound). V=128 per bridge x 5 = 640 cross-bridge concepts.

Trains 5 bio_brain_regions bridges (one per vocab category: noun /
verb / adjective / spatial / functional, V=128 each) across 3 seeds
[42, 43, 44] = 15 bridge trainings total. Captures per-bridge per-word
activity vectors across the bridge's V=128 category-pool neuron union;
writes per-seed-per-bridge .npz cache. After all bridges train,
optionally invokes direction_8_cross_bridge_probe (Task 4) inline to
emit the frozen Direction 8 verdict.

D7 V=320 vocab matches the Direction M G.20 sparse production
deliverable byte-for-byte; this is the biology-faithful counterpart of
the user-facing G.20 sparse 320-concept chat capability at identical
vocabulary.

ETA (per pillar n=109 D6 V=160 production timing + V doubling +
n_lang_input doubling to preserve per-cue training signal footprint):
- Full scale (n_lang_input=4096, n_per_pool=200, n_events=200):
  ~110-130 min/bridge x 5 bridges x 3 seeds = ~27-32 hr GPU
- Smoke scale (n_lang_input=2048, n_per_pool=100, n_events=50):
  ~7-10 min/bridge x 5 bridges x 3 seeds = ~2-3 hr GPU
...
Pre-registered verdict (frozen): research/findings/raw/direction_8_verdict.py
Pre-registered cross-bridge probe primitive (reuse-by-import,
byte-unchanged): research/findings/raw/direction_8_cross_bridge_probe.py
...
- All npz files use numeric-only arrays (safety: object arrays rejected).
...
is local to this runner (mirrors Direction 6's
_apply_per_bridge_topographic_bias byte-pattern; only the per-bridge
V=128 vocab differs).

STRIDE NOTE (deliberate, NOT tuned):
- At V=128 + n_lang_input=4096, stride = 4096/64 = 64 neurons per cue
  band. orthogonal_drive_pattern requires n_active = sparsity*n_lang
  <= stride. sparsity 0.015 -> n_active = 61 < 64 (clean margin;
  preserves D6 V=32 production n_active=61 byte-for-byte across the
  V doubling -- per-cue training signal footprint held constant).
- At V=128 + n_lang_input=2048 (smoke), stride = 32. sparsity 0.01 ->
  n_active = 20 < 32 (matches D6 smoke n_active=20 byte-for-byte).
"""
...
# Direction 8 modules (this arc's net-new code).
from research.findings.raw.direction_8_vocab_spec import (
    ...
    DIRECTION_8_NOUN_NAMES,
    DIRECTION_8_VERB_NAMES,
    DIRECTION_8_ADJECTIVE_NAMES,
    DIRECTION_8_SPATIAL_NAMES,
    DIRECTION_8_FUNCTIONAL_NAMES,
    DIRECTION_8_BRIDGE_CATALOG,
    DIRECTION_8_TOTAL,
)
from research.findings.raw.direction_8_bridge_builder import (
    DIRECTION_8_BRIDGE_BUILDERS,
)
from research.findings.raw.direction_8_verdict import (
    _DIRECTION_8_OB_MIN, _DIRECTION_8_OI_MIN,
    _DIRECTION_8_LOADS, _DIRECTION_8_MIN_SEEDS,
)

# Reuse-by-import only (validated training + capture primitives;
# pillar n=96 OPTION 3 + Direction 3 V=128 + Direction 4 5-bridge patterns).
...
# Mirrors Direction 7 5-bridge runner byte-pattern; the only mechanical
# differences:
# - per-bridge V=128 instead of V=64 (doubled vocab)
# - n_lang_input doubled (4096 -> 8192 full; 2048 -> 4096 smoke) so the
#   orthogonal_drive_pattern stride matches the V=128 cue band count
#   (stride = n_lang_input / V_per_bridge; constraint n_active <= stride)
# - sparsity halved to preserve per-cue n_active=61 footprint exactly
#   the same as D6 V=32 / D7 V=64 production (no inflation, no shrinkage)
# -----------------------------------------------------------------------
FULL_N_LANG_INPUT = 8192
FULL_N_PER_POOL = 200
FULL_N_FS_PER_POOL = 24
FULL_N_TRAIN_EVENTS = 200
# At V=128 + n_lang_input=8192, stride = 8192/128 = 64 neurons per cue
# band. orthogonal_drive_pattern requires n_active = sparsity*n_lang
# <= stride. sparsity 0.0075 -> n_active = 61 < 64 (clean margin).
# Preserves D6 V=32 / D7 V=64 production n_active=61 byte-for-byte
# across the V doubling -- per-cue training signal footprint constant.
FULL_SPARSITY = 0.0075
FULL_M_OBS = 16  # observations per concept (matches D6/D7 production)
FULL_TOPOGRAPHIC_FACTOR = 3.0
FULL_OFF_TARGET_FACTOR = 0.3

SMOKE_N_LANG_INPUT = 4096
SMOKE_N_PER_POOL = 100
SMOKE_N_FS_PER_POOL = 12
SMOKE_N_TRAIN_EVENTS = 50
# At V=128 + n_lang_input=4096, stride = 32. sparsity 0.005 -> n_active=20
# < 32 (matches D6/D7 smoke n_active=20 byte-for-byte).
SMOKE_SPARSITY = 0.005
SMOKE_M_OBS = 8

SEEDS = [42, 43, 44]
LOADS = list(_DIRECTION_8_LOADS)
BAR_OB = _DIRECTION_8_OB_MIN
BAR_OI = _DIRECTION_8_OI_MIN

# Per-bridge per-seed cache directory.
CACHE_DIR = os.path.join(_HERE, "direction_8_cache")
os.makedirs(CACHE_DIR, exist_ok=True)

# Per-bridge word + target-pool map (each bridge has its own V=128 vocab
# pointing to its own pool-name prefix).
_PER_BRIDGE_WORD_LISTS: Dict[str, List[str]] = {
    "A_nouns": DIRECTION_8_BRIDGE_A_WORDS,
    "B_verbs": DIRECTION_8_BRIDGE_B_WORDS,
    "C_adj": DIRECTION_8_BRIDGE_C_WORDS,
    "D_spatial": DIRECTION_8_BRIDGE_D_WORDS,
    "E_functional": DIRECTION_8_BRIDGE_E_WORDS,
}
...
def _per_bridge_target_pool_map(bridge_name: str) -> Dict[str, str]:
    ...
    """
    spec = DIRECTION_8_BRIDGE_CATALOG[bridge_name]
    ...
    if slot == "noun_pool_names":
        ...
        raise ValueError(
            "Direction 8 bridge_catalog has unknown builder_slot '"
...
def _per_bridge_pool_region_names(bridge_name: str) -> List[str]:
    """Per-bridge list of V=128 concept-pool region names (in word-order
...
# -----------------------------------------------------------------------
# Per-bridge topographic-bias helper (V=128 per-bridge; mirrors
...
def _apply_per_bridge_topographic_bias(
    ...
    Direction 3 V=128 + Direction 4 5-bridge: each edge is "target" for
    exactly one word; an edge that is "off-target" for at least one word
    gets dampened once.

    Bridge has only V=128 category pools (its category) in the cross-
    bridge probe scope; motor pools also exist structurally but are
    untouched by the per-bridge bias (motor isn't part of the V=128
    ...
    if len(lang_input_indices) != n_lang_input:
        raise ValueError(
            "Direction 8 per-bridge topographic bias: bridge "
    ...
    # All V=128 output pools for THIS bridge (motor pools deliberately
    ...
    # Pass 2 forward: off-target dampen (within-bridge V=128 pools only)
    dampened_edges = set()
    for word in words:
        target_region = target_map[word]
        global_active = word_to_active[word]
...
def _build_and_train(
    ...
    reset_steps_per_event: int = 50,
):
    """Build + train one (bridge, seed) cell. Returns the trained bridge
    + the bridge's V=128 word list + word_to_idx + sparsity + n_lang_input."""
    ...
    if n_words != 128:
        raise ValueError(
            "Direction 8 bridge " + bridge_name + " expected V=128 words; "
            "got " + str(n_words)
        )
    word_to_idx = {w: i for i, w in enumerate(words)}

    builder = DIRECTION_8_BRIDGE_BUILDERS[bridge_name]
    ...
    if verbose:
        print(
            "  [" + bridge_name + "/seed " + str(seed)
            + "] training V=128 substrate (" + str(n_words)
    ...
    for ei, w in enumerate(schedule):
        train_word_to_pool(
            ...
            word_to_idx=word_to_idx,
            verbose=False,
            stim_steps_per_event=stim_steps_per_event,
            reset_steps=reset_steps_per_event,
        )
...
# -----------------------------------------------------------------------
# Per-bridge activity capture (V=128 per-bridge category pools only).
# -----------------------------------------------------------------------
def _capture_concept_pool_activity(
    ...
    """Per-neuron activity vectors across the V=128 per-bridge category
    pool union. Same shape / window / drive_max_pA as the OPTION 3 V=16
    probe + Direction 3 V=128 runner + Direction 4 5-bridge runner."""
    from sim.backend import get_backend
    cp, _ = get_backend()
    from sim.text_embeddings import orthogonal_drive_pattern
...
def train_one_bridge(
    ...
    bridge, words, word_to_idx, sparsity, n_lang_input = _build_and_train(
        ...
        use_fp16=use_fp16,
        stim_steps_per_event=stim_steps_per_event,
        reset_steps_per_event=reset_steps_per_event,
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 8 5-bridge multi-seed training + cross-"
                    "bridge probe runner (CONTROLLER-ONLY; GPU-bound; "
                    "V=128 per bridge x 5 = 640 cross-bridge concepts)",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help="train only; do NOT invoke direction_8_cross_bridge_probe "
    ...
    # SPEEDUP EXPERIMENT FLAGS (D8). Defaults match D7 byte-for-byte:
    # use_fp16=False, stim_steps_per_event=100, reset_steps=50. Override
    # to test the optimization combo on smoke before committing to
    # production. Per the launcher caveat in
    # direction_7_parallel_production.ps1, multi-process parallelism does
    # NOT speed up on Windows WDDM; per-event compute reduction is the
    # actual lever for sequential single-process throughput.
    ap.add_argument(
        "--use-fp16", action="store_true",
        help="enable cfg.fp16_synapse_state (halves STDP memory bandwidth; "
             "~10-25%% wall reduction; ~0.5%% noise per 1000 events per "
             "sim/config.py; validated <1mV drift but accumulated effects "
             "on cross-bridge composition unmeasured -- A/B verify on "
             "smoke before relying on production)",
    )
    ap.add_argument(
        "--stim-steps-per-event", type=int, default=100,
        help="per-event training stim sub-step count passed through to "
             "train_word_to_pool (default 100 = D6/D7 byte-equivalent). "
             "Halving to 50 gives ~1.5-2x wall reduction; risk: weakens "
             "STDP signal -> verify training quality on smoke first.",
    )
    ap.add_argument(
        "--reset-steps", type=int, default=50,
        help="per-event reset sub-step count (default 50 = D6/D7 byte-"
             "equivalent). NMDA tau ~150ms; CLAUDE.md notes reset_steps=50 "
             "is 25ms which is OK but reset_steps=300 (~150ms) lets NMDA "
             "fully decay. Halving below 50 not recommended.",
    )
    args = ap.parse_args()
    smoke = bool(args.smoke)
    use_fp16 = bool(args.use_fp16)
    stim_steps_per_event = int(args.stim_steps_per_event)
    reset_steps_per_event = int(args.reset_steps)

    xp, backend_name = get_backend()
    gpu = is_gpu_backend()
    print(
        "=== Direction 8 5-bridge multi-seed training runner ===",
        flush=True,
    )
    print(
    ...
    print(
        "  V_per_bridge=128, n_bridges=5, V_total=" + str(DIRECTION_8_TOTAL),
        flush=True,
    )
    print(
        "  speedup_flags: use_fp16=" + str(use_fp16)
        + ", stim_steps_per_event=" + str(stim_steps_per_event)
        + ", reset_steps=" + str(reset_steps_per_event)
        + " (D6/D7 baseline: fp16=False, stim=100, reset=50)",
        flush=True,
    )
    if smoke:
        print(
    ...
    print(
        ...
        + "; min_seeds=" + str(_DIRECTION_8_MIN_SEEDS),
        flush=True,
    )
    ...
    if args.bridges is not None:
        ...
        for b in bridges:
            if b not in DIRECTION_8_BRIDGE_BUILDERS:
                raise ValueError(
                    "Unknown Direction 8 bridge name: " + str(b)
                    + " (expected one of "
                    + str(list(DIRECTION_8_BRIDGE_BUILDERS.keys())) + ")"
                )
    else:
        bridges = list(DIRECTION_8_BRIDGE_BUILDERS.keys())
    ...
    if not args.skip_probe:
        ...
        # verdict).
        all_bridges_trained = (
            set(bridges) == set(DIRECTION_8_BRIDGE_BUILDERS.keys())
            and len(seeds) >= _DIRECTION_8_MIN_SEEDS
        )
        if all_bridges_trained:
            from research.findings.raw.direction_8_cross_bridge_probe import (
                run_cross_bridge_probe,
            )
            tag = "smoke" if smoke else "full"
            print(
                "\n=== Invoking Direction 8 cross-bridge probe (Task 4) ===",
                flush=True,
            )
            ...
            print(
                ...
                "trained and >= " + str(_DIRECTION_8_MIN_SEEDS)
    ...
    out = {
        ...
        "min_seeds": _DIRECTION_8_MIN_SEEDS,
        "V_total": DIRECTION_8_TOTAL,
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_8_5bridge_" + tag + ".json",
    )
