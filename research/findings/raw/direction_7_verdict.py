"""Direction 7 verdict module - pre-registered frozen-threshold scoring.

Computes the Direction 7 cross-bridge-on-bio_brain_regions test verdict
(2x-vocab extension of Direction 6; V=64 per bridge x 5 bridges = 320
...
- _DIRECTION_7_OB_MIN = 0.80     (order-bearing multi-seed-mean min;
                                   same as pillars n=93+ + Directions Q,
                                   3, 4, 6 FROZEN 0.80 multi-seed strict bar)
- _DIRECTION_7_OI_MIN = 0.80     (order-invariant multi-seed-mean min)
- _DIRECTION_7_LOADS  = [2, 3, 5] (pre-registered load ladder; same as
                                   the validated OPTION 3 V=16 probe,
                                   Direction 3 V=32 probe, Direction 4
                                   V=80 probe, Direction 6 V=160 probe)
- _DIRECTION_7_MIN_SEEDS = 3     (minimum number of seeds for verdict)

Verdict tags (distinct categories):
- DIRECTION_7_PASS           : multi-seed-mean OB AND OI both clear the
                                0.80 bar at EVERY load in the pre-
                                registered ladder. Cross-bridge
                                parallel-matching mode-unification
                                extends to 5x64=320 cross-bridge concepts
...
# caught by:
#   tests/test_direction_7_grounding.py::test_direction_7_verdict_thresholds_frozen
# -------------------------------------------------------------------
_DIRECTION_7_OB_MIN: float = 0.80
_DIRECTION_7_OI_MIN: float = 0.80
_DIRECTION_7_LOADS: Tuple[int, ...] = (2, 3, 5)
_DIRECTION_7_MIN_SEEDS: int = 3
...
# without typos).
# -------------------------------------------------------------------
DIRECTION_7_PASS: str = "DIRECTION_7_PASS"
DIRECTION_7_PARTIAL: str = "DIRECTION_7_PARTIAL"
DIRECTION_7_NEGATIVE: str = "DIRECTION_7_NEGATIVE"
DIRECTION_7_VOID_MALFORMED: str = "DIRECTION_7_VOID_MALFORMED"
...
def compute_verdict(
    ...
    """Compute the Direction 7 verdict tag from recorded per-seed
    ...
    Returns:
        One of the DIRECTION_7_* tag constants defined at module top.
    ...
    if per_seed is None:
        return DIRECTION_7_VOID_MALFORMED

    if not isinstance(per_seed, (list, tuple)):
        return DIRECTION_7_VOID_MALFORMED

    if len(per_seed) < _DIRECTION_7_MIN_SEEDS:
        # Multi-seed verdict requires >= _DIRECTION_7_MIN_SEEDS seeds.
    ...
    for entry in per_seed:
        if not _is_valid_seed_entry(entry, _DIRECTION_7_LOADS):
            return DIRECTION_7_VOID_MALFORMED

    # --- Compute multi-seed means ---
    means = _compute_multi_seed_means(
        list(per_seed), _DIRECTION_7_LOADS,
    )
    ...
    all_ob_pass = True
    all_oi_pass = True
    for load in _DIRECTION_7_LOADS:
        ob_mean = means[load]["OB"]
        oi_mean = means[load]["OI"]
        n_cells_total += 2
        if ob_mean >= _DIRECTION_7_OB_MIN:
            n_cells_passed += 1
        else:
            all_ob_pass = False
        if oi_mean >= _DIRECTION_7_OI_MIN:
            n_cells_passed += 1
        else:
            all_oi_pass = False
