"""Direction 8 per-bridge builder wrappers (5 functions, CPU-only spec).

Each function builds a fresh SimulationBridge with ONE category's V=128
...
pillar n=109 D6 dedicated-pool result, scaling to V=128 per bridge x
...
- research.runners.text_minimal_isolation.build_biological_brain_regions
  (the protected builder; this wrapper passes each bridge's V=128 category
...
_DIRECTION_8_BRIDGE_LABEL_SEED_OFFSETS: dict = {
    "A_nouns":      0,
    "B_verbs":      100000,
    "C_adj":        200000,
    "D_spatial":    300000,
...
def _build_bridge_core(
    ...
    if n_active_slots != 1:
        raise ValueError(
            "Direction 8 per-bridge builder requires exactly ONE pool "
    ...
    # inputs.
    _bridge_seed_offset = _DIRECTION_8_BRIDGE_LABEL_SEED_OFFSETS.get(
        label, 0,
    )
    ...
    # SPEEDUP EXPERIMENT (D8): fp16 eligibility trace state, opt-in.
    # CLAUDE.md / sim/config.py documents ~0.5% noise per 1000 events;
    # acceptable for our task per the validated <1mV drift; needs
    # smoke A/B vs fp32 baseline before committing to production.
    cfg.fp16_synapse_state = bool(use_fp16)
    ...
    if verbose:
        ...
        print("[BUILD-D8-" + label + "] V=128 bridge: "
...
def build_direction_8_bridge_A_nouns(
    seed: int,
    ...
    """Bridge A: V=128 nouns on bio_brain_regions concept-pool substrate.
    ...
    return _build_bridge_core(
        ...
        noun_pool_names=DIRECTION_8_NOUN_NAMES,
        verbose=verbose, label="A_nouns",
        use_fp16=use_fp16,
    )


def build_direction_8_bridge_B_verbs(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge B: V=128 verbs on bio_brain_regions concept-pool substrate.
    ...
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        verb_pool_names=DIRECTION_8_VERB_NAMES,
        verbose=verbose, label="B_verbs",
        use_fp16=use_fp16,
    )


def build_direction_8_bridge_C_adj(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge C: V=128 adjectives on bio_brain_regions concept-pool substrate.
    ...
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        adjective_pool_names=DIRECTION_8_ADJECTIVE_NAMES,
        verbose=verbose, label="C_adj",
        use_fp16=use_fp16,
    )


def build_direction_8_bridge_D_spatial(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge D: V=128 spatial words on bio_brain_regions concept-pool
    substrate.
    ...
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        noun_pool_names=DIRECTION_8_SPATIAL_NAMES,
        verbose=verbose, label="D_spatial",
        use_fp16=use_fp16,
    )


def build_direction_8_bridge_E_functional(
    seed: int,
    n_lang_input: int = _V14_N_LANG_INPUT_DEFAULT,
    n_per_pool: int = _V14_N_PER_POOL_DEFAULT,
    n_fs_per_pool: int = _V14_N_FS_PER_POOL_DEFAULT,
    ...
    """Bridge E: V=128 functional words on bio_brain_regions concept-pool
    substrate.
    ...
    return _build_bridge_core(
        seed=seed, n_lang_input=n_lang_input, n_per_pool=n_per_pool,
        n_fs_per_pool=n_fs_per_pool, weak_dynamics=weak_dynamics,
        noun_pool_names=DIRECTION_8_FUNCTIONAL_NAMES,
...
DIRECTION_8_BRIDGE_BUILDERS = {
    "A_nouns": build_direction_8_bridge_A_nouns,
    "B_verbs": build_direction_8_bridge_B_verbs,
    "C_adj": build_direction_8_bridge_C_adj,
    "D_spatial": build_direction_8_bridge_D_spatial,
    "E_functional": build_direction_8_bridge_E_functional,
}
