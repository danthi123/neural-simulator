"""Direction 4 Task 5: 5-bridge training runner (CONTROLLER-ONLY;
GPU-bound).

Trains 5 bio_brain_regions bridges (one per vocab category: noun /
verb / adjective / spatial / functional, V=16 each) across 3 seeds
[42, 43, 44] = 15 bridge trainings total. Captures per-bridge per-word
activity vectors across the bridge's V=16 category-pool neuron union;
writes per-seed-per-bridge .npz cache. After all bridges train,
optionally invokes direction_4_cross_bridge_probe (Task 4) inline to
emit the frozen Direction 4 verdict.

ETA (per CLAUDE.md v14/v16 production timing):
- Full scale (n_lang_input=2048, n_per_pool=200, n_events=200):
  ~30-60 min/bridge x 5 bridges x 3 seeds = ~7-15 hr GPU
- Smoke scale (n_lang_input=1024, n_per_pool=100, n_events=50):
  ~3-10 min/bridge x 5 bridges x 3 seeds = ~1-3 hr GPU
...
Pre-registered verdict (frozen): research/findings/raw/direction_4_verdict.py
Pre-registered cross-bridge probe primitive (reuse-by-import,
byte-unchanged): research/findings/raw/direction_4_cross_bridge_probe.py
...
- All npz files use numeric-only arrays; loaders pass allow_pickle=False.
...
_bias byte-pattern; only the per-bridge V=16 vocab differs).
"""
...
from research.findings.raw.direction_4_bridge_builder import (
    DIRECTION_4_BRIDGE_BUILDERS,
)
from research.findings.raw.direction_4_verdict import (
    _DIRECTION_4_OB_MIN, _DIRECTION_4_OI_MIN,
    _DIRECTION_4_LOADS, _DIRECTION_4_MIN_SEEDS,
)

# Reuse-by-import only (validated training + capture primitives;
# pillar n=96 OPTION 3 + Direction 3 V=32 patterns).
...
# Mirrors Direction 3 V=32 runner constants byte-pattern; the only
# mechanical difference: per-bridge V=16 instead of monolithic V=32.
# -----------------------------------------------------------------------
...
# (This matches v14/v16 production recipe sparsity.)
FULL_SPARSITY = 0.05
...
def _per_bridge_target_pool_map(bridge_name: str) -> Dict[str, str]:
    ...
    """
    spec = DIRECTION_4_BRIDGE_CATALOG[bridge_name]
    ...
    if slot == "noun_pool_names":
        ...
        raise ValueError(
            "Direction 4 bridge_catalog has unknown builder_slot '"
...
def _per_bridge_pool_region_names(bridge_name: str) -> List[str]:
    """Per-bridge list of V=16 concept-pool region names (in word-order
...
# -----------------------------------------------------------------------
# Per-bridge topographic-bias helper (V=16 per-bridge; mirrors
# Direction 3 V=32 _apply_v32_topographic_bias pattern byte-by-byte
# except the vocab is the bridge's own V=16 category).
# -----------------------------------------------------------------------
def _apply_per_bridge_topographic_bias(
    ...
    Direction 3 V=32: each edge is "target" for exactly one word; an
    edge that is "off-target" for at least one word gets dampened once.

    Bridge has only V=16 category pools (its category) in the cross-
    bridge probe scope; motor pools also exist structurally but are
    untouched by the per-bridge bias (motor isn't part of the V=16
    cross-bridge concept set).

    Mirrors Direction 3 V=32 helper's algorithmic shape; the only
    mechanical difference is the per-bridge vocab + target-pool set.
    """
    ...
    if len(lang_input_indices) != n_lang_input:
        raise ValueError(
            "Direction 4 per-bridge topographic bias: bridge "
    ...
    # All V=16 output pools for THIS bridge (motor pools deliberately
    ...
    # Pass 2 forward: off-target dampen (within-bridge V=16 pools only)
    dampened_edges = set()
    for word in words:
        target_region = target_map[word]
        global_active = word_to_active[word]
    ...
    # V=32 + concept_pool_demo apply_concept_topographic_bias).
    try:
...
def _build_and_train(
    ...
    + the bridge's V=16 word list + word_to_idx + sparsity + n_lang_input."""
    ...
    if n_words != 16:
        raise ValueError(
            "Direction 4 bridge " + bridge_name + " expected V=16 words; "
            "got " + str(n_words)
        )
    word_to_idx = {w: i for i, w in enumerate(words)}

    builder = DIRECTION_4_BRIDGE_BUILDERS[bridge_name]
    ...
    if verbose:
        print(
            "  [" + bridge_name + "/seed " + str(seed)
            + "] training V=16 substrate (" + str(n_words)
...
# -----------------------------------------------------------------------
# Per-bridge activity capture (V=16 per-bridge category pools only).
# -----------------------------------------------------------------------
def _capture_concept_pool_activity(
    ...
    """Per-neuron activity vectors across the V=16 per-bridge category
    pool union. Same shape / window / drive_max_pA as the OPTION 3 V=16
    probe + Direction 3 V=32 runner."""
    from sim.backend import get_backend
    cp, _ = get_backend()
    from sim.text_embeddings import orthogonal_drive_pattern
...
def _load_activity_cache(
    ...
    """Plain npz loader; numeric-only arrays; allow_pickle=False."""
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 4 5-bridge multi-seed training + cross-"
    ...
    ap.add_argument(
        ...
        help="train only; do NOT invoke direction_4_cross_bridge_probe "
    ...
    gpu = is_gpu_backend()
    print(
        "=== Direction 4 5-bridge multi-seed training runner ===",
        flush=True,
    )
    print(
    ...
    print(
        "  V_per_bridge=16, n_bridges=5, V_total=" + str(DIRECTION_4_TOTAL),
        flush=True,
    )
    if smoke:
        print(
    ...
    print(
        ...
        + "; min_seeds=" + str(_DIRECTION_4_MIN_SEEDS),
        flush=True,
    )
    ...
    if args.bridges is not None:
        ...
        for b in bridges:
            if b not in DIRECTION_4_BRIDGE_BUILDERS:
                raise ValueError(
                    "Unknown Direction 4 bridge name: " + str(b)
                    + " (expected one of "
                    + str(list(DIRECTION_4_BRIDGE_BUILDERS.keys())) + ")"
                )
    else:
        bridges = list(DIRECTION_4_BRIDGE_BUILDERS.keys())
    ...
    if not args.skip_probe:
        ...
        # verdict).
        all_bridges_trained = (
            set(bridges) == set(DIRECTION_4_BRIDGE_BUILDERS.keys())
            and len(seeds) >= _DIRECTION_4_MIN_SEEDS
        )
        if all_bridges_trained:
            ...
            tag = "smoke" if smoke else "full"
            print(
                "\n=== Invoking Direction 4 cross-bridge probe (Task 4) ===",
                flush=True,
            )
            ...
            print(
                ...
                "trained and >= " + str(_DIRECTION_4_MIN_SEEDS)
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_4_5bridge_" + tag + ".json",
    )
