...
def main():
    ...
    if os.path.exists(CACHE):
        ...
        print(f"=== flat-distinct: capturing 192 FLAT codes (3 distinct-seed bridges, backend={backend}) ===",
              flush=True)
        codes = {}; words = []; bank_of = {}
    ...
    print(f"  192-wide between-concept cos: mean {np.mean(btw):.3f}  max {np.max(btw):.3f}  "
    ...
    if min(results) >= 0.80:
        print("VERDICT: RESOLVES -- distinct FLAT codes compose ROBUSTLY on structured facts at ALL seeds "
              ...
              "Scale to 5 bridges (320) for full robust biological composition.", flush=True)
    else:
