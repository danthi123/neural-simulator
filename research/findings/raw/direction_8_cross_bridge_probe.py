"""Direction 8 Task 4: cross-bridge parallel-matching mode-unification
probe on the bio_brain_regions 5-bridge ensemble (640 cross-bridge
concepts = 5 categories x V=128; 2x-vocab extension of Direction 4 V=80).

Mirrors the pillar n=95 cross-bridge probe pattern
(cross_bridge_mode_unification_probe.py) byte-unchanged in primitive,
and the Direction 4 probe shape (direction_4_cross_bridge_probe.py)
in structure: reuses batched_phase_similarity +
verify_batched_equivalent_to_scalar + global mean-centred grounded-symbol
derivation (DERIV_SEED=90909) + ResonateFireFHRR bind/unbind +
parallel-matching decoder (per-slot argmax over the 320-concept union
via the same batched primitive). The only mechanical difference vs D4:
substrate has 5 x V=128 = 640 concepts (vs D7's 5 x V=64 = 320).

CPU-only by design (operates on cached per-bridge activity .npz files
written by direction_8_5bridge_runner.py, Task 5). Does NOT import
...
1. Load 5 bridges' per-seed cached activity (.npz with per-word arrays
   of shape (M_OBS, n_pool_union_per_bridge)).
2. Consolidate per-(bridge, word) activity: mean across M_OBS to one
   vector per concept.
3. Per-bridge mean-centring (cortical pooled inhibition normalises
   within each cortical region; cross-bridge geometry is handled by
   independent per-bridge derivers projecting d_act -> N_DIM=512).
4. For each load L in {2, 3, 5}: sample n_trials composites uniformly
   from the 320-concept union; FHRR-encode with N_GAMMA_SLOTS positional
   codes; per-slot argmax decode + marginal-sum order-invariant decode
   via batched_phase_similarity over the 320-concept phase matrix.
5. Compute per-load (OB, OI) accuracy; emit verdict via the frozen
   direction_8_verdict module.

PRE-REGISTERED reading (frozen by direction_8_verdict module):
- DIRECTION_8_PASS: multi-seed-mean OB AND OI both clear 0.80 at every
  load in {2, 3, 5}.
- DIRECTION_8_PARTIAL: some cells above bar but not all.
- DIRECTION_8_NEGATIVE: NO load-cell on EITHER readout clears the bar.
- DIRECTION_8_VOID_MALFORMED: instrument-validity failure (per the
...
Activity cache contract (written by Task 5 runner):
  ...
  with per-word arrays keyed by str(word):
    npz[str(word)] -> ndarray (M_OBS, n_pool_union) float32

Tag is "full" or "smoke". Per-bridge n_pool_union may differ across
bridges (different builder pool slots) but stays constant per (bridge,
seed) pair. All npz files are loaded in numeric-only mode for safety.
...
from research.findings.raw.direction_8_verdict import (
    ...
    _DIRECTION_8_OI_MIN,
    _DIRECTION_8_LOADS,
    _DIRECTION_8_MIN_SEEDS,
    DIRECTION_8_VOID_MALFORMED,
)

# Reuse-by-import only (validated mode-unification primitives, pillar
# n=95 cross-bridge primitive + pillar n=96 OPTION 3 substrate pattern).
...
# -----------------------------------------------------------------------
# Constants (mirroring n=95 + OPTION 3 V=16 + Direction 3 V=128 +
# Direction 4 5-bridge patterns).
# -----------------------------------------------------------------------
SEEDS = [42, 43, 44]
...
# DIRECTION_8_ALL_WORDS order in direction_8_vocab_spec). The cross-
# bridge probe takes the union in this exact order so word_idx in
# decoder = position in DIRECTION_8_ALL_WORDS.
BRIDGES: Tuple[str, ...] = (
...
CACHE_DIR: str = os.path.join(_HERE, "direction_8_cache")


# -----------------------------------------------------------------------
# Activity-cache loader (numeric-only npz; safety mode).
# -----------------------------------------------------------------------
...
def _load_per_bridge_activity(
    ...
    Returns: {word: ndarray (M_OBS, n_pool_union)} for the bridge's
             V=128 category words.
    ...
    if not os.path.exists(cache_p):
        raise FileNotFoundError(
            "Direction 8 activity cache missing: " + cache_p
    ...
    for w in words:
        if str(w) not in data.files:
            raise ValueError(
                "Direction 8 activity cache " + cache_p
...
# -----------------------------------------------------------------------
# Global mean-centred grounded-symbol derivation (per pillar n=95 +
# OPTION 3 V=16 + Direction 4 5-bridge pattern).
# -----------------------------------------------------------------------
def derive_global_grounded_symbols(
    ...
    """Build the 320-concept union; mean-centre within each bridge; derive
    grounded phasor symbols via the SAME fixed-seed deriver pipeline used
    by pillars n=93/n=94/n=95/n=96/n=105/n=108.

    Activity vectors per bridge have potentially different lengths
    (different n_pool_union due to different pool kinds populated).
    Each bridge's vectors get mean-centred across the 32 concepts on
    THAT bridge (cortical pooled inhibition normalises within each
    cortical region; cross-bridge geometry is handled by independent
    per-bridge derivers projecting d_act -> N_DIM=512 with the fixed
    DERIV_SEED). The derived phasors then live in a uniform N_DIM
    space for cross-bridge similarity computation.

    Args:
        per_bridge_activity: {bridge_name: {word: (M_OBS, d_act_b)}}
    ...
    for bridge in BRIDGES:
        ...
        # Per-bridge d_act (length of pool-union vector). Each bridge
        # may have a different d_act (different pool kinds + counts).
        d_act = next(iter(consolidated_b.values())).shape[0]
        # Mean-centre within this bridge across the 32 concepts. Each
...
# order-invariant). Mirrors pillar n=95 run_one_seed_one_condition with
# the only mechanical difference: 320-concept union from bio_brain_regions
# substrate (5 bridges x V=128 = 320; vs D6's 5 bridges x V=32 = 160).
# -----------------------------------------------------------------------
def run_one_seed_cross_bridge(
    ...
    Returns:
        per_load: {load: {"order_bearing_accuracy": float,
                          ...
                          "n_trials": int}}
        V: total concepts in the union (=640 if all bridges have V=128)
    ...
    # Build the 320-concept union + grounded phasors (global mean-centred).
    ...
    # start. Same fail-closed primitive pillar n=95 + D4 use.
    ...
    for load in loads:
        ...
        for _ in range(n_trials):
            ...
            # ORDER-BEARING: per-slot argmax over the 320-concept union
...
def run_cross_bridge_probe(
    ...
    frozen Direction 8 verdict.

    Args:
        seeds: list of seeds to probe; default SEEDS=[42, 43, 44].
        loads: composition loads; default _DIRECTION_8_LOADS=(2, 3, 5).
        n_trials: trials per load; default N_TRIALS=200.
    ...
    Returns:
        Result dict with shape:
        {
          ...
          "V": 640, "n_bridges": 5,
          "per_seed": [{"seed": N, "V": 640, "per_load": {...},
                         "verdict_entry": {"L=2": {"OB":, "OI":}, ...}}, ...],
          "aggregate": {load: {"order_bearing_mean":, ...}, ...},
          "verdict": "DIRECTION_8_PASS" | "DIRECTION_8_PARTIAL" |
    ...
    gpu = is_gpu_backend()
    if verbose:
        print("=== Direction 8 cross-bridge probe ===", flush=True)
        print(
            "  backend=" + backend_name + " (GPU=" + str(gpu) + "); "
            "seeds=" + str(seeds) + "; loads=" + str(loads)
            + "; bridges=" + str(list(BRIDGES))
            + "; V_total=" + str(DIRECTION_8_TOTAL)
            + "; tag=" + tag,
            flush=True,
        )
        print(
            "  decoder=parallel_population_matching_batched "
            "(320-concept union); mean-centring=per_bridge_local "
            "(n=95 + D4 pattern); reuses pillar n=95 primitive byte-unchanged",
            flush=True,
        )
    ...
    for seed in seeds:
        ...
        seed_results.append({
            ...
            # direction_8_verdict.compute_verdict).
            "verdict_entry": {
                ("L=" + str(l)): {
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 8 cross-bridge parallel-matching probe "
                    "on bio_brain_regions 5-bridge ensemble "
                    "(V=128 per bridge x 5 = 640 cross-bridge concepts; "
                    "CPU-only; operates on Task 5 activity caches)",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        help="override per-bridge activity cache directory (defaults to "
             "research/findings/raw/direction_8_cache)",
    )
    ap.add_argument(
        "--out", default=None,
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_8_cross_bridge_" + tag + ".json",
    )
