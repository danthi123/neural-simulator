"""Direction 7 Task 5: 5-bridge training runner (CONTROLLER-ONLY;
GPU-bound). V=64 per bridge x 5 = 320 cross-bridge concepts.

Trains 5 bio_brain_regions bridges (one per vocab category: noun /
verb / adjective / spatial / functional, V=64 each) across 3 seeds
[42, 43, 44] = 15 bridge trainings total. Captures per-bridge per-word
activity vectors across the bridge's V=64 category-pool neuron union;
writes per-seed-per-bridge .npz cache. After all bridges train,
optionally invokes direction_7_cross_bridge_probe (Task 4) inline to
emit the frozen Direction 7 verdict.
...
Pre-registered verdict (frozen): research/findings/raw/direction_7_verdict.py
Pre-registered cross-bridge probe primitive (reuse-by-import,
byte-unchanged): research/findings/raw/direction_7_cross_bridge_probe.py
...
- At V=64 + n_lang_input=4096, stride = 4096/64 = 64 neurons per cue
...
- At V=64 + n_lang_input=2048 (smoke), stride = 32. sparsity 0.01 ->
...
from research.findings.raw.direction_7_bridge_builder import (
    DIRECTION_7_BRIDGE_BUILDERS,
)
from research.findings.raw.direction_7_verdict import (
    _DIRECTION_7_OB_MIN, _DIRECTION_7_OI_MIN,
    _DIRECTION_7_LOADS, _DIRECTION_7_MIN_SEEDS,
)

# Reuse-by-import only (validated training + capture primitives;
# pillar n=96 OPTION 3 + Direction 3 V=64 + Direction 4 5-bridge patterns).
...
# Mirrors Direction 6 5-bridge runner byte-pattern; the only mechanical
# differences:
# - per-bridge V=64 instead of V=32 (doubled vocab)
# - n_lang_input doubled (2048 -> 4096 full; 1024 -> 2048 smoke) so the
#   orthogonal_drive_pattern stride matches the V=64 cue band count
...
#   the same as D6 V=32 production (no inflation, no shrinkage)
# -----------------------------------------------------------------------
FULL_N_LANG_INPUT = 4096
FULL_N_PER_POOL = 200
...
# V doubling -- per-cue training signal footprint held constant.
FULL_SPARSITY = 0.015
FULL_M_OBS = 16  # observations per concept (matches D6 production)
FULL_TOPOGRAPHIC_FACTOR = 3.0
FULL_OFF_TARGET_FACTOR = 0.3

SMOKE_N_LANG_INPUT = 2048
SMOKE_N_PER_POOL = 100
SMOKE_N_FS_PER_POOL = 12
SMOKE_N_TRAIN_EVENTS = 50
# At V=64 + n_lang_input=2048, stride = 32. sparsity 0.01 -> n_active=20
# < 32 (matches D6 smoke n_active=20 byte-for-byte).
SMOKE_SPARSITY = 0.01
SMOKE_M_OBS = 8
...
CACHE_DIR = os.path.join(_HERE, "direction_7_cache")
os.makedirs(CACHE_DIR, exist_ok=True)

# Per-bridge word + target-pool map (each bridge has its own V=64 vocab
...
def _per_bridge_target_pool_map(bridge_name: str) -> Dict[str, str]:
    ...
    """
    spec = DIRECTION_7_BRIDGE_CATALOG[bridge_name]
    ...
    if slot == "noun_pool_names":
        ...
        raise ValueError(
            "Direction 7 bridge_catalog has unknown builder_slot '"
...
def _per_bridge_pool_region_names(bridge_name: str) -> List[str]:
    """Per-bridge list of V=64 concept-pool region names (in word-order
...
# -----------------------------------------------------------------------
# Per-bridge topographic-bias helper (V=64 per-bridge; mirrors
...
def _apply_per_bridge_topographic_bias(
    ...
    Direction 3 V=64 + Direction 4 5-bridge: each edge is "target" for
    exactly one word; an edge that is "off-target" for at least one word
    gets dampened once.

    Bridge has only V=64 category pools (its category) in the cross-
    bridge probe scope; motor pools also exist structurally but are
    untouched by the per-bridge bias (motor isn't part of the V=64
    ...
    if len(lang_input_indices) != n_lang_input:
        raise ValueError(
            "Direction 7 per-bridge topographic bias: bridge "
    ...
    # All V=64 output pools for THIS bridge (motor pools deliberately
    ...
    # Pass 2 forward: off-target dampen (within-bridge V=64 pools only)
    dampened_edges = set()
    for word in words:
        target_region = target_map[word]
        global_active = word_to_active[word]
...
def _build_and_train(
    ...
    + the bridge's V=64 word list + word_to_idx + sparsity + n_lang_input."""
    ...
    if n_words != 64:
        raise ValueError(
            "Direction 7 bridge " + bridge_name + " expected V=64 words; "
            "got " + str(n_words)
        )
    word_to_idx = {w: i for i, w in enumerate(words)}

    builder = DIRECTION_7_BRIDGE_BUILDERS[bridge_name]
    ...
    if verbose:
        print(
            "  [" + bridge_name + "/seed " + str(seed)
            + "] training V=64 substrate (" + str(n_words)
...
# -----------------------------------------------------------------------
# Per-bridge activity capture (V=64 per-bridge category pools only).
# -----------------------------------------------------------------------
def _capture_concept_pool_activity(
    ...
    """Per-neuron activity vectors across the V=64 per-bridge category
    pool union. Same shape / window / drive_max_pA as the OPTION 3 V=16
    probe + Direction 3 V=64 runner + Direction 4 5-bridge runner."""
    from sim.backend import get_backend
    cp, _ = get_backend()
    from sim.text_embeddings import orthogonal_drive_pattern
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 7 5-bridge multi-seed training + cross-"
    ...
    ap.add_argument(
        ...
        help="train only; do NOT invoke direction_7_cross_bridge_probe "
    ...
    gpu = is_gpu_backend()
    print(
        "=== Direction 7 5-bridge multi-seed training runner ===",
        flush=True,
    )
    print(
    ...
    print(
        "  V_per_bridge=64, n_bridges=5, V_total=" + str(DIRECTION_7_TOTAL),
        flush=True,
    )
    if smoke:
        print(
    ...
    print(
        ...
        + "; min_seeds=" + str(_DIRECTION_7_MIN_SEEDS),
        flush=True,
    )
    ...
    if args.bridges is not None:
        ...
        for b in bridges:
            if b not in DIRECTION_7_BRIDGE_BUILDERS:
                raise ValueError(
                    "Unknown Direction 7 bridge name: " + str(b)
                    + " (expected one of "
                    + str(list(DIRECTION_7_BRIDGE_BUILDERS.keys())) + ")"
                )
    else:
        bridges = list(DIRECTION_7_BRIDGE_BUILDERS.keys())
    ...
    if not args.skip_probe:
        ...
        # verdict).
        all_bridges_trained = (
            set(bridges) == set(DIRECTION_7_BRIDGE_BUILDERS.keys())
            and len(seeds) >= _DIRECTION_7_MIN_SEEDS
        )
        if all_bridges_trained:
            from research.findings.raw.direction_7_cross_bridge_probe import (
                run_cross_bridge_probe,
            )
            tag = "smoke" if smoke else "full"
            print(
                "\n=== Invoking Direction 7 cross-bridge probe (Task 4) ===",
                flush=True,
            )
            ...
            print(
                ...
                "trained and >= " + str(_DIRECTION_7_MIN_SEEDS)
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_7_5bridge_" + tag + ".json",
    )
