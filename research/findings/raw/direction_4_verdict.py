"""Direction 4 verdict module - pre-registered frozen-threshold scoring.

Computes the Direction 4 cross-bridge-on-bio_brain_regions test verdict
ONLY from recorded per-seed cross-bridge parallel-matching accuracy. Does
not re-run, does not retune, does not import any project module. Standard
library + typing only.
...
- _DIRECTION_4_OB_MIN = 0.80     (order-bearing multi-seed-mean min;
                                   same as pillars n=93+ + Directions Q, 3
                                   FROZEN 0.80 multi-seed strict bar)
- _DIRECTION_4_OI_MIN = 0.80     (order-invariant multi-seed-mean min)
- _DIRECTION_4_LOADS  = [2, 3, 5] (pre-registered load ladder; same as
                                   the validated OPTION 3 V=16 probe and
                                   Direction 3 V=32 probe)
- _DIRECTION_4_MIN_SEEDS = 3     (minimum number of seeds for verdict)

Verdict tags (distinct categories):
- DIRECTION_4_PASS           : multi-seed-mean OB AND OI both clear the
                                0.80 bar at EVERY load in the pre-
                                registered ladder. Cross-bridge
                                parallel-matching mode-unification
                                extends to 5x16=80 cross-bridge concepts
                                on bio_brain_regions substrates;
                                conversational-vocabulary foundation
                                extended to 80 biology-faithful concepts.
- DIRECTION_4_PARTIAL        : at least one cell above bar but not all
                                cells; characterise precisely which
                                loads / readouts pass; biology-
                                translatable comparison to G.20 sparse
                                n=95.
- DIRECTION_4_NEGATIVE       : NO load-cell on EITHER readout clears
                                the bar; cross-bridge composition does
                                not cleanly extend from G.20 sparse to
                                bio_brain_regions substrates (a
                                substantial finding).
- DIRECTION_4_VOID_MALFORMED : instrument-validity failure (None, empty,
...
#   tests/test_direction_4_verdict.py::test_thresholds_frozen_at_design_values
#   tests/test_direction_4_verdict.py::test_threshold_tamper_detection
#   tests/test_direction_4_grounding.py::test_direction_4_verdict_thresholds_frozen
# -------------------------------------------------------------------
_DIRECTION_4_OB_MIN: float = 0.80
_DIRECTION_4_OI_MIN: float = 0.80
_DIRECTION_4_LOADS: Tuple[int, ...] = (2, 3, 5)
_DIRECTION_4_MIN_SEEDS: int = 3
...
# without typos).
# -------------------------------------------------------------------
DIRECTION_4_PASS: str = "DIRECTION_4_PASS"
DIRECTION_4_PARTIAL: str = "DIRECTION_4_PARTIAL"
DIRECTION_4_NEGATIVE: str = "DIRECTION_4_NEGATIVE"
DIRECTION_4_VOID_MALFORMED: str = "DIRECTION_4_VOID_MALFORMED"
...
def compute_verdict(
    ...
    """Compute the Direction 4 verdict tag from recorded per-seed
    ...
    Returns:
        One of the DIRECTION_4_* tag constants defined at module top.
    ...
    if per_seed is None:
        return DIRECTION_4_VOID_MALFORMED

    if not isinstance(per_seed, (list, tuple)):
        return DIRECTION_4_VOID_MALFORMED

    if len(per_seed) < _DIRECTION_4_MIN_SEEDS:
        # Multi-seed verdict requires >= _DIRECTION_4_MIN_SEEDS seeds.
    ...
    for entry in per_seed:
        if not _is_valid_seed_entry(entry, _DIRECTION_4_LOADS):
            return DIRECTION_4_VOID_MALFORMED

    # --- Compute multi-seed means ---
    means = _compute_multi_seed_means(
        list(per_seed), _DIRECTION_4_LOADS,
    )
    ...
    all_ob_pass = True
    all_oi_pass = True
    for load in _DIRECTION_4_LOADS:
        ob_mean = means[load]["OB"]
        oi_mean = means[load]["OI"]
        n_cells_total += 2
        if ob_mean >= _DIRECTION_4_OB_MIN:
            n_cells_passed += 1
        else:
            all_ob_pass = False
        if oi_mean >= _DIRECTION_4_OI_MIN:
            n_cells_passed += 1
        else:
            all_oi_pass = False
