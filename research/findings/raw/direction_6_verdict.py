"""Direction 6 verdict module - pre-registered frozen-threshold scoring.

Computes the Direction 6 cross-bridge-on-bio_brain_regions test verdict
(2x-vocab extension of Direction 4; V=32 per bridge x 5 bridges = 160
cross-bridge concepts) ONLY from recorded per-seed cross-bridge parallel-
matching accuracy. Does not re-run, does not retune, does not import any
project module. Standard library + typing only.

Pre-registered thresholds frozen per the binding D6 discipline:
- _DIRECTION_6_OB_MIN = 0.80     (order-bearing multi-seed-mean min;
                                   same as pillars n=93+ + Directions Q,
                                   3, 4 FROZEN 0.80 multi-seed strict bar)
- _DIRECTION_6_OI_MIN = 0.80     (order-invariant multi-seed-mean min)
- _DIRECTION_6_LOADS  = [2, 3, 5] (pre-registered load ladder; same as
                                   the validated OPTION 3 V=16 probe,
                                   Direction 3 V=32 probe, and Direction
                                   4 5-bridge V=80 probe)
- _DIRECTION_6_MIN_SEEDS = 3     (minimum number of seeds for verdict)

Verdict tags (distinct categories):
- DIRECTION_6_PASS           : multi-seed-mean OB AND OI both clear the
                                0.80 bar at EVERY load in the pre-
                                registered ladder. Cross-bridge
                                parallel-matching mode-unification
                                extends to 5x32=160 cross-bridge concepts
                                on bio_brain_regions substrates;
                                conversational-vocabulary foundation
                                extended to 160 biology-faithful concepts
                                (matches pillar n=95 G.20 sparse vocab
                                size on a cleaner architectural substrate).
- DIRECTION_6_PARTIAL        : at least one cell above bar but not all
                                cells; characterise precisely which
                                loads / readouts pass; biology-
                                translatable comparison to D4 V=80 PASS.
- DIRECTION_6_NEGATIVE       : NO load-cell on EITHER readout clears
                                the bar; D4's dedicated-pool composition
                                does not scale from V=16 -> V=32 per
                                bridge (a substantial finding;
                                characterises the substrate's
                                vocab-per-bridge ceiling).
- DIRECTION_6_VOID_MALFORMED : instrument-validity failure (None, empty,
...
# These values come from the D6 discipline binding. They are NOT
...
# caught by:
#   tests/test_direction_6_grounding.py::test_direction_6_verdict_thresholds_frozen
# -------------------------------------------------------------------
_DIRECTION_6_OB_MIN: float = 0.80
_DIRECTION_6_OI_MIN: float = 0.80
_DIRECTION_6_LOADS: Tuple[int, ...] = (2, 3, 5)
_DIRECTION_6_MIN_SEEDS: int = 3
...
# without typos).
# -------------------------------------------------------------------
DIRECTION_6_PASS: str = "DIRECTION_6_PASS"
DIRECTION_6_PARTIAL: str = "DIRECTION_6_PARTIAL"
DIRECTION_6_NEGATIVE: str = "DIRECTION_6_NEGATIVE"
DIRECTION_6_VOID_MALFORMED: str = "DIRECTION_6_VOID_MALFORMED"
...
def compute_verdict(
    ...
    """Compute the Direction 6 verdict tag from recorded per-seed
    ...
    Returns:
        One of the DIRECTION_6_* tag constants defined at module top.
    ...
    if per_seed is None:
        return DIRECTION_6_VOID_MALFORMED

    if not isinstance(per_seed, (list, tuple)):
        return DIRECTION_6_VOID_MALFORMED

    if len(per_seed) < _DIRECTION_6_MIN_SEEDS:
        # Multi-seed verdict requires >= _DIRECTION_6_MIN_SEEDS seeds.
    ...
    for entry in per_seed:
        if not _is_valid_seed_entry(entry, _DIRECTION_6_LOADS):
            return DIRECTION_6_VOID_MALFORMED

    # --- Compute multi-seed means ---
    means = _compute_multi_seed_means(
        list(per_seed), _DIRECTION_6_LOADS,
    )
    ...
    all_ob_pass = True
    all_oi_pass = True
    for load in _DIRECTION_6_LOADS:
        ob_mean = means[load]["OB"]
        oi_mean = means[load]["OI"]
        n_cells_total += 2
        if ob_mean >= _DIRECTION_6_OB_MIN:
            n_cells_passed += 1
        else:
            all_ob_pass = False
        if oi_mean >= _DIRECTION_6_OI_MIN:
            n_cells_passed += 1
        else:
            all_oi_pass = False
