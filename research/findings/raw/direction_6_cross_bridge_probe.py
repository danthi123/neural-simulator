"""Direction 6 Task 4: cross-bridge parallel-matching mode-unification
probe on the bio_brain_regions 5-bridge ensemble (160 cross-bridge
concepts = 5 categories x V=32; 2x-vocab extension of Direction 4 V=80).
...
parallel-matching decoder (per-slot argmax over the 160-concept union
via the same batched primitive). The only mechanical difference vs D4:
substrate has 5 x V=32 = 160 concepts (vs D4's 5 x V=16 = 80).

CPU-only by design (operates on cached per-bridge activity .npz files
written by direction_6_5bridge_runner.py, Task 5). Does NOT import
...
4. For each load L in {2, 3, 5}: sample n_trials composites uniformly
   from the 160-concept union; FHRR-encode with N_GAMMA_SLOTS positional
   codes; per-slot argmax decode + marginal-sum order-invariant decode
   via batched_phase_similarity over the 160-concept phase matrix.
5. Compute per-load (OB, OI) accuracy; emit verdict via the frozen
   direction_6_verdict module.

PRE-REGISTERED reading (frozen by direction_6_verdict module):
- DIRECTION_6_PASS: multi-seed-mean OB AND OI both clear 0.80 at every
  load in {2, 3, 5}.
- DIRECTION_6_PARTIAL: some cells above bar but not all.
- DIRECTION_6_NEGATIVE: NO load-cell on EITHER readout clears the bar.
- DIRECTION_6_VOID_MALFORMED: instrument-validity failure (per the
...
from research.findings.raw.direction_6_verdict import (
    ...
    _DIRECTION_6_OI_MIN,
    _DIRECTION_6_LOADS,
    _DIRECTION_6_MIN_SEEDS,
    DIRECTION_6_VOID_MALFORMED,
)
...
# -----------------------------------------------------------------------
# Constants (mirroring n=95 + OPTION 3 V=16 + Direction 3 V=32 +
...
# DIRECTION_6_ALL_WORDS order in direction_6_vocab_spec). The cross-
# bridge probe takes the union in this exact order so word_idx in
# decoder = position in DIRECTION_6_ALL_WORDS.
BRIDGES: Tuple[str, ...] = (
...
CACHE_DIR: str = os.path.join(_HERE, "direction_6_cache")
...
def _load_per_bridge_activity(
    ...
    if not os.path.exists(cache_p):
        raise FileNotFoundError(
            "Direction 6 activity cache missing: " + cache_p
    ...
    for w in words:
        if str(w) not in data.files:
            raise ValueError(
                "Direction 6 activity cache " + cache_p
...
def derive_global_grounded_symbols(
    ...
    """Build the 160-concept union; mean-centre within each bridge; derive
    ...
    Returns:
        all_words: list of (bridge_name, word) tuples in cross-bridge
                    union order (matches DIRECTION_6_ALL_WORDS).
...
# the only mechanical difference: 160-concept union from bio_brain_regions
# substrate (5 bridges x V=32; vs D4's 5 bridges x V=16 = 80).
# -----------------------------------------------------------------------
def run_one_seed_cross_bridge(
    ...
    Args:
        ...
        loads: composition loads; default _DIRECTION_6_LOADS.
    ...
    Returns:
        per_load: {load: {"order_bearing_accuracy": float,
                          ...
                          "n_trials": int}}
        V: total concepts in the union (=160 if all bridges have V=32)
    ...
    # Build the 160-concept union + grounded phasors (global mean-centred).
...
def run_cross_bridge_probe(
    ...
    frozen Direction 6 verdict.

    Args:
        seeds: list of seeds to probe; default SEEDS=[42, 43, 44].
        loads: composition loads; default _DIRECTION_6_LOADS=(2, 3, 5).
        n_trials: trials per load; default N_TRIALS=200.
    ...
    Returns:
        Result dict with shape:
        {
          ...
          "V": 160, "n_bridges": 5,
          "per_seed": [{"seed": N, "V": 160, "per_load": {...},
                         "verdict_entry": {"L=2": {"OB":, "OI":}, ...}}, ...],
          "aggregate": {load: {"order_bearing_mean":, ...}, ...},
          "verdict": "DIRECTION_6_PASS" | "DIRECTION_6_PARTIAL" |
                      "DIRECTION_6_NEGATIVE" | "DIRECTION_6_VOID_MALFORMED",
    ...
    for seed in seeds:
        ...
        seed_results.append({
            ...
            # direction_6_verdict.compute_verdict).
            "verdict_entry": {
                ("L=" + str(l)): {
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 6 cross-bridge parallel-matching probe "
    ...
    ap.add_argument(
        ...
        help="override per-bridge activity cache directory (defaults to "
             "research/findings/raw/direction_6_cache)",
    )
    ap.add_argument(
        "--out", default=None,
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_6_cross_bridge_" + tag + ".json",
    )
