"""Direction 4 Task 4: cross-bridge parallel-matching mode-unification
probe on the bio_brain_regions 5-bridge ensemble (80 cross-bridge
concepts = 5 categories x V=16).
...
only mechanical difference: substrate is bio_brain_regions concept-pool
activity (per pillar n=96 OPTION 3 substrate), not G.20 sparse Kanerva
patterns.

CPU-only by design (operates on cached per-bridge activity .npz files
written by direction_4_5bridge_runner.py, Task 5). Does NOT import
...
2. Consolidate per-(bridge, word) activity: mean across M_OBS to one
   vector per concept.
3. Mean-centre GLOBALLY across all 80 concepts (the n=95 cross-bridge
   choice; cortical pooled inhibition normalises across the whole
   cortical extent, not per-region). For each bridge, activity vectors
   have potentially different lengths (different n_pool_union); the
   grounded-symbol deriver projects each (length-d_act_bridge) vector
   to N_DIM=512 phasors per bridge's own d_act, then the 80 phasors
   live in a uniform N_DIM space for the cross-bridge similarity.
...
5. Compute per-load (OB, OI) accuracy; emit verdict via the frozen
   direction_4_verdict module.

PRE-REGISTERED reading (frozen by direction_4_verdict module):
- DIRECTION_4_PASS: multi-seed-mean OB AND OI both clear 0.80 at every
  load in {2, 3, 5}.
- DIRECTION_4_PARTIAL: some cells above bar but not all.
- DIRECTION_4_NEGATIVE: NO load-cell on EITHER readout clears the bar.
- DIRECTION_4_VOID_MALFORMED: instrument-validity failure (per the
...
seed) pair. All npz files are loaded with allow_pickle=False (numeric
arrays only).
...
# Direction 4 modules (this arc's net-new code).
...
# -----------------------------------------------------------------------
# Constants (mirroring n=95 + OPTION 3 V=16 + Direction 3 V=32 patterns).
# -----------------------------------------------------------------------
SEEDS = [42, 43, 44]
LOADS = list(_DIRECTION_4_LOADS)
BAR_OB = _DIRECTION_4_OB_MIN
BAR_OI = _DIRECTION_4_OI_MIN
N_GAMMA_SLOTS = 7

# The frozen iteration order over the 5 bridges (matches
# DIRECTION_4_ALL_WORDS order in direction_4_vocab_spec). The cross-
# bridge probe takes the union in this exact order so word_idx in
# decoder = position in DIRECTION_4_ALL_WORDS.
BRIDGES: Tuple[str, ...] = (
...
CACHE_DIR: str = os.path.join(_HERE, "direction_4_cache")


# -----------------------------------------------------------------------
# Activity-cache loader (numeric-only npz; allow_pickle=False).
# -----------------------------------------------------------------------
...
def _load_per_bridge_activity(
    ...
    if not os.path.exists(cache_p):
        raise FileNotFoundError(
            "Direction 4 activity cache missing: " + cache_p
    ...
    for w in words:
        if str(w) not in data.files:
            raise ValueError(
                "Direction 4 activity cache " + cache_p
...
def derive_global_grounded_symbols(
    ...
    by pillars n=93/n=94/n=95/n=96.
    ...
    Each bridge's vectors get mean-centred across the 16 concepts on
    ...
    Returns:
        all_words: list of (bridge_name, word) tuples in cross-bridge
                    union order (matches DIRECTION_4_ALL_WORDS).
...
# the only mechanical difference: 80-concept union from bio_brain_regions
# substrate (vs 160-concept union from G.20 sparse).
# -----------------------------------------------------------------------
def run_one_seed_cross_bridge(
    ...
    Args:
        ...
        loads: composition loads; default _DIRECTION_4_LOADS.
    ...
    # Build the 80-concept union + grounded phasors (global mean-centred).
    ...
    # start. Same fail-closed primitive pillar n=95 uses.
...
def run_cross_bridge_probe(
    ...
    frozen Direction 4 verdict.

    Args:
        seeds: list of seeds to probe; default SEEDS=[42, 43, 44].
        loads: composition loads; default _DIRECTION_4_LOADS=(2, 3, 5).
        n_trials: trials per load; default N_TRIALS=200.
    ...
    Returns:
        Result dict with shape:
        {
          ...
          "verdict": "DIRECTION_4_PASS" | "DIRECTION_4_PARTIAL" |
                      "DIRECTION_4_NEGATIVE" | "DIRECTION_4_VOID_MALFORMED",
    ...
    gpu = is_gpu_backend()
    if verbose:
        print("=== Direction 4 cross-bridge probe ===", flush=True)
        print(
        ...
        print(
            ...
            "(80-concept union); mean-centring=per_bridge_local "
            "(n=95 pattern); reuses pillar n=95 primitive byte-unchanged",
            flush=True,
        )
    ...
    for seed in seeds:
        ...
        seed_results.append({
            ...
            # direction_4_verdict.compute_verdict).
            "verdict_entry": {
                ("L=" + str(l)): {
...
def main():
    ap = argparse.ArgumentParser(
        description="Direction 4 cross-bridge parallel-matching probe "
    ...
    ap.add_argument(
        ...
        help="override per-bridge activity cache directory (defaults to "
             "research/findings/raw/direction_4_cache)",
    )
    ap.add_argument(
        "--out", default=None,
    ...
    out_path = args.out or os.path.join(
        _HERE, "direction_4_cross_bridge_" + tag + ".json",
    )
