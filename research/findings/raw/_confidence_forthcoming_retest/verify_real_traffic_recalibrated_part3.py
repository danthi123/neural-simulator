"""PART 3: redo the default-floor and floor-override UNCERTAIN measurements using the EXACT seed (9013,
sigma=1.3) that _sigma_sweep.py already confirmed produces a genuine still-answering, below-LO-confidence
real turn (noise is stochastic per-seed -- seeds 4001/4002 at the SAME sigma abstained in part2, seed 9013
did not). Reuses part2's lesion result (which already succeeded at seed=4003) and all confident-side /
byte-identical-off data. Produces the FINAL merged results + verdict.
"""
import os, json, time
...
_PART2 = "research/findings/raw/_confidence_forthcoming_retest/verify_real_traffic_FINAL.json"
...
SIGMA_UNCERTAIN = 1.3
SEED_GOOD = 9013   # the exact seed _sigma_sweep.py confirmed: answers, same fact, mrc=0.284 < LO=0.30
RENDERER = "stub"

with open(_PART1) as f:
    part1 = json.load(f)
with open(_PART2) as f:
    part2 = json.load(f)  # carries the already-good lesion-uncertain result
log("loaded part1 + part2")
...
# ═══ (1+3a) default floor, uncertain, seed=9013 ═══════════════════════════════════════════════════════════
log("=== (1+3a) default-floor UNCERTAIN, seed=9013 ===")
d_unc = _noised_query("cf3_unc_default", SIGMA_UNCERTAIN, SEED_GOOD)
...
# ═══ (3b) floor override, uncertain, seed=9013 ═════════════════════════════════════════════════════════════
log("=== (3b) floor-override UNCERTAIN, seed=9013 ===")
FLOOR = tuple(part1["part3b_floor_override"]["floor"])
d3b_unc = _noised_query("cf3_unc_floor", SIGMA_UNCERTAIN, SEED_GOOD, floor=FLOOR)
...
# ═══ lesion (already good from part2) ══════════════════════════════════════════════════════════════════════
d4_unc = part2["part4_lesion"]["uncertain"]
d4_conf = part1["part4_lesion"]["confident"]
log(f"  (reusing part2's lesion-uncertain result: {d4_unc})")

# ═══ MERGE + VERDICT ═══════════════════════════════════════════════════════════════════════════════════════
same_fact = bool(d_unc.get("recalled_svo") == ["brain", "use", "spikes"])
final = {
    "runner": "verify_real_traffic_FINAL (part1 confident-side + part3 recalibrated-seed uncertain-side "
              "[default-floor, floor-override] + part2 lesion-uncertain, merged; through the real "
              "/api/brain-chat handler)",
    ...
    "seed_uncertain": SEED_GOOD,
    "sigma_seed_note": "synaptic noise is stochastic per-seed: sigma=2.2 (matching the raw-composer recalib "
                       "sweep) abstained outright through the FULL rich+VERIFY pipeline; at sigma=1.3, seeds "
                       "4001/4002 ALSO abstained (part2) while seed=9013 (found by _sigma_sweep.py) answers "
                       "cleanly (same fact, mrc=0.284<LO=0.30) -- seed=9013 reused for every uncertain "
                       "condition below for a clean apples-to-apples comparison.",
    ...
    "part4_lesion": {
        "confident": d4_conf, "uncertain": d4_unc,
