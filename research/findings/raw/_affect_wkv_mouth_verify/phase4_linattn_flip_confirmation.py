...
PROVENANCE NOTE (2026-09-04): this script and the FAIL verdict it originally produced
(`_affect_wkv_mouth_verify_phase4_linattn_flip_confirmation_BEFORE.json`, preserved alongside this file) are the
historical record of the diagnosed gap: `Q1_affect_loadbearing_PASS=false` (raw byte-identical lesion0 vs
lesion1) even with a real +mood established (organ differential +0.040). See
`phase4_linattn_flip_confirmation_rerun.py` for the SAME two questions re-run against the sharpness-aware/
saturating/habituating coupling fix, writing a SEPARATE `..._AFTER.json` so both the before and after states
stay on record.
"""
import json
import os
