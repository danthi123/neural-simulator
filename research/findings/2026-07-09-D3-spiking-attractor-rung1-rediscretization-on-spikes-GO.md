...
- **Only the RE-DISCRETIZATION is on-spikes; the TRANSITION (δ: state×input→scores) is still the rate-learned weights** (host-computed scores drive the pools). Next rung: the transition ON-SPIKES. **DESIGN DETERMINED (cheap-first, `discrete_attractor_rnn(linear=...)`): the transition NEEDS a nonlinear hidden layer** — a LINEAR map (no hidden nonlinearity) collapses to step-delta 0.584 / deeper 0.163 (the group-multiplication δ is not linearly separable), vs the tanh-hidden 1.000/1.000. ⇒ the on-spikes transition is a **2-LAYER spiking network** (state pools + input pools → a spiking HIDDEN pool with a rate-coded nonlinearity → the K score pools → the FS-WTA re-discretization → the recurrent loop), NOT a single synaptic projection. The project's graded-spiking-read machinery realizes the rate-coded nonlinear hidden layer.
