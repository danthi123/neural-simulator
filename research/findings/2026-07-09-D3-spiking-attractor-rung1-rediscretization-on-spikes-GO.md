...
- **⇒ FS LATERAL INHIBITION rung (built + validated as the fix, `build_fswta_score_bridge`): K attractor pools + a shared INHIBITORY FS pool (each pool excites FS; FS inhibits all → the winner suppresses runners-up).** On A5 it lifts the spiking re-discretization from plain-WTA 0.75 (agree 0.964) → **FS-WTA 0.90 (agree 0.986)** — the competitive circuit cleans the selection at K=60 (the biology: the project's shared_FS / concept-pool WTA). A small residual (1.4% per-step) still compounds; **fully-clean at large K is an FS-weight/settle TUNING polish** (stronger fs→exc + longer settle so the competition fully resolves). At S3 the plain WTA is already clean (large margins). ⇒ the spiking re-discretization runs on the substrate at both scales; FS lateral inhibition is the confirmed clean-attractor mechanism.
