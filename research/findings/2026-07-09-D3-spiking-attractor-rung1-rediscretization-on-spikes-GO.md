...
- **The plain-drive WTA is faithful (decode the winner) but not a CLEAN one-active attractor** (losers also fire proportionally). Next: **FS lateral inhibition** among the K pools (the project's shared_FS / concept-pool WTA) → only the winner active = a clean spiking attractor state.
