...
seed-waiver: single-seed (42) full-size de-risk; the 6-seed 42/43/44/100/101/102 came back PARTIAL (3/6 GO) — see the PARENT-VERIFIED section. The stable risk-coverage metrics generalize; the strict GO gate does not.
artifacts:
...
## PARENT-VERIFIED (6-seed) — PARTIAL (3/6 GO); the stable metrics generalize, the strict gate does not
<!--derived-->
The parent ran the 6-seed (42/43/44/100/101/102, full-size, artifact
`research/findings/raw/lanes/metacog/laneC_neural_abstain_gate_6seed.json`, verdict PARTIAL, n_go=3/6).
What GENERALIZES (positive on every interpretable seed): the neural abstain gate improves selective accuracy
over no-gate by mean +0.121 and over a first-order winner-magnitude gate by mean +0.129; risk-coverage AUC gain
+0.132; the contingency gap (0.369, hedges familiar-wrong 0.515 vs high-conf-correct 0.146) is 100% attributable
to the self_schema->assert synapse on ALL 6 seeds (lesion -> gap 0.000); and it catches the familiar-but-wrong
confabulation cases a first-order confidence structurally CANNOT (first-order hedges confident errors 0.00 every
seed). The core claim — a NEURAL, learned-confidence-gated hedge that reduces confident errors where a magnitude
read fails — holds.
What does NOT clear the strict 6-seed GO gate (the honest boundary): (1) moat-safe False — on PURE zero-signal
novelty the learned monitor asserts more than first-order (novel-assert 0.50 vs 0.26), because raw
winner-magnitude is itself informative on pure novelty; (2) per-seed variance — seed 101's first-order task fell
out of the [0.60,0.90] type-1 window (acc 0.492), uninterpretable there, and the small confident-error sample
(~7/seed) makes the per-seed contingency noisier than the RC-AUC. VERDICT: PARTIAL — a real neural
honesty-behavior effect on the stable risk-coverage metrics, NOT a clean 6/6 GO. Named next: the production
honesty-hook wire-in (route this confidence into the default-off hook + re-run the hard-moat battery to confirm
the 475/475 abstains hold), and a larger confident-error battery to stabilize the per-seed contingency.
