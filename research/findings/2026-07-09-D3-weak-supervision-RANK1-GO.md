...
## Honest scope + next
- Validated on S3 (K=6). **A5 (K=60) weak-supervision = honest BOUNDARY of end-state-only credit at the structureless worst case:** the curriculum reaches only ~0.71 same-length property and chance (0.017) deeper-track even at nperlen=8000/150-epoch (the coverage lever did NOT fix it). Learning a *structureless* 3600-entry 60-way table from END-STATE-only (weak) credit is fundamentally harder than from per-step teaching (the transition needed nperlen=8000 WITH per-step teaching to reach 0.98). **Mission-relevant framing: A5 is the theorem-backed PESSIMAL case (non-solvable, zero interpolable structure); real LANGUAGE transitions carry compositional structure → they learn from far less, so this boundary is specific to the structureless proxy.** The next mechanism for the structureless case is a DENSER self-supervised signal (RANK-3 per-step observation prediction).
