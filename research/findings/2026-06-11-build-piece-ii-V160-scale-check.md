...
## Honest framing
- This is a **real, honest scale result**, reported as found: the build's capability (generalization) scales; one global structure-recovery diagnostic for the brain-based read-out is soft at scale. The read-out propagation lever is FALSIFIED; the production homeostatic-recurrent learn is the indicated lever (in flight). This is a build-time learn-recipe question, not a capability failure — and exactly what the cheap-first scale-check is for (catch it at V=160, before the V=320 / multi-seed spend).
