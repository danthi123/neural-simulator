...
- The engram approach DOES achieve genuine v1 SOUNDNESS with SELECTIVE retrieval (R1 grounding probe, full scale: the true filler out-fires the seven distractors by a wide margin -- not "all fire equally, top correct by luck"). So iteration 2's positive sub-claim is real; the engram store IS a reliable role-filler memory. It simply is not the EMERGENT mechanism the integrated-loop verdict requires.
