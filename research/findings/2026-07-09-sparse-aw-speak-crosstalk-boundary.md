...
**A validated PARTIAL surpass, not a wall:** the research-gated decorrelating read-out kernel doubles the A→W speak accuracy (0.25→0.50) via a committed kernel, NO `sim/` edit, and (with Turrigiano synaptic-scaling) gives a CLEAN MOAT (novel_margin 0.0005). The winner-inactive rule (post-side, depress-inactive-input) + weight stabilizers plateau at ~50% word-specific. The remaining GO lever is the DECORRELATION the post-side rule can't supply — **Foldiak 1990 anti-Hebbian LATERAL inhibition BETWEEN the `language_output` bands** (the research's Rank-1a′): learned inhibitory connections between co-active output units that push their codes apart, reducing the residual band-band correlation the read-out weights alone can't (this is the piece Foldiak showed is needed on top of feedforward Hebbian to reach a truly sparse/decorrelated code). A bounded next sub-arc (add a plastic anti-Hebbian `language_output↔language_output` lateral); if it also plateaus, the shared-read-out has a genuine ~point-neuron ceiling and Rank-2 is the answer. **PROVEN pragmatic fallback:** the EMERGE-75 multi-bridge dispatch (dedicated per-word pools = word-specific by construction, live) broadens the speakable vocab today. Cross-cutting theme: the point-neuron substrate's spiking READ-OUTs are the recurring hard part — CA3 completion needs a selection sparsifier; A→W articulation needs read-out specificity (now half-solved by the committed decorrelating kernel) + clean pool firing.
