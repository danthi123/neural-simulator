...
- **To fix Rank 1 (biology-preferred):** add a **`language_output` WTA / E%-max sparsifier** (a `language_output_FS` feedback/feedforward pool mirroring `shared_FS`, or a spiking-kWTA / divisive-normalization read-out) so only the strongly-driven word band survives → clean decode; AND a read-out training that stays word-specific (the divergence-under-training suggests a weight-cap / homeostatic read-out). This is the E%-max/divisive-norm family (de Almeida-Idiart-Lisman; Pouille feedforward inhibition) — non-trivial (the same family the CA3 arc characterized as needing sub-dt timing or diversity).
