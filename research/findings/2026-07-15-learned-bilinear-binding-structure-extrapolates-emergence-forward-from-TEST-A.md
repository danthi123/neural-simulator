...
## Blind-seed confirmation (12-seed total): the transport-free biological rule ROBUSTLY beats the unstructured learner
Extended to blind seeds 103/104/105/200/201/202 (`raw/_learned_bilinear_binder_blindseeds.json`):
- transport-free held mean **0.89** on the blind seeds (even stronger than the dev seeds' 0.63); gradient 0.90; fixed bind 0.96; **unstructured MLP 0.48** — the transport-free (biological) rule beats the MLP on ALL 6 blind seeds too.
- ⇒ across all 12 seeds: every STRUCTURED binder (fixed / gradient / transport-free) extrapolates and beats the from-scratch unstructured classifier — the emergence claim (a biological rule learns the binding structure, given the multiplicative inductive bias) is 12-seed robust. The strict GO gate stays memfloor-split-noisy (~5/12) — that is the held-out-selection split-luck (a bounded gate-tightening), NOT a mechanism issue; the load-bearing structure-vs-unstructured comparison is 12/12.
