...
|------|---------|---------|
| 2026-05-01 | [**🎉🎉 Cluster K v2 visual cortex BREAKTHROUGH**](2026-05-01-cluster-k-v2-breakthrough.md) | **GO — NEW BIOLOGY-GROUNDED FLAGSHIP.** 16×16 perception-only (no heuristic, no beacon, no landmark, no place cells) = **2.87 ± 0.19 (n=6)**. Beats 8×8 perception-arc baseline (4.08) on a 4× larger grid. Closes 4 of 5 original cheats. Architecture: retina (32×32 ON/OFF) → V1_simple (Gabor pre-init, Hubel-Wiesel 1962) → V1_complex (phase-pooled) → V2 → IT → cortex_X (gated, zero-init, STDP+reward grows post-warmup). Felleman & Van Essen 1991 ventral stream. |
