...
## On spikes (the fully-spiking mission payoff) — PARTIAL with a characterized cost
`research/runners/_d3_reference_spiking_derisk.py` runs the discourse-referent tracking with the RE-DISCRETIZATION on the spiking FS-WTA substrate (K=6 Izhikevich attractor pools + shared inhibitory FS, the winner referent-pool FIRES, the next holder decoded from spikes). The learned reference-δ tracks the referent on spikes to held-out-DEEPER lengths at **0.683 (host-agree 0.925), 6-seed, ~4× chance** vs the rate 0.885 — a PARTIAL: the spiking re-discretization loses ~7.5% because the reference-δ has intrinsically **small margins** (on a no-op clause the mentioned-but-not-selected entity `b` competes with the holder → a close call the FS-WTA doesn't always resolve), unlike the group DFA's confident scores (S3 spiking was 1.0). Higher input-gain SATURATES the pools (worse); stronger/longer FS-WTA helps marginally. The honest fix (named, not built): train the transition for LARGER margins (a temperature/margin loss) or a stronger recurrent attractor. So D3 tracks discourse referents ON SPIKES with a characterized small-margin re-discretization cost (analogous to the A5 loop PARTIAL).
