...
- **CONFIRMED config-fair (the premature-config check):** the builder's 1-seed smoke used a slightly LARGER config than the runner defaults (ep40/settle30/credit18/subsample160/lr0.3 vs ep30/settle25/credit15/subsample120/lr0.25). Re-running the FULL 6-seed at the builder's exact trainable config STILL gives **0/6 trains_at_all** (plain-FA 0.185 / burstprop 0.198 / microcircuit 0.185, all below chance 0.333) and **microcircuit-vs-FA delta = +0.000** (3/6). The 1-seed smoke (mc 0.370) does NOT even reproduce at its OWN config (seed 42 → 0.074) — i.e. the on-bridge read has run-to-run noise and the "mc>FA" was pure single-seed noise. So the negative is config-fair + 6-seed-robust across BOTH configs, not an artifact of a too-small default.
