...
(`_onebrain_twopool_organread_verify.py`) and artifact were landed from the `research/onebrain-twopool-organ-read`
branch as a documentation-only cherry-pick — the runner code itself was NOT merged, superseded by the declarative
`research/runners/onebrain_merge_framework.py`, which this de-risk's own text names as its "working PROTOTYPE"
(`2026-08-27-onebrain-merge-framework-DESIGN.md`). The framework's current `REGISTRY` does not yet include
`metacog`/`pragmatic` (only `surprise`/`world-model`/Group-A), so this doc is kept as prior art for that follow-on
migration, not as a currently-runnable result.
