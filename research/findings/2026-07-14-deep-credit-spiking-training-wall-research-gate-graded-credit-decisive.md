...
## ▶ K=8 FULLY CLOSES the production gap (3-seed) — the on-bridge deep-credit result reaches the LIF ceiling
Scaling the population lever to K=8: on-bridge **inherit 0.877** (per-seed 0.889/0.926/0.815), train 0.963 — **≈ the LIF ceiling 0.89**. The clean √K trend: **K=1 0.47 → K=4 0.62 → K=8 0.877 → LIF 0.89**. ⇒ population coding fully closes the on-bridge forward-SNR residual; **transport-free biological e-prop + population trains deep compositional credit on the PRODUCTION Izhikevich spiking bridge to the ceiling.** The deep-credit-on-spikes arc is COMPLETE end-to-end on the production substrate — rule (LIF, anti-cheat-clean) → ports (positive control) → full-scale closure (K=8 = LIF ceiling). NO `sim/` edit anywhere. (Honest: seed-43 permuted 0.481 is the recurring task-instance outlier; the other seeds are ~chance and all 3 train+generalize genuinely.)
