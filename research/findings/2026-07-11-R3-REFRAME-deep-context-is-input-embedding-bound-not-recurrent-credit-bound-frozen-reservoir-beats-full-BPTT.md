...
## ✅ THE KP LEVER (built + run): learned local feedback ROUGHLY DOUBLES the plain plateau (→ 78% of the ceiling), then plateaus — a characterized residual, not a wall
`eprop_learn_win_kp` = the biological arm + Kolen-Pollack learned feedback (Bfb co-evolves with W_out via the transpose increment `(hᵀδ)/batch` → Bfb → W_outᵀ; local h,δ only, NO weight transport). lr_in sweep, seed 42, 2M tokens:
| lr_in | plain `eprop_learn_win` (fixed random fb) | `eprop_learn_win_kp` (KP learned fb) |
|---|---|---|
| 5.0 | +0.037 | +0.286 |
| 10 | +0.117 | +0.330 |
| 20 | +0.177 | +0.356 |
| 50 | (plateau ~+0.18) | **+0.368 (plateau)** |
- **KP learned feedback ~DOUBLES the plain plateau** (fixed-random-fb ~+0.18 → KP ~+0.37) and closes **78%** of the random-reservoir→full-backprop-on-W_in gap (up from ~70%), improving ctx1 −1.45 → **−0.75** (toward the ceiling's −0.12) — confirming the feedback-alignment lever works, on the RIGHT bottleneck.
- **Multi-seed (4 seeds 42/43/44/100, lr_in=20): the biological arms are init-robust to an EXTRAORDINARY degree** — plain `eprop_learn_win` **+0.176 ± 0.001**, `eprop_learn_win_kp` **+0.351 ± 0.003**; KP beats plain and both beat the bigram on ALL 4 seeds. The ~2× KP lift + the positive deep margin are not seed artifacts.
- **KP ALSO plateaus (~+0.37), ~22% short of the +1.257 ceiling.** The residual is the HONEST, characterized cost of the remaining biologically-plausible approximations: (a) the local **delta read-out** vs BPTT's AdamW (BPTT_frozen_wrec trains W_out by AdamW); (b) **partial KP alignment** (Bfb→W_outᵀ is asymptotic, imperfect during finite training); (c) the **diagonal input eligibility** itself (e-prop's input credit for the DEEP prediction still flows through the recurrence, whose off-diagonal terms the diagonal trace drops — visible even at ctx1, where KP reaches −0.75 not the ceiling's −0.12, so part of the gap is the credit signal even with NO recurrence). Each is a KNOWN limit with a further lever, NOT a wall.

## ⇒ ARC CONCLUSION (the honest bottom line — the biology-legal long-range path is VALIDATED to ~78%, the recurrent-credit wall is SIDESTEPPED)
The long-range language capability the external-transformer scaffold provides is, at this scale, **input-representation + read-out bound, not recurrent-credit bound.** A fully biology-legal circuit — **a FIXED random recurrent reservoir + an e-prop-learned INPUT representation (input-synapse eligibility) + LEARNED LOCAL (Kolen-Pollack) feedback + a LOCAL read-out**, all one-step-local, NO backprop-through-time, NO weight transport — captures the deep-context/long-range structure and reaches **~78% of the way from a random reservoir to full backprop-on-the-input** (a positive deep margin, beats the add-1 bigram, deep-specific). The hard off-diagonal RECURRENT-credit problem (the target of the whole R1-R2 arc, and the field's months-scale wall) is **SIDESTEPPED ENTIRELY** — the recurrent weights need no training (training them is counterproductive). The residual ~22% to full backprop is the characterized cost of the local read-out + partial-feedback-alignment + diagonal-input-eligibility, each with a known further lever. **⇒ a de-risked, on-substrate-realizable path to replace the transformer scaffold's long-range capability with simulated LOCAL-learning circuitry** — NO `sim/` rewrite for the rate-level mechanism; the next real step is the SPIKING realization on a `SimulationBridge` reservoir (a fixed recurrent `BrainRegion` + the on-bridge BDSP-style input eligibility + a learned-feedback read-out).
