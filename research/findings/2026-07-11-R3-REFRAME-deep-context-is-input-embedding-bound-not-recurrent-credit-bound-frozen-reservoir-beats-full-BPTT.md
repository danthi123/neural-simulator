...
## Honest scope
Single seed (42), single layer, n_pool=300, 2M tokens — a decisive single-run reframe; multi-seed confirmation + the `eprop_learn_win` biological test are the immediate follow-ons. The BPTT arms are AdamW (a reference ceiling); the biological realization uses e-prop + delta. All rate-level torch, GPU, NO `sim/` edit. This reframe supersedes the "off-diagonal recurrent credit / multi-layer" next-direction earmarked earlier — the bottleneck is the input representation, not the recurrent credit.
