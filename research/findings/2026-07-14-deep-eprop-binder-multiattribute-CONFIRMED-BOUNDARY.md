# Learned cortical binder (ROADMAP #2) — a DEEP e-prop-trained binder MEMORIZES multi-attribute bundling but does NOT GENERALIZE it: depth is NOT the lever, the wall is SYSTEMATICITY (3-seed CONFIRMED-BOUNDARY)

## ⚠️ CORRECTION (2026-07-14, a-1) — the "dendritic-multiplication substrate is the next lever" conclusion below is REFUTED by our own prior record; the honest verdict is a TASK/REPRESENTATION boundary the fixed FHRR primitive already answers

Reading `2026-06-24-burndown-3B-deep-dendritic-binder.md` (which this de-risk should have a-1-checked BEFORE naming the next lever): the DEEP **DENDRITIC** (multiplicative sigma-pi / Hadamard) learned binder for this EXACT multi-attribute task was **already comprehensively tested** and is a certified NEGATIVE — and it is the **WORST** generalizer (held-out **0.007**, *worse* than tonight's point-neuron 0.002-level and the single-layer 0.168), because a learned role-conditioned read-out learns the **per-role allowed-filler priors** and generalizes *against* the held-out answer (the fixed FHRR wins **because it doesn't learn** those priors). **Decisively, the ORACLE (hand-derived TRUE gradient, unlimited capacity) ALSO fails** (held-out 0.007, train 0.984) — so the multi-attribute-bundle generalization failure is a **TASK/REPRESENTATION boundary**, NOT the learning rule, NOT depth, and NOT point-neuron-vs-dendrite. ⇒ **the "dendritic multiplication (D2) is the honest next lever" conclusion below is WRONG** — the dendrite was already ruled out (it's the worst, and even true gradients fail). The honest verdict: the multi-attribute *learned* bind is a comprehensively-characterized boundary; the **fixed ±1 / FHRR binding-by-coincidence primitive is the biology-grounded STRUCTURAL answer** (a structural neural primitive — the production composer already binds the LEARNED codes *through* it — NOT a host shortcut, NOT a build). **Tonight's value survives as a confirming data point** (a NEW credit rule — transport-free e-prop — fails the SAME way, from the point-neuron side), reinforcing the boundary; only the next-lever recommendation is retracted. This is the a-1 discipline catching a near-re-derivation + a wrong next-lever before building on it.

---

## (ORIGINAL — the "dendritic D2 is the next lever" recommendation is SUPERSEDED by the CORRECTION above; the 3-seed numbers + the depth-memorizes-not-generalizes finding stand)
