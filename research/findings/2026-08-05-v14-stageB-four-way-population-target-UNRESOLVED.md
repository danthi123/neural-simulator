...
A same-day DOI-, title-, author-, and repository-specific web search found the two primary articles, the PMC HTML/figure assets, the published summary tables, NeuroElectro's sodium summary table, and the Kv3 supplementary-figures PDF. It found no Dryad, Figshare, Zenodo, GitHub, institutional, or DOI-linked archive containing the original point-level activation, inactivation, recovery, or deactivation data. PMC's open-access-package API reports both article identifiers as not open access. Direct terminal retrieval of the APS PDFs returned HTTP 403, but the publisher's normal article pages allowed both PDFs to be downloaded through the interactive browser. `pdfimages -list` verified that the relevant figures are embedded raster images rather than recoverable vector paths. Their figure images are no higher resolution than the already authenticated PMC tile assets: the sodium PDF uses 300-dpi raster figures, while the Kv3 PDF uses a mixture of 300- and 600-dpi rasters whose relevant full-figure dimensions are equal to or smaller than the existing assets. The publisher PDFs therefore do not provide stronger measurement authority. Their local SHA-256 digests are `56a27f6b6983dbdc5ba1bbfb39c7257044202185fd8fa8831d9cccb4cc58eed9` (sodium) and `f610fd0d762104ea61c26a76a4311e5959bea3b626015dcffc68775b1c844375` (Kv3); the copyrighted PDFs remain outside the repository. The papers identify Fu-Ming Zhou as the corresponding author; requesting original numeric data is a valid later user-approved external action, not something this result silently assumes will succeed.
