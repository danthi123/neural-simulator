...
`.../lbf_off/<seed>/lbf_settle_off_<seed>.json` (`<seed>` is a placeholder for the actual seed number, e.g. `s42`;
not a literal path). The harvest copies each node into its own subdirectory (`lbf_on/<node>/<seed>/`), so no two
files can collide.
