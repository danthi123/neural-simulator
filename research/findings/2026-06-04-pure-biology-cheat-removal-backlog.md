# Pure-biology backlog: cheats/shortcuts to remove (standing directive) — 2026-06-04

**Owner directive (2026-06-04):** *"When possible (no other high-priority work remaining), I'd like to remove
any remaining cheats/shortcuts to get the sim back into a pure-biology-backed state."*

This is a **standing backlog**, not immediate work — it runs when no higher-priority task remains. It serves
the top-line goal directly (artificial life with a *proper* brain analogue; biology-translatable insights;
honest negatives under strict biology are the deliverable). This doc is the honest, growing audit of where
the current system still departs from pure biology, so the departures are explicit and removable rather than
silent.

## AUDIT 2026-06-04 (cheat-removal arc start)

Re-audited each item against the project's current state. **Headline: the biggest departure (#1, composition as
algebra not spikes) is RESOLVED this session** — so the current state is substantially cleaner than this backlog
first suggested. Status: #1 RESOLVED; **#2 RESOLVED 2026-06-04** (genuine D1⊣GPi⊣thal disinhibition opens the
gate, no direct thalamic current — 11/12 multi-seed); **#3 PARTIAL 2026-06-04** (cortico-striatal STDP genuinely
LEARNS the selection — correct verb→D1 grows, wrong stays at init; end-to-end routing pending a synaptic-drive fix
for the high-rheobase MSN); **#4 PARTIAL 2026-06-04 — both grounding levels' mechanism validated** (word-cue
RESOLVED; visual/Gabor cheap-first RESOLVES — V1 features → 97%-cleanup-able codes; agent integration + non-visual
coverage remain); #5 strengthened (resonate-and-fire validated for the full composition); #6/#7 untouched
(secondary / older nav thread). So this session (owner's "2 and 3 then 4") moved #2→resolved, #3→learning-validated,
#4→both-levels-validated. Remaining open: #3 end-to-end (drive strength), #4 agent integration + multi-modal,
#6/#7 (secondary).

## Current departures from pure biology (honest audit, this arc)

| # | Shortcut | Where | Pure-biology target |
|---|---|---|---|
| 1 | **Composition runs as numpy *algebra*, not spikes.** ~~The phasor FHRR composition agent computes with complex-number arithmetic; the brain analogue is the spiking realization.~~ | `nested_composition_agent.py` (the fast CPU path remains) | **RESOLVED 2026-06-04:** the full unified agent is realized in GENUINE SPIKES — `spiking_unified_agent.py` (Orchard-2023 substrate, full benchmark 72/72 = 100% at 320 vocab) AND `_rf_unified_agent_probe.py` (the BIOLOGICAL resonate-and-fire neuron + ResonateFireTPAM cleanup, full benchmark 36/36 = 100% at 320 vocab). The brain analogue now does fact memory + 1/2-attribute composition + clauses + who/what + the no-confab moat in genuine spikes. The numpy algebra agents remain as the labelled fast-CPU path (not the brain-analogue claim). See `2026-06-04-spiking-unified-agent-stage2.md`. |
| 2 | ~~**BG gate selection is a stand-in.** Thalamic gate pools are driven *directly* with input current.~~ | `gated_compose_bg_demo.py` | **RESOLVED 2026-06-04:** the gate is now opened by a genuine direct-pathway cascade `D1 -| GPi -| thal` (GPi tonically pacing silences its relay; a D1 "go" signal silences GPi → disinhibits the relay → opens the route gate). `gated_compose_bg_genuine_demo.py`, 11/12 across 3 seeds, D1→GPi inhibition isolated + confirmed. The non-obvious blocker was synaptic **weight scale** (weight ~300 explodes g_i to ~2300, clamps V to the −75 reversal, breaks Izhikevich numerics into rebound firing → "inhibition" reads as excitation); g11_bg-scale weights (D1→GPi=15, GPi→thal=8) fix it. Insight: conductance-based inhibition is only inhibitory within a physiological conductance range. (WHICH D1 is driven is still commanded → that's #3.) See `2026-06-04-cheat2-genuine-bg-disinhibition-RESOLVED.md`. |
| 3 | **Gating binding is *commanded*, not learned/emergent.** | gated-compose demos | **PARTIAL 2026-06-04** (`gated_compose_bg_learned_demo.py`): (1) LEARNING validated — cortico-striatal STDP selectively grows the CORRECT verb→D1 synapse (0.5→~16) while wrong targets stay at 0.5 (genuinely learned, not commanded). Bug found: `_run_one_simulation_step()` doesn't advance `current_time_ms` → direct-call STDP was a silent no-op. (2) DRIVE wall closed — cheap-first `_msn_synaptic_drive_probe`: a ≥300-neuron cue pool fires the high-rheobase MSN-D1 at the learned weight → the genuine #2 cascade completes from the cue alone (silent at 30/100). (3) FULL multi-binding close = **HONEST NEGATIVE**: at `--n-verb 500`, TRUE 3/12 AND PERMUTED 3/12 (both chance) — every verb routes to the structural-N-bias motor regardless of teacher. Pool scaling fixes the DRIVE wall but not the multi-binding SELECTION-ARBITRATION wall (the documented silent-motor-trap / N-bias). Localized remaining fix: motor WTA / FS lateral inhibition (one-binding-wins), NOT more drive. See `2026-06-04-cheat3-learned-gate-selection-PARTIAL.md`. |
| 4 | **Codes are random/constructed or learned-but-not-grounded.** Concept codes are random phasors or STDP-learned from a hashed word encoder, not from real sensory grounding. | phasor agents; `vocab_to_drive_pattern` | Sensory-grounded codes (the visual-cortex / Gabor pipeline, Cluster K v2) feeding the concept representations. **PARTIAL 2026-06-04 — both levels' MECHANISM now validated.** (a) *grounded word-cue* level RESOLVED: codes learned from `vocab_to_drive_pattern` via STDP support full composition through pattern completion (grounded-cleanup mode 92.3% = constructed, anti-cheat-passed). (b) *visual/Gabor sensory* level RESOLVED incl. full agent integration: real V1 Gabor receptive fields (`sim/visual_cortex.py`) → grounded codes that are well-separated + cleanup-able (97%) + compose (100%/92%); and the FULL unified-agent benchmark on V1-grounded codes + a ventral-hierarchy decorrelation step (ZCA = efficient coding) **matches the constructed baseline EXACTLY (92.3%, 6-category core 100%)** — `unified_agent_visual_grounded.py --decorrelate`. The blocker was single-V1-layer inter-code coherence; decorrelation (V1→IT) yields composition-ready codes. A complex-vs-phase format bug was caught + corrected (smell-test). See `2026-06-04-cheat4-visual-grounding-cheap-first-RESOLVES.md` + `2026-06-04-cheat4-visual-grounding-agent-integration.md`. Remaining open: multi-modal coverage of non-visual/abstract concepts (no canonical image — embodied-cognition limit), and using the project's actual V1→V2→IT stack in place of the ZCA stand-in. |
| 5 | **Phasor binding is a biologically-grounded *hypothesis*, not an established brain mechanism.** | the whole FHRR substrate | Not removable per se — but keep it labelled as a hypothesis; strengthen the biological grounding (resonate-and-fire, theta-gamma phase coding, the high-Q/Ih insight). **STRENGTHENED 2026-06-04:** the full composition agent now runs on the genuine resonate-and-fire neuron model (Izhikevich 2001 / Frady-Sommer 2019) + a complex-valued attractor-network cleanup (ResonateFireTPAM; abstention = a basin-of-attraction property) — the binding hypothesis is now realized in a recognized biological neuron model, not only algebra. Still a hypothesis (the brain may bind differently), kept labelled. |
| 6 | **Generation baseline uses a non-biological transformer teacher** (Generator-F) for the fluency reference. | generator arc | Acknowledged secondary/labelled; the from-scratch spiking generation is a documented wall. Keep clearly separated as "what spiking *inference* can do given non-biological weights." |
| 7 | **Older navigation/RL cheats** (heuristic, direct (x,y)/(gx,gy) access) — documented long-standing. | `g11_bg_runner.py` flags | The perception-arc closures already removed most (4 of 5 cheats closed, 2026-04-27); audit what remains and whether it's still used. |

## Process when this backlog is worked

For each item: (a) confirm it is actually still load-bearing (some may already be vestigial); (b) build the
pure-biology replacement; (c) re-validate the capability under the stricter regime — **a capability that only
worked with the shortcut, and honestly fails without it, is itself a finding** (the deliverable is honest
negatives under strict biology). Do NOT quietly keep a shortcut to preserve a number.

This list grows as more departures are noticed. It is the explicit ledger that keeps "proper brain analogue"
honest.
