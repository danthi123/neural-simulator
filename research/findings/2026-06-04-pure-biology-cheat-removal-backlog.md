...
| 4 | **Codes are random/constructed or learned-but-not-grounded.** Concept codes are random phasors or STDP-learned from a hashed word encoder, not from real sensory grounding. | phasor agents; `vocab_to_drive_pattern` | Sensory-grounded codes (the visual-cortex / Gabor pipeline, Cluster K v2) feeding the concept representations. **PARTIAL 2026-06-04 — both levels' MECHANISM now validated.** (a) *grounded word-cue* level RESOLVED: codes learned from `vocab_to_drive_pattern` via STDP support full composition through pattern completion (grounded-cleanup mode 92.3% = constructed, anti-cheat-passed). (b) *visual/Gabor sensory* level cheap-first RESOLVES: real V1 Gabor receptive fields (`sim/visual_cortex.py`) turn distinct visual stimuli into concept codes that are well-separated (mean cosine 0.25) and robustly cleanup-able (97% under noise+≤2px translation) — same attractor-cleanup mechanism. See `2026-06-04-cheat4-visual-grounding-cheap-first-RESOLVES.md`. Remaining open: agent integration (feed visual-grounded codes as `external_codes` + benchmark) + multi-modal coverage of non-visual/abstract concepts (no canonical image — embodied-cognition limit). |
