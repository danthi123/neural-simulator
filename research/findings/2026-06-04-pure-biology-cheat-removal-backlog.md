...
| 3 | **Gating binding is *commanded*, not learned/emergent.** | gated-compose demos | **PARTIAL 2026-06-04** (`gated_compose_bg_learned_demo.py`): the LEARNING half is validated — cortico-striatal STDP selectively grows the CORRECT verb→D1 synapse (0.5→~15-18) while wrong targets stay at 0.5, so the cue genuinely LEARNS to select its D1 (not commanded). Found a load-bearing bug en route: `_run_one_simulation_step()` doesn't advance `current_time_ms` (caller must, bridge.py:3179) → direct-call STDP was a silent no-op (frozen clock → delta_t=0). REMAINING (engineering, not science): the learned weight doesn't drive the high-rheobase MSN-D1 to fire *synaptically* at inference (same wall #2 sidestepped with direct current). Continuation: Tier-1-scale presynaptic pools (500-1000 + motor FS) or an excitable cortico-striatal relay → then learned cue fires D1 → genuine #2 cascade → gate. See `2026-06-04-cheat3-learned-gate-selection-PARTIAL.md`. |
