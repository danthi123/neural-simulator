...
first suggested. Status: #1 RESOLVED; **#2 RESOLVED** (genuine D1⊣GPi⊣thal disinhibition opens the gate, no direct
thalamic current — 11/12 multi-seed); **#3 RESOLVED** (cortico-striatal STDP genuinely LEARNS the selection; learned
cue → genuine cascade → gate → 12/12 multi-seed + permuted-teacher anti-cheat PASSES); **#4 RESOLVED for the visual
subset** (word-cue + visual/Gabor grounding both feed the agent; full agent integration matches constructed codes
92.3% via ventral-hierarchy decorrelation); #5 strengthened (resonate-and-fire validated for the full composition);
#6/#7 untouched (secondary / older nav thread). **So the owner's "2 and 3 then 4" are ALL RESOLVED.** Plus the
brain-analogue unification capstone (spiking agent on V1-grounded codes = 72/72 = 100%). Remaining open: #4
multi-modal (non-visual concepts), #6/#7 (secondary), and the top-line conversational frontiers (clause-depth2,
vocab scaling, richer dialogue).
...
| 2 | ~~**BG gate selection is a stand-in.** Thalamic gate pools are driven *directly* with input current.~~ | `gated_compose_bg_demo.py` | **RESOLVED 2026-06-04:** the gate is now opened by a genuine direct-pathway cascade `D1 -| GPi -| thal` (GPi tonically pacing silences its relay; a D1 "go" signal silences GPi → disinhibits the relay → opens the route gate). `gated_compose_bg_genuine_demo.py`, 11/12 across 3 seeds, D1→GPi inhibition isolated + confirmed. The non-obvious blocker was synaptic **weight scale** (weight ~300 explodes g_i to ~2300, clamps V to the −75 reversal, breaks Izhikevich numerics into rebound firing → "inhibition" reads as excitation); g11_bg-scale weights (D1→GPi=15, GPi→thal=8) fix it. Insight: conductance-based inhibition is only inhibitory within a physiological conductance range. (WHICH D1 is driven is still commanded → that's #3.) See `2026-06-04-cheat2-genuine-bg-disinhibition-RESOLVED.md`. |
| 3 | ~~**Gating binding is *commanded*, not learned/emergent.**~~ | gated-compose demos | **RESOLVED 2026-06-04** (`gated_compose_bg_learned_demo.py --n-verb 1000`): the gate that opens is selected by a TRAINED cortico-striatal pathway, not commanded. Cortico-striatal STDP selectively learns verb→D1; the learned cue fires its D1 → the genuine #2 disinhibition cascade → opens the correct gate → routes. **TRUE 12/12 across 3 seeds + permuted-teacher anti-cheat PASSES** (permuted teacher → learns permuted mapping 10/12; true mapping → 1/12 below chance = no structural bias). The "structural N-bias / needs WTA" reading was wrong (decode-default artifact); 3 smell-test corrections found the real fix: rebalance the GPi pacemaker tonic to the LEARNED regime (`GPI_TONIC_PA=600` vs #2's 2200) so a weak STDP-learned d1 (0.06) can silence its GPi. Bugs found en route: `_run_one_simulation_step()` doesn't advance `current_time_ms` (direct-call STDP silent no-op); a cascade tuned for strong direct-drive input fails on weak learned input. See `2026-06-04-cheat3-learned-gate-selection-PARTIAL.md`. |
| 4 | **Codes are random/constructed or learned-but-not-grounded.** Concept codes are random phasors or STDP-learned from a hashed word encoder, not from real sensory grounding. | phasor agents; `vocab_to_drive_pattern` | Sensory-grounded codes (the visual-cortex / Gabor pipeline, Cluster K v2) feeding the concept representations. **PARTIAL 2026-06-04 — both levels' MECHANISM now validated.** (a) *grounded word-cue* level RESOLVED: codes learned from `vocab_to_drive_pattern` via STDP support full composition through pattern completion (grounded-cleanup mode 92.3% = constructed, anti-cheat-passed). (b) *visual/Gabor sensory* level RESOLVED incl. full agent integration: real V1 Gabor receptive fields (`sim/visual_cortex.py`) → grounded codes that are well-separated + cleanup-able (97%) + compose (100%/92%); and the FULL unified-agent benchmark on V1-grounded codes + a ventral-hierarchy decorrelation step (ZCA = efficient coding) **matches the constructed baseline EXACTLY (92.3%, 6-category core 100%)** — `unified_agent_visual_grounded.py --decorrelate`. The blocker was single-V1-layer inter-code coherence; decorrelation (V1→IT) yields composition-ready codes. A complex-vs-phase format bug was caught + corrected (smell-test). See `2026-06-04-cheat4-visual-grounding-cheap-first-RESOLVES.md` + `2026-06-04-cheat4-visual-grounding-agent-integration.md`. Remaining open: multi-modal coverage of non-visual/abstract concepts (no canonical image — embodied-cognition limit), and using the project's actual V1→V2→IT stack in place of the ZCA stand-in. |
