...
> **➡ See also the MULTI-SEED companion `2026-06-11-learned-graded-embedding-homeostasis-GO.md` (subagent a8798a54c0d0d500b, commit f0800378): the 3-seed (42/43/44) GO that this single-seed arc anticipated — and it explains the smoke "BOUNDARY" as a brittle G5-anti-cheat boolean (the permuted-co-occurrence `perm_is_graded` flips on a structureless matrix), verified controller-side from the raw JSON (seed-43 g1_ok+g2+permuted-Pearson-collapse all pass; only the bare boolean trips).**
>
> **Verdict: BOUNDARY at smoke scale → RESOLVED to GO at full scale (see the UPDATE section below) + MULTI-SEED GO 3/3 (companion doc).** The load-bearing question — *does canonical biological homeostasis make the learned embedding's faithfulness cycle-INDEPENDENT, so the months-scale build doesn't have to hand-pick `cycles=2`?* — is answered **YES (GO).** Both Turrigiano synaptic scaling and Oja's rule flatten the un-normalized saturation collapse from slope **−0.0214/cyc → −0.0020/cyc** (≈11× shallower) and hold faithfulness high across cycles 2→20 *and* across 2× store volume. The BOUNDARY label came from a single STRICT sub-bar at smoke scale: the within-cluster-vs-between-cluster **cosine-margin `graded` flag** read 0 at both cycle counts (n_pool=1000, pattern=60), even though the Pearson, 2nd-order cat~dog margin, and generalization bars it predicts all PASSED. **The full-scale confirmatory (below) shows that soft flag was a scale + set-point artifact: at production scale the homeostatic recurrent holds `graded=1` across cycles with generalization 1.000.**
