...
- **sizes are the emergent regime** — mean 23.1 cells (per-seed means 19.7–26.0; individual assemblies 14–33) vs the
  readout's pre-assigned 0.18*N = 360-cell mask.
- **nearly disjoint from the pre-assigned set** — Jaccard vs the random-permutation the readout would use ≈ 0.005–0.02
...
NOT a BTSP-formation failure. **BTSP-completion GO = 0/6 OU-off AND 0/6 OU-on.** 6-seed means (OU-off, mean over 6
seeds × 2 wmax):

| density | arm | held_cue | held_perm | held_nocue | cross_dw |
|---|---|---|---|---|---|
| 0.12 | handinstall | 0.291 | 0.261 | 0.285 | — |
| 0.12 | btsp | 0.286 | 0.238 | 0.273 | −0.015 |
| 0.35 | handinstall | 0.426 | 0.427 | 0.445 | — |
| 0.35 | btsp | 0.426 | 0.430 | 0.447 | +0.001 |
| 0.50 | handinstall | 0.463 | 0.471 | 0.481 | — |
| 0.50 | btsp | 0.463 | 0.471 | 0.466 | +0.001 |

- **perm ≈ nocue ≈ cue everywhere** — the attractor fires but is not cue-selective and SELF-IGNITES. At the least-bad
  density (0.12) cue (0.286) is only marginally above perm (0.238) and barely above rest (nocue 0.273 = the held-out
  cells fire almost as much with NO cue) — far below the 3× GO bar. `attributable_to` reports correct-cue vs
  permuted-cue as NON-attributable (the control equals or exceeds the treatment).
- **the density sweep makes it WORSE, not better** — raising CA3 recurrent density to give the small assembly
  ...
  by density 0.5 perm (0.471) EXCEEDS cue (0.463). Higher density adds cross-assembly cross-talk + self-ignition — the
...
- **BTSP FORMATION is genuine and weight-specific** (that piece is intact) — `no_encoding` held_cue = 0.000 max (the
  ...
  `cross_dw ≈ 0` (−0.015..+0.001), `w_within` tracks `btsp_w_max`, `genuine_formation`=True on every row. BTSP writes
...
size-variable across the 3 co-stored assemblies, 14–33 cells), while the slow-NMDA completion's fixed global FS-basket
...
Jaccard ≈ 0.01–0.02 vs pre-assigned, mossy-lesion collapses to 0, index-space 6/6). BTSP FORMATION is genuine and
...
BTSP-completion GO 0/6 OU-off and 0/6 OU-on — and it fails HAND-INSTALLED too, so the seam is the completion operating
...
`research/findings/raw/_gap5_e2e/e2e_6seed.json` (6-seed, both OU, density×wmax sweep),
`research/findings/raw/_gap5_e2e/smoke_s42.json` (seed-42 per-density decomposition). Composition runner:
