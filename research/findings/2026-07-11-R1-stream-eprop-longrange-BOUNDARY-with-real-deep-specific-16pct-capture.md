...
**Verdict (R1 → R1b):** the pre-registered gate returns **BOUNDARY** (`frac_deep < 0.25` vs the all-params BPTT) — but that denominator is CONFOUNDED (BPTT also learns the input embedding, which e-prop does not). **R1b isolates the pure recurrent-credit rule on identical frozen embeddings → the clean deep fraction is ~44% (a genuine PARTIAL), not 16%.** So on identical embeddings, e-prop's one-step-local biological credit captures ~44% of the deep-context recurrent-credit margin full backprop achieves — deep-specific, anti-cheated, credit-structure-dependent — BEFORE the ALIF deep-tail lever (research-expected to push it toward ~0.7-0.85). The honest headline: a real PARTIAL recurrent-credit long-range capture that the confounded gate under-reported as a boundary. (Note: the filename's "16pct" is the confounded number; the clean recurrent-credit figure is ~44% — see the R1b section.)
...
## ✅ R1b RESULT (the confound isolated) — the CLEAN recurrent-credit deep fraction is ~44% (PARTIAL), not the confounded 16% (BOUNDARY)
The additive `BPTT_fixed_win` arm (full backprop but `W_in` FROZEN to the same random projection e-prop uses) isolates the pure recurrent-credit RULE on identical embeddings:
| arm | deep margin | deep gain over fixed floor |
|---|---|---|
| fixed_reservoir (floor) | −2.760 | — |
| plastic_eprop | −2.158 | **+0.602** |
| **BPTT_fixed_win** (recurrent-credit ceiling, frozen W_in) | −1.401 | **+1.359** |
| BPTT_same_net (all-params, learns W_in) | +0.902 | +3.662 |
- **The confound was large:** BPTT_same_net's deep gain (+3.662) is >2.5× BPTT_fixed_win's (+1.359) — **more than half of the all-params ceiling's deep margin is INPUT-EMBEDDING learning, not recurrent credit.** The pre-registered `frac_deep` (plastic / BPTT_same_net = 0.164) was measuring e-prop against a ceiling inflated by embedding learning it never attempts.
- **CLEAN recurrent-credit deep fraction** (plastic gain / BPTT_fixed_win gain, identical frozen embeddings) = **0.602 / 1.359 = 0.443 (~44%)**. ⇒ **on identical embeddings, e-prop's one-step-local biological credit captures ~44% of the deep-context recurrent-credit margin full backprop-through-time achieves** — the diagonal RTRL truncation loses ~56% of the deep recurrent credit but KEEPS 44%. That is a genuine PARTIAL (0.25-0.50), not a boundary — and it is BEFORE the ALIF deep-tail lever (research-expected to lift it toward ~0.7-0.85).
- **Both the shallow fractions are ~0** (plastic shallow gain −0.001; even BPTT can't lift shallow much with a frozen lossy random embedding) — confirming the recurrent-credit story lives entirely at DEPTH, exactly as intended.
- **Honest framing:** the pre-registered gate returns BOUNDARY on its (embedding-confounded) metric; the scientifically meaningful clean recurrent-credit metric returns **PARTIAL ~44%**. Reported side by side; the clean number is the "can the biological credit RULE reach BPTT's recurrent long-range credit" answer.
