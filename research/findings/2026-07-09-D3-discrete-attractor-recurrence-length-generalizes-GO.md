...
Ran A5 (blind seeds 100/101/102, the theorem-backed FF-impossible group). RESULT: discrete-attractor step-delta **0.708** (NOT 1.0), deeper state-track **0.022** (≈chance). The S3 GO held because step-delta was **1.000** (the 36-entry transition learned perfectly, tiny K); at A5's 3600-entry transition table the per-step delta is only 71% learned — likely because the 60 element pool-codes OVERLAP in n_pool=64 (the RNN can't cleanly read each input) — and **autoregressive rollout COMPOUNDS the per-step error** (`0.708^L → chance` over deep rollouts). So the honest mechanism is: **the discrete-attractor length-generalizes IFF the per-step transition is learned to ~100%; a sub-perfect transition compounds to chance over depth.** **⇒ CONFIRMED: with cleaner codes (n_pool 64→256, so the 60 A5 element codes no longer overlap) + more data, A5 step-delta RECOVERS to 1.000 and DEEPER state-track = 0.996** (the continuous RNN + state-sup stay at 0.000). So the A5 error-compounding was a fixable CODE-CAPACITY issue (overlapping inputs → ambiguous transition), NOT a fundamental wall. **The discrete-attractor mechanism length-generalizes recurrent multi-hop composition on BOTH S3 AND the theorem-backed non-solvable A5 (where FF is PROVABLY NC¹-hard, not merely empirically length-limited).** The honest scale caveat stands: larger K needs sufficient code capacity + data for the per-step transition to reach ~100% (else autoregressive error compounds); re-discretization removes DRIFT but not per-step CLASSIFICATION error. The continuous-RNN comparison is decisive (the attractor is NECESSARY — continuous fails even at S3, and 0.000 at A5).
