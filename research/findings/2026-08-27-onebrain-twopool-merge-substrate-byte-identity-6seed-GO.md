...
**Branch-consolidation note (2026-08-27, added on land to `main`):** this finding's cited runner
(`_onebrain_twopool_merge_derisk.py`) and artifact were landed from the `research/onebrain-twopool-merge` branch as
a documentation-only cherry-pick — the runner code itself was NOT merged, superseded by the declarative
`research/runners/onebrain_merge_framework.py` (`2026-08-27-onebrain-merge-framework-DESIGN.md`, which cites this
exact result as the seed of its design). The reconciliation table below (param-het/homeostasis/NMDA/GABA_B/wiring
seams for merging pool #1 + pool #2's organs) is the only place `metacog`/`pragmatic` are reconciled onto a shared
pool — the framework's current `REGISTRY` covers only `surprise`/`world-model`/Group-A, not yet metacog/pragmatic —
so this doc is kept as prior art for that follow-on migration, not as a currently-runnable result.
