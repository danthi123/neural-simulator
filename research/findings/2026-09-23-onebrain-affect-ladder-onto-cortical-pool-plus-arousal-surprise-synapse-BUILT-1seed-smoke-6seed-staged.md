...
**Status: PARTIAL.** This is a 1-seed smoke plus a 1-seed weight calibration. The pre-registered 6-seed gate (M1-M7
migration, X1-X7 cross-edge) is queued on the mini-PC pool and has not been harvested. Everything here is DEFAULT-OFF.
It is not wired into production by default, and no default was flipped.
...
- `research/runners/onebrain_affect_pool.py`
  ...
  - **`AROUSAL_XEDGE`** connects all 160 arousal-rung neurons to the `surprise` pool at a fixed weight of 0.05. It
    sits behind the transmission gate `affect_arousal_to_surprise`.
...
| **0.05 (chosen)** | **+0.224** | **+0.224** | **0.007 (same as base)** |
...
relays are silent there (asserted on every step). At w=0.05 the shift is gain-like: contradict rises and confirm
does not move. The rule, fixed before any 6-seed result existed, picks the smallest weight whose shift is at least
0.10 Hz for both valences while confirm stays under threshold.
...
5. **Seed 42 was used for calibration** and is also included in the 6-seed run.
...
This needs every check in M1-M7 and X1-X7 to hold on all 6 seeds. A missing arm counts as not passed.
