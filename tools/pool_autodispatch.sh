...
node_is_idle() {
  ...
  out=$(timeout 12 ssh -o BatchMode=yes -o ConnectTimeout=6 "$1" \
        "echo \$(nproc) \$(cut -d' ' -f1 /proc/loadavg) \$(pgrep -c -f '^[^ ]*/?python[0-9.]* .*-m [r]esearch\.runners' 2>/dev/null | head -1) \$(awk '/MemAvailable/{print int(\$2/1048576)}' /proc/meminfo)" 2>/dev/null) || return 1
  set -- $out
  local cores="${1:-0}" load="${2:-99}" procs="${3:-99}" avail_gb="${4:-0}"
