...
- **`ROADMAP.md`** — the plain-language status surface (synced through the 2026-08-10 cycle; its §"2026-08-10 INTEGRATION PIVOT + gap#5 CLOSE" block is the canonical prose for the current frontier). Section 3 is the one-screen picture; §8 lists the stand-ins; §9 is the honest frontier. Its "Project shorthand" table (top) decodes every coinage.
...
**CURRENT FRONTIER (as of 2026-08-11, through the 2026-08-10 cycle) — THE INTEGRATION ARC.** The owner steered from *mechanism-first-in-isolation* to **continuous integration**: wire validated GO faculties into the LIVE conversational loop (`research/runners/_stageA_full_integration_derisk.py::build_one_brain`, driven by `_conversation_turing_test_derisk.py`) and gate on *"did the conversation get BETTER"* on the real 14-turn chat, because running the actual chat is what exposes mis-scoped isolated GOs. Canonical prose: `ROADMAP.md` §"2026-08-10 INTEGRATION PIVOT + gap#5 CLOSE".
Two headlines: **(A)** gap#5 episodic memory is mechanistically CLOSED at the readout via an intrinsic per-cell dendritic dAP completion (6/6 GO), now wired into the chat's turn-7 recall on the numpy substrate; **(B)** seven integration results (#1–#7) landed on the real chat this cycle (§10.1, §10.3(b)).
**Honest scope that governs all of it:** the eval is a **TOY WORLD** (2 agents, 3 actions, a small fact set); of 14 turns only the in-domain ones genuinely engage and the ~8–10 out-of-domain turns correctly **ABSTAIN** — silence is the moat working, not a gap. The generator 'mouth' is still the declared Broca-like articulation scaffold (a 0.5B spiking-LLM, mostly OFF/CPU in this CPU-lane work); grounded CONTENT is the brain's own RF-VSA read.
...
- **gap#5 episodic composition seam — mechanistically CLOSED at the readout (6/6 GO, 2026-08-10).** The emergent loop now composes end-to-end: emergent-DG *selects* the assembly → BTSP one-shot *forms* the recurrent attractor → an intrinsic **per-cell dendritic dAP readout completes it cue-specifically**. The prior recurrent completion hit a self-drive-vs-cue wall because the emergently-selected assemblies are SMALL (~14–33 cells) — a control in which a random 72-cell assembly failed identically eliminated SIZE as the cause (the real cause is self-drive-vs-cue coupling at composition scale).
  Lever B's size-independent apical UP/DOWN latch (`enable_two_compartment_dap`) reads them: held-out members reach UP 0.29–0.83 from a partial cue, while permuted-cue / silent-rest / no-encode / recurrence-zeroed are ALL exactly 0.000 and the linear coincidence-off control FAILS 6/6 (the plateau is the completer, not the weights alone); mossy-lesion collapses membership 100%. Findings `2026-08-10-gap5-lever-B-dendritic-dAP-readout-*` + `544c0b742`.
  **Honest scope:** a de-risk GO at density 0.5, read DURING-cue; the shipped default completion is still the recurrent slow-NMDA read, so wiring dAP as the default was the integration step — done for the live chat by #2b (§10.3(b)). Named residuals (each a next method, none a wall): a homeostatic per-cell plateau threshold so `kthresh` self-adjusts, robustness across the 0.12–0.5 density range, and the hold-after-cue-offset standalone-attractor test. **Distinct from** the tested-NEGATIVE dendritic deep-CREDIT/BDSP rule — the plateau READS a BTSP-formed weight; no hidden-credit learning (gap#4 stays a mapped, deprioritized boundary).
...
- **Honest boundary you must not paper over:** genuine metacognitive sensitivity (meta-d′) is a *measured 6-seed negative* — meta_d≈0 on 4/6 seeds because the monitor reads winner-*rate* (a wrong winner also fires strongly = margin-blind). The named fix (read winner−runner-up margin) is not yet built for this meta-d monitor — though the same read-the-asymmetry principle DID land 6/6 for the turn-13 self-model certainty band via an opponent comparator (see the integration bullet below). And **production abstain is still a host `if`**: `sim/constrained_realize.py:30-34` and `sim/grounded_decode.py` call `research.runners.abstention_gate.gate(ranked, threshold=650)` byte-unmodified — the validated self-schema does not drive it.
- **Integration-arc honest read-outs, live on the 14-turn chat (2026-08-10):** the **sub-clausal no-confab moat** now verifies EACH main AND subordinate clause of the generator-mouth's reply against the neural VSA moat (not just the leading SVO) — on the real chat, confabulations 3→0 (invented `because …` causal clauses dropped; grounded facts, affect tone, the curiosity-ask, and honest abstains survive), 6/6 seeds with zero SVO-moat / `because` leaks (`2026-08-10-INTEGRATION1-*`).
  Turn-5 *"how do you feel?"* → a FUNCTIONAL affect self-report from the spiking valence differential (+0.07, warmth level 3), explicitly *"an honest functional read-out, not a feeling"* — never phenomenal. Turn-13 *"are you a simulated brain?"* → an honest STRUCTURAL self-affirmation (*"a simulated spiking substrate … not a person"*) plus a **graded certainty band** from the self_schema confidence relay: a per-class **opponent comparator** (each workspace class excites its own slow-NMDA subpool and cross-inhibits the other; read = |rate(opp₁) − rate(opp₀)|) reads the ASYMMETRY of the settled competition, so confident-vs-tie separation is +0.066 mean and ALL 6 seeds clear the +0.02 bar (`2026-08-10-INTEGRATION-3c-*`).
  **Honest scope:** the read is still a host abs-subtraction over two neural population rates (a fully-neural rectified-opponent pair is the named next step); the disclaimer/self-report *templates* are declared host scaffolds; the production abstain path above is unchanged by this.
...
- **Grounded conversational breadth wired into the live chat (2026-08-10, 6/6 GO each).** #6 stores relational SVO facts *mined from the corpus the brain heard* (TinyStories) via one additive `vocab` kwarg on `build_one_brain`: grounded-subject breadth 2→9, grounded replies 4→9 vs the matched 6-fact baseline, confab=0, the moat holds by construction, OOD turns still abstain; anti-cheats hold (permuted-corpus overlap ~0 ⇒ knowledge is token-ORDER-derived; the empty-kb control shows competence is in the FACTS not the vocab; capacity holds to K=40, under the ~320-concept single-bridge bound).
  #7 is its EMERGENCE-BAR burn-down at demo scale: the brain is TAUGHT 3 facts by corrective interaction so each fact becomes an **e-prop WEIGHT CHANGE** on a spiking Izhikevich readout (`OnBridgeEpropNet`) — taught-recall 0→3/3 while a FROZEN readout recalls 0, the facts are absent from `comp.kb`, and lesioning the learned gate collapses the novelty margin (confab returns). A follow-on burn-down then made the no-confab MOAT itself **fully spiking** (the standing v320 anti-Hebbian gate read through a resonate-and-fire phasor conjunction, 6/6), so at demo scale BOTH the acquisition AND the moat are the brain's own.
  **Honest scope:** #7 is SMALL-K (3), a jointly-taught demo standing BESIDE #6's host-`comp.store` breadth; scaling #7 to #6's K=40 is gated on the still-open continual/sequential multi-fact arc (§10.3(c)). Findings `2026-08-10-INTEGRATION-6-*`, `-7-*`, `-7-burndown2-*`.
...
**(b) Grounded integration — the "someone home" gap (the charter's real target) — NOW THE ACTIVE FRONTIER.** The 2026-08-10 owner pivot turned this from a plan into the working arc: validated faculties are being wired into the LIVE 14-turn chat and gated on *"did the conversation get better."* Landed this cycle (each verified on the real conversation; all `SIM_BACKEND=numpy`, additive/runner-side, byte-identical default):
the sub-clausal no-confab moat (#1, confab 3→0, 6/6); an honest turn-7 episodic dialogue recall now flowing through the **spiking** gap#5 dAP completion on numpy (#2 single-seed behavioural; #2b — correction below); functional affect + certainty-band self-reports (#3/#3c, 6/6 clear the bar); an honest causal-query disclaimer (#5: turn-4 *"why did the dog go east?"* CONFIRMS the stored fact via the moat, then HONESTLY DISCLOSES the absent causal faculty — *"I have learned associations, not causes; I will not invent a reason"* — 6/6, confab=0, only turn 4 changes); and corpus-/plasticity-learned grounded breadth (#6/#7, 6/6 each — §10.1).
A composed live chat (seed 42) gives 6 honest replies (facts / affect / curiosity-ask / episodic / self-model), 8 honest silences, 0 confabulations.
**The correction doc rewriters must get right (registered in `docs/RETRACTED.md`):** the earlier claim that the spiking dAP recall path was numpy-*backend-blocked* / a forward-Euler limit is **SUPERSEDED** — it was mis-verified at `kthresh=30` (fires on NEITHER backend); at the corrected `kthresh=8` it fires cue-specifically 6/6 on cupy AND on the live numpy substrate (seed 42: cat 0.929 / dog 0.909; the smallest 13-cell emergent assemblies fire; permuted / no-cue / lesion 0.000). Do NOT re-cite the old numpy-negative.
**What is still genuinely open (the integration arc has NOT closed these):**
...
- **The "one brain" merge is not done.** #7 stands up two co-resident Izhikevich bridges in one process — NOT merged into one shared substrate; that merge is the named next arc (**#7 burn-down 1**). Named residuals that remain host: the argmax patient read-out (a neural FS-WTA word-decode de-risk now matches the host cosine-argmax 1.000, 6/6, decision on spikes via lateral inhibition, but is not yet wired into `ConceptFrameSpeaker.spell`), the host-oracle fact-CONTENT inside episodic recall, and #6's grounded breadth still riding a host SVO-mine + `comp.store` write.
...
5. **Pick up the mission-critical thread**, not a narrow faculty polish: the CURRENT FRONTIER is the **integration
   arc** — wiring GO faculties into the live 14-turn chat and gating on *"did the conversation get better."* Seven
   integrations landed through the 2026-08-10 cycle (§10); the named next arc is **#7 burn-down 1 — merge the two
   co-resident bridges into ONE brain**. The board's top block (`GAP_CLOSURE_MISSION.md` CURRENT STATE) holds the
   exact next action; if it points at older isolated-faculty work, re-judge it against the integration frontier +
   the charter first.
